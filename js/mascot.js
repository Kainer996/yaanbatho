...
(function () {
  ...
  // Floor: walk just above the MUSIC button — already fixed at bottom-right of page
  // Anchors to visible landmark instead of invisible viewport edge
  function floorY() {
    const music = document.querySelector('.btn-music-float');
    if (music) {
      const r = music.getBoundingClientRect();
      return r.top - CFG.charH - 6;
    }
    return window.innerHeight - 130 - CFG.charH;
  }

  // Header: character top = just below header's bottom red line
  let _headerTopY = 0;
  function headerY() { return _headerTopY; }
  function measureHeader() {
    const h = document.querySelector('header');
    const hBottom = h ? h.getBoundingClientRect().bottom : 70;
    _headerTopY = hBottom + 4; // character top starts 4px below the red line
  }
  ...
  // ─── POSITION ──────────────────────────────────────────────────────────────
  function applyPos() {
    // y = viewport distance from top to character top — use top: directly (reliable on mobile)
  ...
  function doGrapple() {
    ...
    // Grapple hook fires at the header's bottom red line
    const tx = window.innerWidth / 2;
    const ty = headerY(); // hook lands right where character will walk
