...
(function () {
  ...
  function init() {
    ...
    // First quip after 5 seconds, then auto every 12-22s
  ...
  function fireQuip() {
    ...
    // Check if next quip is a "continuation" (starts with ...)
    const nextQuip = CFG.quips[quipIndex % CFG.quips.length];
    if (nextQuip && nextQuip.startsWith('...')) {
      // Fire follow-up bubble after current one fades (3.5s + 0.8s gap)
