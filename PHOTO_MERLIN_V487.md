...
**Where and when.** The page sends the rough place (two decimals) and the BirdNET week with the photo. It does so only for a fresh camera shot or a library photo from the last two days, and only while the location switch is on. `photo_id.py` runs the BirdNET Geomodel V3.0.2 that already serves sound, in the same Flask process. It rounds the place to 0.1° first. Nothing is stored.

**Gemini sees the local list.** The worker (`photo_gemini.py`) sends the picture first, then the question: the place to half a degree, the season ("late September") and up to 300 local species, most likely first. Gemini looks once, with "medium" thinking. It gives field marks, a bird box and up to five ranked species with a plumage.

**The range decides the order.** The adapter maps every name onto the geomodel's labels. Same Latin name, same common name (Grey = Gray), or a genus move with the same bird word all count as one bird. Old names for split species follow the place: `Saxicola torquatus` in Lancashire is a European Stonechat; `Larus argentatus` in Maine is an American Herring Gull. Each score is weighed by the range: a bird expected here (occurrence ≥ 0.05) keeps its weight; rarer birds fade down to a floor of 0.02, so a vagrant stays pickable but cannot win on a guess.

**The player confirms.** The result shows the photo, then ranked bird cards with art, the Latin name, a match label, "Likely here", "Rare here" or "Not expected here", and the plumage. Each card has **This is my bird**. Nothing enters the Birdex until the player taps it. A bird outside the Burbz catalogue shows "Not in Burbz yet" and cannot be chosen.

A match counts as strong (HTTP 200, `found`) only when all hold: the ranked score is at least 0.80, it leads by 0.20, Gemini itself gave it at least 0.50, the bird box passes the pixel check, there are two field marks, and it is likely here. Everything else is `pick-your-bird` (HTTP 422) with the same cards.
