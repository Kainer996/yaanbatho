# Camera recognition v407 — historical checkpoint, superseded by live main

> **CRITICAL CORRECTION — READ BEFORE ANY IMPLEMENTATION.** A fresh production read after this historical checkpoint was uploaded found main/live already advanced to **e7adaf5d596d82f41f9f7471d3b12b15ef7d672f (Home farming v409)**. Photo v407 and photo journal v407b ARE ALREADY PUBLISHED. The active healthy worker is `bioclip25-birder-local`, bundle `photo-models-v407`, source SHA `d3e6df24099f0a6faaeee12abdc10fc427a52d3b5d5256ead4e315323f0ce677`, bundle hash `a147ebc64060a3bd874d179e2386f79376cfd6566ab8f9365d853b96dfeedaa0`, models `/opt/burbz-photo/models-v407`. Current repository worker hash exactly matches live. **This checkpoint's code and remaining unpublished-work claims below are historical, superseded by main. Do not merge this branch over current main or repeat the completed release.**
>
> **FIRST ACTION:** fetch current main and read `public/burbz/PHOTO_RECOGNITION_V407.md`, its production evidence in `/root/burbz-photo-recognition-evidence/`, and latest release history. Audit/rebase against actual current main before any work. Current release records a paired 33-case comparison: 18/19 clear birds identified, 21/24 total bird photos, no wrong accepted identities, all nine negatives rejected; the clear frontal gull still abstains. Fourteen real HTTP controls and public/PWA checks were subsequently completed. Historical 24-case results here do not supersede them.
>
> **Newest user clarification:** the user previously found Google Gemini accurate and is now asking how to restore/pay for that successful integration after the free replacement's failures. A separate New voice chat is diagnosing this, read-only. The active Burbz web process still has a nonempty Gemini key and selectors `gemini` / `gemini-2.5-flash`, but the local-only adapter intentionally ignores them. Key validity, billing and spending approval are not established. **No new paid request, service switch or speculative deployment is authorized by that diagnostic conversation.** Coordinate before continuing a broad local-model programme. Only ONE cloud continuation has been submitted by the existing first-person UI task; do not launch another.
