...
(function(root){
  ...
  const PIN={'world_nature_core.js':'alderwing-steady-v472-20260925','world_nature.js':'alderwing-seamless-v480-20260925','world_horizon.js':'alderwing-seamless-v480-20260925','world_water_core.js':'alderwing-nature-v470-20260925','world_water.js':'alderwing-nature-v470-20260925','open_land_core.js':'alderwing-flight-shores-v418-20260915','shore_water.js':'alderwing-nature-v470-20260925','flight_craft.js':'realistic-flight-v478-20260925','wilderness_places_core.js':'wilderness-discoveries-v406-20260914','wilderness_places.js':'wilderness-discoveries-v406-20260914','wilderness_places.css':'wilderness-discoveries-v406-20260914','building_work_core.js':'builder-help-v404-20260914','building_work.js':'building-opening-v405-20260914','building_work.css':'craft-home-repair-v440-20260922','world_sky.js':'alderwing-seamless-v480-20260925','village_discovery_core.js':'destination-cleanup-v434-20260921','first_person_map.js':'alderwing-flight-shores-v418-20260915','village_world_core.js':'alderwing-seamless-v480-20260925','village_world.js':'alderwing-seamless-v480-20260925','village_harvest.js':'calm-asmr-v462-20260924','village_discoveries.js':'destination-cleanup-v434-20260921','village_discoveries.css':'destination-cleanup-v434-20260921','village_walk.css':'alderwing-qst-ui-save-v432-20260921','village_harvest_core.js':'map-pictures-v374-20260908','interior_life_core.js':'map-pictures-v374-20260908','interior_life.js':'calm-asmr-v462-20260924','academy_flight_core.js':'realistic-flight-v478-20260925','academy_flight.js':'realistic-flight-v478-20260925','building_rooms_core.js':'tavern-hall-open-v450-20260923','building_rooms_scene.js':'hall-music-v458-20260923','building_rooms.js':'realistic-flight-v478-20260925','village_walk_core.js':'alderwing-followups-v417-20260914','first_person_hud.js':'hands-on-world-tutorial-v429-20260920','first_person_hud.css':'craft-home-repair-v440-20260922','village_walk_scene.js':'wilderness-discoveries-v406-20260914'};
  let session=null,dependencies=null,closingFullscreen=Promise.resolve();
  // A pilot aloft can look straight down past the dashboard; on foot the
  // original head range stays.
  function clampPitch(player,pitch){const [low,high]=player?.mode==='fly'?[-1.5,1.2]:[-1.10,1.10];return Math.max(low,Math.min(high,Number(pitch)||0));}
  ...
  function load(){
    if(!dependencies)dependencies=Promise.all([
      ...
      script('shore_water.js','BurbzShoreWater'),
      script('open_land_core.js','BurbzOpenLandCore'),
      ...
      Promise.all([script('village_world_core.js','BurbzVillageWorldCore'),script('world_nature_core.js','BurbzWorldNatureCore').then(()=>script('world_nature.js','BurbzWorldNature')),script('world_horizon.js','BurbzWorldHorizon'),script('world_water_core.js','BurbzWorldWaterCore').then(()=>script('world_water.js','BurbzWorldWater'))]).then(()=>script('village_world.js','BurbzVillageWorld')),
  ...
  function close(reason='exit'){
    ...
    s.tutorial?.dispose();s.work?.dispose();s.combat?.dispose();s.harvest?.dispose();s.rooms?.dispose();s.flight?.dispose();s.discoveries?.dispose();s.hud?.dispose();s.unbatch?.();
    s.overviewHomes?.forEach(row=>row.object.visible=row.visible);
    if(s.snapshot){
  ...
  function open(options){
    ...
    function tutorialLocked(){return !!s.portal||s.tutorial?.locked()===true;}
    function fullscreen(){
    ...
    for(const event of ['pointerdown','click'])on(el,event,e=>{
      if(tutorialLocked()&&!e.target.closest('.first-village-guide,.vw-exit,.vw-error')){e.preventDefault();e.stopImmediatePropagation();return;}
    ...
    on(document,'keydown',e=>{
      ...
      if(e.code==='Tab'){
        const scope=s.uiBusy?el.querySelector('.fp-panel:not([hidden]),.il-panel:not([hidden]),.vd-panel:not([hidden])')||el:el;
        const buttons=[...scope.querySelectorAll('button,input,select,textarea,[tabindex]:not([tabindex="-1"])')].filter(b=>!b.disabled&&b.getClientRects().length&&!b.closest('[hidden]'));
        if(!buttons.length){e.preventDefault();scope.focus?.({preventScroll:true});return;}
    ...
    function pointerDown(e){
      if(e.button!==0)return;e.preventDefault();e.stopPropagation();if(s.failed||s.uiBusy||tutorialLocked()||!auto)return;
    ...
    function pointerMove(e){
      if(tutorialLocked()){s.reset();return;}
      ...
      }else if(s.player){const yaw=s.player.yaw,pitch=s.player.pitch,k=e.pointerType==='touch'?.004:.003;root.BurbzVillageWalkCore.look(s.player,e.clientX-p.x,0,k);s.player.pitch=clampPitch(s.player,pitch-(root.BurbzLookSettings?.vertical(e.clientY-p.y)??e.clientY-p.y)*k);root.BurbzAlderwingIntro?.inputLook(el,e.pointerId,e.clientX-p.x,e.clientY-p.y,Math.hypot(s.player.yaw-yaw,s.player.pitch-pitch));}
    ...
    on(window,'blur',pause);on(window,'focus',resume);on(document,'visibilitychange',()=>{if(document.hidden)s.continuity?.save();document.hidden?pause():resume();});on(window,'pagehide',()=>close('pagehide'));
    function resize(){
      if(!s.source||s.closed)return;s.reset();
      const w=el.clientWidth,h=el.clientHeight;if(!w||!h)return;
      s.maxDpr=Math.min(devicePixelRatio||1,1.5,Math.sqrt(720000/(w*h)));
    ...
    function frame(ts){
      ...
      try{
        ...
        s.last=ts;s.frames++;
        if(options.isCurrent?.()===false){close('navigation');return;}
        s.tutorial?.update();if(tutorialLocked())s.reset();
        autoSync();const before={x:s.player.x,z:s.player.z},movement={side:input.side+(keys.has('KeyD')?1:0)-(keys.has('KeyA')?1:0),forward:input.forward+auto.forward()+(keys.has('KeyW')?1:0)-(keys.has('KeyS')?1:0),lift:(keys.has('Space')?1:0)-(keys.has('ShiftLeft')||keys.has('ShiftRight')?1:0)};
        if(!s.continuity&&!s.room&&!s.flight&&!s.uiBusy&&options.boundaryExit&&core.outwardBoundary(s.player,movement,s.world))leaveForWorld();
        if(s.flight&&!s.room)s.flight.update(dt);
        else if(!s.uiBusy&&!tutorialLocked()){if(s.continuity&&!s.room)s.continuity.move(movement,dt);else core.move(s.player,movement,dt,s.world);}
        if(!s.uiBusy&&s.player.mode!=='fly'&&s.player.mode!=='swim'&&(!s.flight||s.room))s.footsteps?.update(before,s.player,dt,s.room?'wood':s.world.surface?.(s.player.x,s.player.z));else s.footsteps?.reset();
        if(!s.uiBusy&&!s.room&&s.player.mode!=='fly'&&s.player.mode!=='swim'){
        ...
        const turn=tutorialLocked()?0:Math.min(.05,dt)*1.45;s.player.yaw+=((keys.has('ArrowLeft')?1:0)-(keys.has('ArrowRight')?1:0))*turn;
        s.player.pitch=clampPitch(s.player,s.player.pitch+(root.BurbzLookSettings?.vertical((keys.has('ArrowUp')?1:0)-(keys.has('ArrowDown')?1:0))??((keys.has('ArrowUp')?1:0)-(keys.has('ArrowDown')?1:0)))*turn);
        ...
        camera.position.set(s.player.x,s.player.y+(s.player.mode==='swim' ? .35 : options.flight ? .45 : s.seated ? .86 : core.EYE)+motion.bob,s.player.z);camera.rotation.set(s.player.pitch+motion.pitch,s.player.yaw,motion.roll,'YXZ');
        // Speed past cruise widens the view a little; the lens returns as the bird slows.
        const lens=Number.isFinite(motion.fov)?motion.fov:0;if(Math.abs(lens-(s.lens||0))>.02){camera.fov+=lens-(s.lens||0);s.lens=lens;camera.updateProjectionMatrix();}camera.updateMatrixWorld();
    ...
    return load().then(async()=>{
      ...
      if(s.closed||options.isCurrent?.()===false){source?.dispose?.();if(!s.closed)close('navigation');return false;}
      ...
      if(options.continuousWorld){s.overviewHomes=scene.children.filter(o=>o.userData.overheadHome).map(o=>({object:o,visible:o.visible}));s.overviewHomes.forEach(row=>row.object.visible=false);}
      s.world=source.world||(s.flight?s.flight.world:options.room?root.BurbzBuildingRoomsCore.world(root.BurbzBuildingRoomsCore.plan(options.room)):root.BurbzVillageWalkScene.create(root.THREE,scene,source.buildings,source.movers,scene.userData.walkTerrain));
      s.unbatch=source.world||options.room||options.flight?null:root.BurbzVillageWalkScene.batch(root.THREE,scene,source.movers);
      ...
      if((options.room||s.options.startRoom)&&!s.rooms.enter(options.room||s.options.startRoom))throw Error("This building is not ready to enter.");
  ...
  function diagnostics(){
    ...
    return {open:true,tutorialLocked:!!s.portal||s.tutorial?.locked()===true,portal:!!s.portal,auto:s.auto?.state(),work:s.work?.diagnostics?.(),combat:s.combat?.diagnostics(),continuity:s.continuity?.diagnostics(),hud:s.hud?.diagnostics(),harvest:s.harvest?.diagnostics(),flight:s.flight?.diagnostics(),interiors:s.rooms?.diagnostics(),discoveries:s.discoveries?.diagnostics?.(),ready:!!s.player&&!!s.canvas,failed:s.failed,frames:s.frames,running:!!s.raf,player:s.player?{...s.player}:null,dpr:s.dpr,sampleCount:sorted.length,medianMs:sorted[Math.floor(sorted.length*.5)]||0,meanMs:mean,p95Ms:sorted[Math.floor(sorted.length*.95)]||0,fps:mean?1000/mean:0,draws:s.source?.renderer.info.render.calls,triangles:s.source?.renderer.info.render.triangles,segments:s.world?.segments.length,buildings:s.source?.buildings.map(b=>({id:b.userData.buildingId,level:b.userData.modelLevel,construction:!!b.userData.construction,x:b.position.x,z:b.position.z})),memory:s.source?{...s.source.renderer.info.memory}:null};
  }
  root.BurbzVillageWalk={open,close,isOpen,diagnostics,clampPitch,whenClosed:()=>closingFullscreen,continueWorld(mode){const s=session;if(!s?.continuity||s.room)return false;s.hud?.closePanel?.();return mode==='fly'?s.continuity.toggleFlight('fly'):true;}};
  if(/^(localhost|127\.0\.0\.1)$/.test(location.hostname))root.__burbzVillageWalkDebug={state:diagnostics,nature:()=>session?.continuity?.diagnostics(true),world:()=>session?.world,place:(p)=>{if(session&&(p.mode==='fly'?session.world.allowed3?.(p.x,p.y,p.z):session.world.allowed(p.x,p.z))){Object.assign(session.player,p,{y:(session.flight||p.mode==='fly')&&!session.room?p.y??session.player.y:session.world.height(p.x,p.z)});if(p.mode)session.continuity?.toggleFlight(p.mode);return true;}return false;},resetSamples:()=>{if(session){session.samples=[];session.intervals=[];}}};
