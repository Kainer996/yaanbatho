Fold screen v488 (`fold-screen-v488-20260925`): Adam's Galaxy Fold got stuck on "Tilt your phone again to go back to portrait". An open book-style foldable is near-square, so its browser view stays wider than tall whichever way it is held, and the button never woke. `alderwing_intro.js` now treats a screen whose sides differ by less than 1.3× (`square()`, from `screen.width`/`height`) as already turned for both tilt lessons, with its own words and an open Continue. Ordinary phones and tablets from 1.3× up still tilt. Test: `tests/test_fold_screen_v488_20260925.cjs`.
