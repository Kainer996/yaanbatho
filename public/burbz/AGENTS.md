...
Current release: 2026-08-25 (`art-same-origin-v325-20260825`) — **the artwork stops coming from GitHub.** Not a feature: a quota fix, and the reason the Pages deploy had failed every day since 2026-08-20. The repo keeps ~1.6 GB of bird art in Git LFS; GitHub's free allowance is **1 GB of storage and 1 GB of download a month**, and when it is spent GitHub **BLOCKS** further LFS downloads. There is no billing on this account, so no money makes the block go away — only spending less does. **Two leaks, both closed.** *Players:* at boot `index.html` rewrote most art paths onto `BIRD_ART_GITHUB_RAW_BASE` (five rewrite sites — the `BUILT_IN_BIRD_ART` loop, the manga-warrior pass, `birdCutoutUrlFor`, the `MAP_BIRD_CUTOUT_ART` loop, and the ordering around the `GENERATED_ART_COMPLETION` override), and `sw.js` **precached ~292 of those GitHub URLs on every service-worker install, for every player**. With an Android TWA launch imminent that was about to get very much worse. *Deploys:* `scripts/update-live-burbz.sh` curl'd EVERY art file from the LFS endpoint on EVERY deploy — ~1.6 GB, several times a day, with no player involved at all. **The comment that justified all of it was stale.** It claimed the VPS serves "LFS pointer text"; yaanbatho.com has been serving real image bytes, byte-identical to GitHub, for a long time — verified across main paintings, RPG paintings, warrior art, cutouts and habitat backgrounds before a line was changed. The updater's own pointer-detection guard re-proves it on every run. **Nothing was deleted.** The 1.6 GB stays in LFS untouched as the second copy; this release changes only where files are fetched FROM. **Verify before you touch anything:** `scripts/check-burbz-art-on-vps.sh` enumerates the complete referenced set from the real sources — the literal maps in `index.html`/`sw.js`, every expansion and art-release module, the warrior slugs, the derived `file.png → cutouts/file_cutout.png` names and the habitat backgrounds — and curls each against the VPS, failing any file that answers 200 with ≤300 bytes (a pointer is ~132, so a pointer can never pass). It ran clean: **1543/1543 required files and 435/435 derived cutouts present, zero gaps**, so nothing had to be filled. Re-run it before any future art change. **The backup:** `/var/backups/burbz-art/burbz-art-20260825/` — 4146 files, 3.59 GB, outside the web root and outside every git checkout, with a sha256 manifest alongside and zero LFS pointers in it. **The updater now sources art locally, never downloads it**: `stage_art_file` prefers the repo checkout, falls back to the live directory, and a file already correct on the server is left alone. **Keep the pointer guard** — `is_lfs_pointer`/`pointer_die` fail the deploy loudly and name the backup to restore from, because a local source is only safe while something refuses to ship 132 bytes of text as a painting. **One deliberate survivor:** the `cacheableArtHost` github.com allowance in the sw fetch handler stays for ONE release, so a client mid-update with old remote URLs in flight is not broken. Remove it next release. **No migration for old saves** — `syncFlockArtUrls` re-resolves `bird.artUrl` from the maps on boot, so a stored GitHub URL heals itself. **Five suites pinned the old rerouting and were retargeted, never weakened** (`test_uk_bird_expansion_50`, `test_regional_bird_completion_20260715`, `test_barracks_ui`, `test_generated_art_restore_20260730`, `test_live_updater_completeness_20260803`) — each keeps its original claim and names this release in a comment. Contracts: `tests/test_art_same_origin_20260825.py` (16 tests) + `tests/run_art_same_origin_evidence.js` (7 browser checks). **Two traps in that harness, both worth copying:** serve from `public/`, NOT `public/burbz`, or every absolute `/burbz/…` art path 404s; and reach it on **localhost**, not 127.0.0.1, or `index.html` never registers the service worker and the precache half of the proof silently does not run. Suite 1803 passed, 5 skipped — the 7 reds are all already red on `origin/main` and untouched. No core changed, so no `?v=` moved.

Previous release: 2026-08-25 (`forge-opens-on-the-anvil-v323-20260825`) — the Forge opens on its Crafting tab. Yaan's ask: "when the player clicks to enter the forge, that forge screen opens on the Crafting tab please, not on the Equip tab". Crafting is what the room is *for*; equipping is something you do to a bird. **Where the reset has to live:** the dock routes generically (`data-game-route data-screen="forge"`), so `switchScreen` is the only place every entry passes through — putting the default in `renderForge` would fight the player's own tap, and putting it on the buttons would leave the dock reopening on whatever was last shown. `forgeSetTab` (the tap) never consults the default, so a tab the player picks sticks while they are inside; only *entering* resets. **Two callers keep their own tab.** The battle board's "Gear up" and the Stores' "EQUIP YOUR FLOCK AT THE FORGE" say equipping in their own words, so they call `openForge('equip')` — shipping the new default without them would have made two buttons lie. **Two traps this hit.** `switchScreen` returns early when the screen is already open (`if (name === currentScreen) return;`), so `openForge` from *inside* the Forge was silently ignored until it handled that case itself — caught in the browser, not by reading. And `openForge` is called from inline `onclick`s while the game is an IIFE, so it must be added to the `Object.assign(window, {...})` export list or the button throws on tap; the first browser run failed on exactly that. A one-shot `forgeEntryTab`, spent by the next entry, keeps the override from sticking. Contracts: `tests/test_forge_opens_on_the_anvil_20260825.py` (11 tests). No core changed, so no `?v=` moved. Suite 1719 passed, 17 skipped — plus the 6 already red on `origin/main` from PR #248. Browser run at 360px covered all five paths: dock → Craft, a tap on Equip sticking, leaving and returning → Craft, `openForge('equip')` from inside and from outside → Equip, bare `openForge()` → Craft. **Merged with `empire-grid-v322-20260825`, which landed on main mid-flight.** Renamed v321 → v323 so the build stamp is the newest marker in the lineage; v319 and v320 keep the numbers main deliberately left free. **The grid supersedes v319 outright** — main's tiers already lead with VILLAGES and already carry `showCounties = !!(regions.length || regionCandidates.length)`, and `test_empire_grid_20260825.py` pins both, so this branch's `test_villages_first_county_merge_20260824.py` was retired rather than left asserting a `navTabsHtml` that no longer exists. **The grid's Lord Mayor needed carrying, not copying.** empire-grid gave `rolePostCardHTML` an `options.title` so a Town Hall desk could be titled by its caller; v320 had already removed that card's head, so the override had nothing to feed. `rolePostTitle(scope, key, role)` derives it once through `empirePostTitleFor`, and every surface — badge label, desk row, sheet head, holder line — reads it, so no caller can forget and a new surface gets it free. **Watch for the identifier the merge orphans:** dropping the card's head also dropped `const roleTitle`, while the holder line still referenced it. A render-time ReferenceError no assertion would have caught — only a browser boot.
