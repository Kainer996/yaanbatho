...
Photo recognition v407 (`photo-recognition-v407-20260914`) is prepared for owner-approved publication. It upgrades to pinned BioCLIP 2.5 visual weights and matching taxonomy, retains the tested bounded two-view consensus and separate Birder worker, and adds explicit full-photo identification through the existing cancellation/save funnel. Do not reintroduce the rejected all-square internal preprocessing experiment or weaken thresholds to pass photos. Keep models-v407 separate from the old models rollback bundle; loaded source/model/bundle/manifest hashes and fourteen real photo plus unchanged BirdNET proofs are mandatory. See [PHOTO_RECOGNITION_V407.md](PHOTO_RECOGNITION_V407.md) for sourced app research, evidence and limits.
