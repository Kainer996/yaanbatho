Alderwing living water v496 (`alderwing-water-v496-20260926`): Yaan's screenshots showed still, flat water, weird waterfalls and a reservoir split in two. One water shader now lives in `shore_water.js` (`CHUNK`: `shoreTap`, `shoreStill`, `shoreSurface`) and is shared by streams (`world_water.js` `streamMaterial`), lakes and the sea (`style`), and the distant water (`world_horizon.js`); all use the shared uniforms from `BurbzShoreWater.uniforms()` and `sky.time` is the one water clock. Ripples come from one mipmapped tile made at load (`world_water_core.js` `texture()`: ripple slopes RG, foam B, caustics A), advected with a two-phase flow and anchored to the world, so tile pieces never seam. `shoreSurface`'s last argument is the bank angle: narrow becks mirror their banks, open water (0) mirrors the sky. Stream ribbons no longer lift over falls; `build()` returns `shape` (flow x/z, half width, part: 0 stream, 1 pool, 2 wet rock under a fall), a `curtain` for each fall (owned by the lip's chunk) and `splash` droplets; it skips quads inside mapped lakes (`wet`) and overlapping tile pieces. Settlement rivers (host and neighbours, `riverCorridors()` in `village_world.js`) run as stream lines with per-sample widths (`prepare(...,widthAt)`); their flat planes hide and the v408 stepped cascades are gone. Lake and sea chunk materials come from `waterSurface()` (ground clone + `styleGround`), so they bend onto the distant water. River areas flow along the nearest mapped centre lines (`flowSegments`, up to 8 per chunk). Near running water `createDrift` floats foam and leaves and `createSound` mixes three ElevenLabs loops (`assets/audio/water/`) from `listen()`; gated by `BurbzCalmAudio.birds()` and silent indoors. Checks: `node --test tests/test_alderwing_water_v496.cjs`; browser proof `node tests/run_alderwing_water_v496.cjs` (VIEWS=beck,waterfall,rapids,reservoir,river,village for a subset). Details: [../../ALDERWING_WATER_V496.md](../../ALDERWING_WATER_V496.md).

Photo Merlin v494 (`photo-merlin-v494-20260926`): photo ID works the way Merlin does. The page sends the rough place and BirdNET week (fresh camera shots and library photos under two days old, location switch on). `photo_id.py` runs the BirdNET Geomodel already loaded for sound, sends Gemini a local species list, maps every name onto the geomodel labels (synonyms, genus moves, place-decided splits) and re-weighs the ranking by range. `photo_gemini.py` makes one medium-thinking call, picture first. The cropper shows ranked bird cards; nothing enters the Birdex until the player taps This is my bird. This supersedes v425's "never turn tentative output into a discovery": the player may pick only from this photo's receipted matches, never from the catalogue. Keep `photo-gemini-v494`/`merlin-v494` aligned across page, `photo_queue.js`, adapter, worker, installer and proof; keep the place out of the ledger digest (the adapter re-weighs stored readings per request, and stable proof ids must never meet request-conflict). See [../../PHOTO_MERLIN_V494.md](../../PHOTO_MERLIN_V494.md).

Calm sound v492 (`asmr-sound-v492-20260925`): iPhones ignore `audio.volume`, so music and taps played at full mastered level. `audio_core.js` now routes every sound through one shared `AudioContext` (`sharedBus()`: ui/sfx/nature/music inputs → master → -5 dB high shelf → gentle compressor, plus a dark room reverb). `createAudioManager` uses the bus when the caller passes no `Audio`; callers with their own `Audio` keep the element path and its tables. Tap, page, unlock, coins and error are synthesized (`SOFT_SOUNDS`) with envelopes that reach silence; files fade in and out; stop() fades; a full voice pool releases its oldest voice. Bus levels live in `BUS_VOLUMES`. Music is routed through a gain node (`route` option on `createMusicManager`, `bus.routeMedia`) at `BURBZ_MUSIC_BASE_VOLUME = 0.045`, low-passed at 3.6 kHz. `asmr_soundscape.js` adds air, leaf rustles, rain drips and a far wind chime under every screen; it must stay silent while the microphone listens (`allowed` gate in index.html). Garden birds and `getAudioCtx` share the bus context. Checks: `node --test tests/test_asmr_sound_v492.cjs`, browser `node tests/run_asmr_sound_v492.cjs`. Details: `ASMR_SOUND_V492.md`.

Home hub v491 (`home-hub-v491-20260925`): Home is the hub. Every Home box is one big button: a tap anywhere in it opens its screen (`boxAction` in `scan_home.js`), while rows and tiles inside keep their own actions and the completed-notices sheet is excluded. Training opens the Training Hall (`openAcademyRoomHudShortcut`), and a drill row opens the room it runs in (`room` on snapshot rows, carried by `scan_home_core.js`); keep `openTrainingHub` for its other callers. Villages, Towns and Regions column titles are `.desk-empire-open` buttons with counts; they open the Empire's `villages`, `towns` and `realm` pages (target kind `empire-page`, gated on `village`), and Regions scrolls to `#empireRealmPanel`. `data-home-attention` gives a gold edge to Crafting (gear ready), Kitchen (hungry or urgent bird), Training (drill done) and Hospital (bird needs care). Short landscape (`data-home-layout="short-landscape"`) puts the Empire, scanner and quest strip on the left three columns, the Academy tall on the right and the four rooms in one bottom row, with wide Empire tiles; the Academy's ready chip moves left because Merlin perches there. The Empire track is sized from its measured heading and column titles, so the first tile is never clipped. Portrait keeps screenshot B. Checks: `node --test tests/test_home_hub_v491.cjs`, browser `node tests/run_home_hub_v491.cjs`. Details: `HOME_HUB_V491.md`.

Academy tree and garden birds v490 (`academy-garden-birds-v490-20260925`): once the house is built, the Academy grows beside it as a tree (`BurbzAcademy3D.buildHomeTree`, 0.56 scale, woodland-style `leafMasses` canopy and plain warm bark; the Academy screen's own tree is untouched). It hangs only the buildings `academyBuiltRoomIds()` returns, via `window.BurbzAcademyBuiltRooms`. Its spot lives in `playerHome.academyTree`, chosen by `pickAcademySpot` clear of everything already placed and saved with the next change; never move or drop a player's items for it. `ACADEMY_TREE.clear` keeps furniture and plots off its roots; `ACADEMY_TREE.walk` keeps the player out of the trunk and low treehouses. Garden birds live in `garden_birds.js` (robin, blue tit, great tit, chaffinch, goldfinch, wren, blackbird; twelve birds, four meshes each on one material). They perch on the tree, house and garden things, forage, fly in bounds or straight, crab in a side wind, face into it, flee a close player and roost in the tree at dusk. Songs are ElevenLabs Sound Effects takes in `assets/audio/garden-birds/`, placed in space with Web Audio. **They must stay silent while the microphone is open**: `BurbzCalmAudio.birds()` gates them and `setBurbzMusicSuppressed('sound-scan', true)` hushes them at once, so Merlin's wand never hears the game's own birds. Keep these songs out of the SFX bank. The v487 treetops ambience (`BurbzCalmAudio.birdsong`) also drops to silence while the microphone is open. Checks: `node tests/test_garden_birds_v490.cjs`, browser `node tests/run_garden_birds_v490.cjs`. Details: `ACADEMY_GARDEN_BIRDS_V490.md`.
Fold and full screen v489 (`fold-fullscreen-v489-20260925`): Adam's Galaxy Z Fold 8 Ultra got stuck on "Tilt your phone again to go back to portrait". An open book-style foldable is near-square (2504×2256), so its browser view stays wider than tall whichever way it is held. `alderwing_intro.js` treats a screen whose sides differ by less than 1.3× (`square()`, from `screen.width`/`height`) as already turned for both tilt lessons, with its own words. The portrait lesson is the last one, so its Continue never locks on any phone; turning upright still finishes it by itself (`tilting` drives that, not the button). The landscape lesson still waits for a tilt. Yaan also asked for full screen: `#headerFullscreenBtn` sits left of Settings in a browser tab that can go full screen and hides once on or in an installed app; Settings has the `#toggleFullScreen` switch wherever the Fullscreen API exists (not iPhone Safari). Merlin's opening overlay covers the header, so the button is reachable after it. Test: `tests/test_fold_fullscreen_v489_20260925.cjs`.

Village folk v486 (`village-folk-v486-20260925`): Yaan said the human NPCs looked silly and asked for AAA village animals. Every villager and animal is now one `THREE.SkinnedMesh` built by `BurbzSettlementModels.rig(THREE)` in `settlement_models.js`: bones with rest positions in model space, parts bound to a bone or eased between two, merged into one draw call. Phones without float vertex textures get jointed rigid pieces instead (`skinningSupported`, `setSkinning`, set from the renderer in both scene builders). `resident()` builds storybook folk from a hash of the person's id, dressed and equipped by `role` and `jobId||job`; `animateResident()` walks them by distance (`state.stride` assumes one cycle per .95 units and each villager rescales it to their own legs) and knows `Pushing`, `Hammering`, `Resting`, `Working` and `Idle`. `humanoidRig.arms` and `legs` are swing bones: at motion 0 they rest at exactly 0 and the stance lives on child bones. The new `village_animals.js` holds dogs, cats, hens, roosters, ducks, sheep, cattle, goats, pigs and horses plus their lives (`make`, `pose`, `livestock`, `settle`, `fowl`, `small`, `STRIDE`). Dog, livestock, horse and duck face +X and swing legs on `rotation.z` only; humans, hens, roosters and cats face +Z. Keep the makers' `r()` draw counts in `index.html`, or every village layout shifts. Budgets: a villager 2,600 triangles, dog, cat and fowl 1,400, sheep, goat and pig 1,800, cattle and horse 2,600. Details: `VILLAGE_FOLK_V486.md`.

Smooth sky, plain plot v485 (`smooth-sky-plain-plot-v485-20260925`): Yaan saw straight seams in the clouds and found the trees round his plot silly. The cloud shader (`world_sky.js`) now normalises the sky direction per pixel, so the dome's triangles no longer show as flat facets. Its noise uses a sine-free hash, a quintic blend and turned octaves, so phone GPUs draw no lattice lines. `visibleTrees` in `player_home_core.js` returns nothing, so the shelter and the built house stand on open ground; `TREES` stays so old saves still load. Timber comes from village trees.

Academy day and night v483 (`academy-day-night-v483-20260925`): the Academy screen and Home's Academy keep the game's clock (`daylight_core.js`) through `academy_daynight.js`. Each scene stacks a live night sky (`[data-dn-sky]`: stars, moon, shooting star), a moonlit night tree with its sky cut away, and the day painting on top, which fades with `--dn-sun`. Houses carry a lit-up night picture (`data-night-src`, swapped in only from dusk, when the scene gets `.dn-awake`), lamplight pools glow on the bark with `--dn-lamps`, and a soft-light wash follows `--dn-warm`. The 2D and 3D Academies share `lampFactorForHour` and `isNightHour` from `academy_daynight.js`; `daylight_core.js` is untouched. Night art lives in `assets/academy-night-20260925/` and is baked from the day art by `source/bake_night.py`, so it lines up pixel for pixel; rebake if a painting or a house changes. Checks: `node tests/test_academy_day_night_v483.cjs`, browser `node tests/run_academy_day_night_v483.cjs`. Details: `ACADEMY_DAY_NIGHT_V483.md`.
Alderwing seamless v482 (`alderwing-seamless-v482-20260925`) stops pop-in by distance, the way CDLOD terrain does. The detailed ground bends onto the distant land between `MORPH` 84–124 m (square distance from the eye), always inside the shown square, and its grass detail fades with it. Far trees and boulders end at their own 100–124 m (`END`, round) and ride the bend through a per-instance `natureDrop` (the lattice height less the ground's, refreshed by `redrop` when the land changes). Canopy crowns (`crownBroad`, `crownConifer`, lod 4) carry woods on to 250–300 m (`CROWNS`), built in 64 m cells from `horizon.sampleAt` on a fixed 6.4 m grid; the distant canopy rises by distance over `RISE` 250–320 m. The distant land's fine-to-coarse bend is distance-based too, so recentring draws nothing new; only changed data eases in, and the ground's bend eases with it (`groundFarPrev`). Shadows step 16 m in whole texels (`shadowCentre`) and fade out by 100 m (`SHADE`). `nature.flush()` runs once, after the frame's last pool change. The weather is now the game's own, made to look good (Yaan: it need not match the real sky): `burbzWeatherAt` in index.html follows the clock, mostly fair with about one passing shower an hour, and sends nothing off the phone. This replaces v464's Open-Meteo call. Its cover drives a cloud layer, grey light and haze in `world_sky.js`; rain always brings cover and falls in flight. Keep the seven module pins in step. See [../../ALDERWING_SEAMLESS_V482.md](../../ALDERWING_SEAMLESS_V482.md). Tests: `node --test tests/test_alderwing_seamless_v482.cjs`; browser proof `node tests/run_alderwing_seamless_v482.cjs`.

Glide on release v481 (`glide-release-v481-20260925`): Yaan asked that letting go of Flap stops the wingbeats and glides until the craft loses height naturally, and for one Flap button further right with no Dive button. Only Flap (the button, Space, the Academy slider up) beats the wings now; the stick, W and Auto no longer flap, and while Flap is held the stick forward still drives the beats ahead. Still wings trim to a glide in `academy_flight_core.js`: the path sits `GLIDE` (the best glide, atan 1/8) below the view at cruise and `SETTLE` lower per cruise speed lost, eased in over about a third of a second, so the craft holds cruise and sinks about 1 m per 8 flown, and a slow craft noses down to find its speed. The stall warning needs `CALM` (2 s) of still wings, so the drop out of a hover never lights Flap; a pull-up still stalls. The hover hold now works only below stall speed, so Flap at flying speed climbs on at about 13 m/s instead of braking into a hover. Take-off with Flap still rises straight up. The craft has one Flap button (`wc-climb`): landscape puts it above the health panel beside Attack, portrait above Attack. Shift still tucks the wings; on touch, look down to dive. Checks: `node tests/test_glide_release_v481.cjs tests/test_real_flight_v478.cjs tests/test_flight_controls_v371.cjs tests/test_geographic_world_v386.cjs`; browser evidence `tests/run_glide_release_v481.cjs` (replaces `run_real_flight_v478.cjs`) and `tests/run_craft_controls_v427.cjs`. See [../../GLIDE_RELEASE_V481.md](../../GLIDE_RELEASE_V481.md).
Merlin flight v479 (`merlin-flight-v479-20260925`): Yaan said Merlin looked silly in flight, shrank on Home, and taps meant for a pebble pressed buttons. Merlin's perch is now one size on every screen: `scan_home.css` no longer scales `#merlinPerchAssembly` on Home (the landscape Home still moves it left, unscaled). The play flight is rebuilt (`merlin_flight.js`): one painted side view from two ElevenLabs sheets (an 8-pose wing beat plus reach, grab, lift and landing flare; `assets/merlin-flight/merlin-flight-v3.webp`, 1312×912, rebuilt by `assets/merlin-flight/source/build_atlas.py`). He moves with steering physics, so every path is an arc; the swoop and the landing end on Hermite curves, so his talons meet the pebble exactly and he lands on the perched Merlin's own spot. He flies at 96 px beak to tail, the size he sits. The whole screen is the play sky: every tap drops a pebble where the finger lands, even on the dock or a button, and nothing underneath is pressed. Only the play bar (Care, Finish) stays live, just above the dock. He carries each pebble to a little pile under his perch; after four he flies home and play ends. 25 s with no tap also sends him home. The v1/v2 atlases are gone. Checks: `node --test tests/test_merlin_flight_v3_20260925.cjs`, browser `node tests/run_merlin_flight_v3.cjs`.
Real flight v478 (`realistic-flight-v478-20260925`): the craft and the Academy bird fly like real birds (`academy_flight_core.js`, shared through `geographic_world_core.js`). Gravity always pulls. Wings lift only with airspeed; below half cruise they stall. The view steers the path: look down to dive and gather speed, look up to climb and bleed it. Wingbeats are pulses that finish once begun, so a tap is one full stroke. Flap (Space, slider up) lifts and hovers; W flaps ahead to cruise; S airbrakes; A/D and the stick's sideways push bank into turns (no strafing); Dive (Shift, slider down) tucks the wings. Take-off gives one wingbeat (`launch`); a mid-air resume with no wing state picks up in a glide. Interruptions release controls but keep momentum in flight. The craft's buttons are Flap and Dive (combat styles them by those labels); Flap glows while stalling; the left cockpit dial reads airspeed with red stall ticks. The camera adds wingbeat bob, stall shudder and nose drop, and widens up to 9° in a fast dive; reduced motion keeps it still. Checks: `node tests/test_real_flight_v478.cjs tests/test_flight_controls_v371.cjs tests/test_geographic_world_v386.cjs`; browser evidence `tests/run_real_flight_v478.cjs`. See [../../REAL_FLIGHT_V478.md](../../REAL_FLIGHT_V478.md).
Academy manga v475 (`academy-manga-v475-20260925`): the Academy 2D screen is redrawn in the bird-card manga style. New tree (`assets/academy-manga-20260925/tree.webp`, plus `tree-wide.webp` for boxes 600 px and wider through a container query), four new boughs that reach in from the frame edges, and one manga picture per building in `ACADEMY_BUILDING_ASSETS`. The house designs match Home's living tree. `academy_alive_core.js` ANCHORS are fractions of these square sprites; retune them if a house is repainted. Details: `ACADEMY_MANGA_V475.md`.

Home dock and Back v474 (`home-dock-back-v474-20260925`): Home now leads the bottom dock on every screen, with a new painted cottage icon (`assets/ui/burbz-icon-set/home.webp`, cached in both worker icon lists and the updater). A Back button (`#headerBackBtn`) stands top-left in the header and calls `handleBurbzBackPress()`, so it does exactly what the phone's back does: close the top sheet, then walk the screen trail. The logo still goes Home too. Keep Home first in the dock. Checks: `node --test tests/test_separate_home_20260924.cjs tests/test_alderwing_steady_v472.cjs`.
Living Academy tree v473 (`academy-living-tree-v473-20260925`): Home's Academy box shows a new painted tree with only the houses the player has built. Each house stands where the player placed it on the Academy screen (`treeSpot` in `scan_home.js` maps the saved x/y onto the painting). Art lives in `assets/academy-living-tree-20260925/`: `tree.webp` (3:4), `tree-wide.webp` (16:9, used by a container query when the box is wide) and one cut-out per house id. Four soft-masked boughs sway on their own pivots, houses rock, leaves fall, light motes drift, two birds fly past and the Kitchen chimney smokes. Ready-to-build houses show as one "🔨 N ready" chip. Reduced motion stops it all. If a new house id is added, add its cut-out and its id to `TREE_HOUSES`. Checks: `node tests/test_separate_home_20260924.cjs`, `tests/test_scan_home_v378.cjs`, `tests/test_scan_home_v384.cjs`.
Alderwing steady v472 (`alderwing-steady-v472-20260925`) keeps every tree whole. A tree swaps its detailed form for a matched simple one at its own 28–36 m, and far trees and boulders end one by one 12–28 m inside the shown ground (`world_nature.js`); only shrubs and ground plants still shrink. The detailed square grows side by side and eases at 80 m/s, shrinking first (`showTarget` and `easeSquare` in `village_world_core.js`). Its edge bends height, colour and normal onto the distant land's own lattice (`horizon.lattice`), and changed chunks rebuild in place. The distant land eases for 1.2 s after every rebuild, with normals from the whole grid (`world_horizon.js`). Woods are dense again, unmapped land keeps much of Alderwing's woodland, and open country has copses (`world_nature_core.js`); far woods wear a painted treetop pattern. The cockpit is a leather coaming over a walnut panel with three gauges set in its face (`flight_craft.js`). Keep whole-or-nothing trees, the shrink-first ease, the chunk-aligned cut, the lattice-matched edge and the seven exact module pins. See [../../ALDERWING_STEADY_V472.md](../../ALDERWING_STEADY_V472.md).

Alderwing nature v470 (`alderwing-nature-v470-20260925`) colours the ground from real OpenMapTiles cover, elevation, slope, latitude and season (`world_nature_core.js`), draws real elevation out to about 4.5 km behind a long haze (`world_horizon.js`), pools eighteen plant and stone kinds per kind per 96 m cell with near/far tree forms (`world_nature.js`), turns every mapped stream into flowing water with waterfalls over steep real drops (`world_water_core.js`, `world_water.js`), and gives lakes and the sea waves, sky reflection and glints (`shore_water.js`). The craft is a new feathered ornithopter; aboard, only a dashboard shows and a pilot aloft may look down to −1.5 pitch. Keep the shared per-cell pools, per-kind fade ranges inside their registration distances, the no-crack ground morph at the detailed square's edge, the fog fallback when distant tiles are missing, and all eleven exact module pins in consumers, three worker lists and the updater. See [../../ALDERWING_NATURE_V470.md](../../ALDERWING_NATURE_V470.md).

Paced tutorial v469 (`paced-tutorial-v469-20260924`): after Alderwing the opening is five short lessons (Birdhouse, discover a bird, meet Merlin, the Kitchen, explore), 12 bubbles instead of 22. After each lesson Merlin rests for two minutes (`MERLIN_LESSON_REST_MS`, localStorage `burbzTutorialRest:*`): no chapter auto-starts and nothing hands off across screens. A lesson's own chapters still flow on (`MERLIN_LESSON_GROUPS`). The Home goal ends the rest and starts the next lesson at once. Meet Merlin is its own `companion` chapter; the Play step and `academy_tour` are gone. The play flight clears the care menu on takeoff, Care lands Merlin first, Play waits during a lesson, and no lesson starts mid-flight. See PACED_TUTORIAL_V469.md. Tests: `node tests/test_paced_tutorial_v469.cjs`, browser `node tests/run_paced_tutorial_v469.cjs`.

Merlin opening v467 (`merlin-opening-v467-20260924`): Yaan asked for a shorter, prettier opening. The story is now four beats plus the hub, one painting each: `lesson-0`, `alderwing-story-v420-4`, `-5`, `-7` and `alderwing-hub-v420`. Saves paused on a retired beat resume at the beat that tells the same part (the `retired` map in `merlinTutorialResumePosition`). The painting fills the screen; on tall phones it sits above Merlin's card and fades into a blurred wash of itself. Each scene's `focus` keeps its subject in frame. Keep both story-scene pins in index.html and all three worker lists. Tests: `node tests/test_merlin_story_scenes_20260923.cjs`, `node --test tests/test_merlin_story_art_20260924.cjs tests/test_motivated_opening_20260914.cjs`.

Live weather v464 (`calm-weather-v464-20260924`): Yaan chose real weather. `burbzWeatherAt` in index.html asks Open-Meteo for current rain and wind with the position rounded to 0.1° (about 10 km), no credentials or referrer, at most every 15 minutes per cell. Rain drives the rain bed and visible streaks around the player in `exploration.js`; wind lifts the breeze. Failure keeps it dry with a light breeze. Credit lives in audio-credits.html#weather (CC BY 4.0). No game rule may depend on weather.

Calm camp v463 (`calm-camp-v463-20260924`) builds on v462. A lit campfire adds 40 m to its camp's protected circle (`empireTerritoryLight`). Unlit camps show an in-world "Build campfire · 5 timber" action within 11 m; it calls the same `buildCampFire` transaction. Any lit campfire unlocks the 🪑 tripod stool in `.fp-tools`: sitting drops the eye to 0.86 m via session-only `s.seated`, and any movement, flight, swimming or a room stands the player up. Fires now carry a halo, a warm light pool, smoke and mist (`fireFX`), and one shared point light follows the nearest lit fire. A light wind bed always plays outdoors. The rain bed ships but stays silent: no weather source exists, and a live-weather lookup would send the player's location to a third party, which needs the owner's decision.
Home Academy dock v463 (`home-academy-dock-v463-20260924`) stacks Crafting, Kitchen, Training and Hospital on the left of Home and fills the right with a condensed Academy box. Built rooms open straight away; unbuilt ones lead to their real build card. The dock keeps only Map, Battle, Birds, Stores and Ranks in one short row. The Burbz logo is the Home button. Lesson-12 targets the Home Academy heading; lesson-25 targets the logo. Keep the three Home pins in index.html and all three worker lists. Test: `node --test tests/test_separate_home_20260924.cjs`.

Calm sounds v462 (`calm-asmr-v462-20260924`) replaces the clanking footsteps with soft, levelled ElevenLabs takes and adds a calm camp layer. Calm is the default mood. `wilderness_combat.js` sets tense when a living foe is within 30 m or the hero is down; tense swaps outdoor steps to gritty gravel at the same volume and hushes the fire. `exploration.js` fades the campfire loop in from 14 m of a lit camp fire. Chop, eat and dust play through `window.BurbzCalmAudio`. Keep all seven changed module pins, the three worker lists and the updater in step. Tests: `node tests/test_calm_asmr_audio_20260924.cjs`. Provenance: `assets/audio/footsteps/manifest.json` and `assets/audio/camp/manifest.json`.
Bird patch map v466 (`bird-patch-map-v466-20260924`) draws the home patch of each bird the player hears or photographs on the real map, pulses it while the bird sings, and teaches that a bird met again in its patch is the same bird. Meetings stay on the phone. Keep the exact v466 pins in index.html, all three worker lists and the updater. See [BIRD_PATCH_MAP_V466.md](BIRD_PATCH_MAP_V466.md).

Gold trail/Raven v454 (`gold-trail-raven-v454-20260923`) replaces the large red destination markers with subtle 24px pins and a rounded wide gold trail. Preview and saved active quests display exact banked building/character/bird encounter icons; retain persistence, cleanup and dashed unverified guidance. Raven/Common Raven card paintings use the supplied lossless WebP with independent unchanged warrior/transparent sprites; keep its exact art-module pin and asset in all worker lists and the updater. See [../../GOLD_TRAIL_RAVEN_V454.md](../../GOLD_TRAIL_RAVEN_V454.md).

House entry/landing v438 (`house-entry-landing-v438-20260922`) routes the house quest through Home’s native Enter Alderwing button. Preserve independent craft/quest saves, bounded full-hull descent checks, atomic landing rollback, visible four-second blocked-landing feedback, and all five exact consumer/worker pins. See [../../HOUSE_ENTRY_LANDING_V438.md](../../HOUSE_ENTRY_LANDING_V438.md) for verification and publication status.

GPS shelter/swimming v439 candidate follows Yaan's 22 September clarification: a built house replaces/deletes the temporary shelter and stays at its chosen anchor. Temporary door entry uses fresh real GPS, stays on foot, and reserves a small clear doorstep; explicit house construction stops GPS relocation. Preserve the real-map house marker, flat ground/water landing, swimming/reboarding/shore transitions and atomic craft/journey writes. This supersedes the v426 fallback-location and v427 occupied-door-entry restrictions only where described in [GPS_SHELTER_SWIMMING_V439.md](GPS_SHELTER_SWIMMING_V439.md). Deliberately parked craft and paid settlement ledgers remain authoritative. Candidate is not yet published.

Red/gold pocket v437 keeps the destination route and both pins under saved active-quest ownership after Begin, close, navigation, style reload and offline restart. The second map tap starts planning automatically; weighted routing prefers public footpaths over roads/pavements. Destination walks gather every available pickup within the canonical 185 m circle on fresh accurate GPS, independent of marker caps or document visibility; retain once-only receipts, rollback, no inferred movement and the OS-location-pause disclosure. See [../../RED_GOLD_POCKET_V437.md](../../RED_GOLD_POCKET_V437.md).

Destination anywhere v436 supersedes the 45 m endpoint rejection and strict map-only Main Quest rule. Always bank valid selected endpoints: public walking roads join paths; explicit dashed guidance bridges missing topology and provider outages. v2 guidance parts retain separately validated v1 mapped evidence and never certify gaps. Preserve v1 saves, cancellation, honor-based Finish my walk, atomic rewards and exact changed JS worker pins. See [../../DESTINATION_ANYWHERE_V436.md](../../DESTINATION_ANYWHERE_V436.md).

Destination touch v435 preserves immediate red geographic selection markers, planner ownership of pan/GPS until close, bounded official OSM-node fallback and canonical access/topology validation. Begin still requires a validated banked route. Keep exact three-module consuming/worker pins. See [../../DESTINATION_TOUCH_V435.md](../../DESTINATION_TOUCH_V435.md).

Destination walks/cleanup v434 adds destination Main Quests while preserving old Side Quests, v430 map overview and v429 tutorial/inversion. Preserve predeparture banked content, separate walk/story completion, canonical atomic receipts/three distinct same-species meetings, native guest-room readiness and cancellation ownership, and default route/access validation. New/changed modules use exact v434 pins in every consumer and all three worker lists. See [../../DESTINATION_CLEANUP_V434.md](../../DESTINATION_CLEANUP_V434.md).

Hands-on opening v429 (`hands-on-world-tutorial-v429-20260920`) inserts current-step swipe/look, forwards/backwards and sideways practice after the landscape/one-to-one Earth introduction. Preserve fresh pointer/key receipts, real camera/distance thresholds, failed-save retry and existing completed-save non-replay. `settings.invertVerticalLook` uses canonical durable settings; all direct and held camera inputs share its sign. The existing Settings modal is borrowed/restored by world/home sessions with input pause, focus and teardown cleanup. Six changed modules must keep exact consumer and three-worker-list pins. See [HANDS_ON_WORLD_TUTORIAL_V429.md](HANDS_ON_WORLD_TUTORIAL_V429.md).

Walk/shelter controls v427 (`walk-shelter-controls-v427-20260920`) keeps nearby walks in a compact, nonmodal map panel with its own scroll area. Preserve the existing route selection/details/Begin flow, actual projected marker fitting around panel/control occluders, touch/keyboard map panning and cleanup on navigation. The first shelter instruction locks home movement/look/actions until its durable Next transition; Merlin stays operable, while completed/other-home sessions stay free. Starter craft berth search stays left of the fixed south-facing shelter doorway on verified dry ground. Only the unoccupied starter craft blocking an active tutorial may be relocated; preserve all travelled/occupied parking. Enter craft is visible only within the same 1.8m/height gate used by actual boarding, with readable 44px controls. Nine consuming/three-list pins agree. See [WALK_SHELTER_CONTROLS_V427.md](WALK_SHELTER_CONTROLS_V427.md).

Shelter tour v426 (`shelter-tour-v426-20260919`) keeps the active saved intro out of the house picker when GPS is absent, pending, or overlaps a settlement. Preserve checked temporary tier-zero placement, existing anchors, profile/navigation/cancellation guards and ordinary later house selection. See [../../SHELTER_TOUR_V426.md](../../SHELTER_TOUR_V426.md).
...
Photo shape acceptance v452 permits identifiable silhouettes through the unchanged confidence, species-margin, original-pixel and two-view verification gates. Preserve the v425 API contract, existing fresh retry IDs, ledger and sound/save behavior. See `PHOTO_SHAPE_V452.md` at repository root.
...
Home v423 (`home-b-stable-v423-20260917`) supersedes progressive panel hiding: the user chose screenshot B as the persistent composition for every save. Always retain Empire, scanning, Goal/build/Equipment, Crafting/Kitchen and Training/Hospital in their existing B positions. Locked rooms show truthful unavailable states; all native gameplay gates remain authoritative. Do not remove or expand panels based on unlock count. This fixes the sparse Goal/Kitchen-only view missed by populated-save checks. Deep Rest removal is separate and not included.

Home v422 (`home-b-charcoal-v422-20260917`) preserves the user-selected historical B arrangement with charcoal panels and muted gold, moves the inactive Home opening action into Current Goal, and uses two larger navigation rows in portrait. Landscape retains its existing side rail. Current Goal follows canonical lesson/opening/quest state and re-resolves native actions without granting progress. Preserve modal tutorial targets, keyboard focus, small-phone fit and all three Home worker pins. Photo recognition remains explicitly held and unchanged. Details: [HOME_B_CHARCOAL_V422_RELEASE.md](../../HOME_B_CHARCOAL_V422_RELEASE.md).

UI-only v421 (`home-squares-v421-20260917`) adds square Empire columns, Current goal/eligible build browser/Equipment, hidden-until-scanning results drawer, Crafting-to-Forge and the approved compact dark Merlin guidance. Kitchen stays in its prior panel. Preserve native eligibility rechecks without spending, canonical quest ownership, per-panel scrolling, 44px controls and all three Home module worker pins. Recognition repairs are explicitly held and are not part of v421; its worker/adapter/prompt/thresholds remain v420 bytes. Details: [HOME_SQUARES_V421_RELEASE.md](../../HOME_SQUARES_V421_RELEASE.md).

Released v420 (`alderwing-intro-desk-v420-20260917`) (PR357, runtime 4ce3e473) adds saved-photo selection, the user’s new Merlin opening, Home’s three-column Your Empire, one-line companion names and compact provider attribution. The opening replaces the older greeting/five-bubble story limit at the user’s explicit request; preserve stable lesson IDs, actual door/chair receipts, saved `tutorialFlow.alderwingIntroPhase`, phone orientation gates, desktop continuation, failed-save rollback and the later Birdhouse/Kitchen transactions. Only one Merlin welcome may appear. Existing completed players must not replay. `alderwing_intro.js` is in all three worker lists and the updater. Preserve the current immediate image-recognition path (no new background-photo storage), shared £5 monthly cap and sound recognition. Full credits live in Settings; OpenMapTiles requires map-corner attribution, so retain the native expandable credits. Details and verification: [ALDERWING_INTRO_DESK_V420_RELEASE.md](../../ALDERWING_INTRO_DESK_V420_RELEASE.md). The independent compact prerequisite-popup handoff is integrated; retain its collapsed secondary routes, small Home Goal resume, global banner removal and unchanged resolver/transaction authority.

Aircraft/flocks/countryside v418 (`alderwing-flight-shores-v418-20260915`) uses one canonical `flightCraft` record committed with `worldJourney`. Preserve actual boarding, parked coordinates, interrupted-flight recovery, expected-receipt/profile/home-revision guards and wounded manual return; Auto requires occupancy and clears on interruption. Zombie species use real Forge/discovery progression and persisted camp rosters, bounded 3D flight and canonical light/combat/reward authority. Mapped open land requires explicit grassland/heath subclasses with woodland precedence; shoreline coverage changes existing terrain colour, not geometry/collision. Mixed-chunk textures must dispose on retirement. Keep all14 changed JS/CSS consumer/three-worker-list pins exact. Acceptance and limits are in [ALDERWING_FLIGHT_SHORES_V418_RELEASE.md](../../ALDERWING_FLIGHT_SHORES_V418_RELEASE.md); Rowan’s story remains paused.

Released v417 integrates the finished card/world/common-guidance handoffs on v416 (PR353). Public byte, installed/offline, cards, common guidance and real-provider world verification passed; machine-readable proof is reports/BURBZ_V417_PUBLIC_VERIFICATION.json at repository root. Current result and acceptance boundaries are in ALDERWING_FOLLOWUPS_V417_RELEASE.md at repository root; earlier intake-status paragraphs below are historical. Equipped or spare owned items remain usable independently of the Stores-screen unlock. Do not add an inventory-screen gate to the canonical equipment transaction: that creates a circular equip quest prerequisite. Missing-item guidance still uses real wearer, anvil and recipe requirements.
...
Prepared opening slice (2026-09-14, not released): Birdhouse → actual discovery/card or honest Merlin continuation → companion interaction → later real Kitchen shortage → once-only useful supply return → Kitchen meal. Stable quest/lesson IDs and existing rewards persist; no forced inventory loss or existing hunger rewrite. Preserve the new supply receipt, failed-save rollback and independent owner's trailer/Settings/Home/navigation hunks. See [MOTIVATED_OPENING_20260914.md](MOTIVATED_OPENING_20260914.md) for exact integration regions, economy compatibility, tests and remaining native/browser/cache acceptance. Global release/module pins are deliberately left to the sole owner.
...
Village houses and terrain v408 (`houses-terrain-v408-20260914`) projects the saved personal house into village/town overviews using the shared home model. Preserve personal-versus-civic economy, anchor/upgrade/room cache invalidation, hiding before walking collision/batching, and restoration on exit. Streamed trees require known gentle root footprints; rocks keep full footprints clear of paths/water; cascades use actual loaded downhill watercourses and the existing renderer/lifecycle. Keep all consuming/three-list pins exact. Evidence and limitations: [HOUSES_TERRAIN_V408.md](HOUSES_TERRAIN_V408.md).

Photo journal v407b (`photo-journal-v407b-20260914`) fixes an existing first-camera-photo save ordering bug exposed by the real v407 installed-app check. Create/update the canonical discovery before calling `rememberPlayerBirdPhoto`; the helper must see the current record before writing its IndexedDB photo and `photoAt` flag. Preserve the first stored photo on later sightings, existing one-time rewards and unchanged sound/model gates. The Scan results & settings drawer is intentionally modal; repeat-camera tests must use its native Close or Choose a bird photo control.

Photo recognition v407 (`photo-recognition-v407-20260914`) upgrades to pinned BioCLIP 2.5 visual weights and matching taxonomy, retains the tested bounded two-view consensus and separate Birder worker, and adds explicit full-photo identification through the existing cancellation/save funnel. Do not reintroduce the rejected all-square internal preprocessing experiment or weaken thresholds to pass photos. Keep models-v407 separate from the old models rollback bundle; loaded source/model/bundle/manifest hashes and fourteen real photo plus unchanged BirdNET proofs are mandatory. See [PHOTO_RECOGNITION_V407.md](PHOTO_RECOGNITION_V407.md) for sourced app research, evidence and limits.

Wayside room title v406b (`wayside-room-title-v406b-20260914`) corrects a publicly observed misleading heading after room return. Restore the current retained location name from the existing navigation frame, with the original fallback only outside prepared places. Preserve the same canvas, coordinates, room lifecycle and module-specific cache pins. The release test waits for the lazy module and asserts the actual returned title. See [WILDERNESS_DISCOVERIES_V406.md](WILDERNESS_DISCOVERIES_V406.md).

Wilderness discoveries v406 (`wilderness-discoveries-v406-20260914`) adds explicitly requested, independently saved wayside shelters/keepers and lore in the existing world. Preserve fixed geographic identity, bounded prepared content, loaded-ground/settlement exclusion, shared scene and one-time atomic rewards. Wilderness encounters do not invent owned settlement economy or real GPS authority. Translate matrices before computing culling centres, and reattach controllers when retired content reloads. See [WILDERNESS_DISCOVERIES_V406.md](WILDERNESS_DISCOVERIES_V406.md).

Building opening v405 (`building-opening-v405-20260914`) keeps unstaffed timer-ready buildings inactive until opened at the reachable real scaffold. Only the actual canonical manager opens remotely. Preserve per-project/order/Hall identity, released crews, original rewards and atomic level/plot/save rollback. Retain the existing renderer and player; stage model/batch/collision changes, cancel on departure or UI/world teardown, and refresh paid doors. All constituent civic seats and legacy bulk orders keep their own authority. See [BUILDING_OPENING_V405.md](BUILDING_OPENING_V405.md) for validation and limitations.
...
GPS quest buildings v402 (`quest-buildings-v402-20260913`) exposes the actual bounded settlement meshes at canonical mapped/quest points. Preserve real GPS/certification authority, fresh 45 m room gates, original actions, full-footprint DEM foundations, conservative loading and tile-specific terrain cache invalidation. Transparent touch targets must retain fallback wayfinding, labels and visited state. Linear model colours need display conversion in the raw MapLibre shader. Exact scope, before/after timings and the existing real-map performance limitation are in [QUEST_BUILDINGS_V402.md](QUEST_BUILDINGS_V402.md).

Distant sky v401 (`distant-sky-v401-20260913`) removes the finite dome rim and settlement-relative celestial objects in retained first-person travel. Preserve camera-translation-free far-depth projection, actual terrain occlusion, the existing local-clock daily path, bounded shared resources, original scene restoration and exact module/three-list/updater pins. Evidence and honest laptop-only frame-time limits: [DISTANT_SKY_V401.md](DISTANT_SKY_V401.md). Physical GPS quest buildings, photo accuracy and commissioning/assistance remain separate unfinished work.

Unified Alderwing v400 (`unified-alderwing-v400-20260913`) joins actual canonical destination settlements and the shelter door through the retained world renderer. Preserve saved settlement identity, native rooms/quests, real metre/DEM terrain, conservative visibility, and the Home-first Merlin shelter/desk sequence. Building a permanent house requires loaded clear current-world ground and an atomic 25-timber confirmation. See [UNIFIED_ALDERWING_V400.md](UNIFIED_ALDERWING_V400.md). Whole-world celestial motion, physical quest buildings, on-site commissioning and camera accuracy remain separate unfinished work.

Home follow-up v399 (`home-followup-v399-20260913`) makes the painting a full-width responsive header, removes only the literal Field desk heading, and restores Merlin's shared screen position and care menu. A generated transparent warrior replaces both Equipment silhouettes; actual shared slots/ownership stay authoritative. The current quest reserves room for its title and instruction, including short landscape. Preserve the compact six independently scrolling trays, scanner gestures and all three asset/cache lists. See [HOME_FOLLOWUP_V399.md](HOME_FOLLOWUP_V399.md). Canonical shared-world arrivals/shelter, global sky motion, camera repair and builder/wayside work remain separate unpublished follow-ups.

Home equipment v398 (`home-equipment-v398-20260913`) moves the Home gear tray behind one Equipment silhouette control. Its compact figure/slots popup reuses the shared transactional sheet and actual first-person effects. Village assignment portraits read the canonical Project Manager/Lord Mayor post, with labelled colour states; never infer new commissioning authority from a title. Preserve full art, original-size Merlin, scanners and six trays. The on-site building rule and unified-world/shelter work remain unfinished and separate. See [HOME_EQUIPMENT_V398.md](HOME_EQUIPMENT_V398.md).

Desk composition v397 (`desk-composition-v397-20260913`) restores the complete generated painting and the original-size real Merlin companion in reserved Home space, with equipment in a separate shared-inventory tray. The Your villages panel associates canonical owned village names with actual construction/completion and available projects. Keep the small-screen dock’s original destinations operable, all six independent trays, scanner gestures/cancellation and exact module/three-list pins. The upcoming on-site building commissioning rule and unified world/shelter work remain unfinished, not included. See [DESK_COMPOSITION_V397.md](DESK_COMPOSITION_V397.md).

Desk panels v396 (`desk-panels-v396-20260913`) adds compact independently scrolling Stores, Kitchen, Training, Hospital, unchecked building and affordable project panels, plus the actual player loadout and shared Satchel sheet. Preserve scanner gestures/cancellation, gated actions, immutable dashboard reads and durable shared equipment/crafting. Building notices remain saved beyond the four-card toast limit. Camera/Sound remain the largest controls and the desk fits short portrait/landscape viewports. The tested shadow-crow/archery handoff joins this release; ongoing world/tutorial/photo/builder work does not. See [DESK_PANELS_V396.md](DESK_PANELS_V396.md).

Living desk v395 (`living-desk-v395-20260913`) adds original desk art, compact live Home summaries/build suggestions and independently centred solo battle cards. Preserve existing scan handlers, economy authority, same-origin artwork and all three worker lists. The shelter/tutorial/world revisions remain separate unfinished work. See [LIVING_DESK_V395.md](LIVING_DESK_V395.md).

Walking archery/shadow crows (`wilderness-birds-v395-20260913`, integrated in desk-panels-v396-20260913): preserve the existing terrain/arrival renderer and HUD/minimap. Bounded articulated crows must pursue and resolve real, telegraphed body-targeted Peck damage only in known wilderness. Birds use existing player walking footprints beneath foliage, while shots keep 3D canopy collision; trunks, structures, water and safe-area checks still apply. Fresh defaults equip the existing one-copy Ember Wisp Fireball; never replay that grant over old/cloud inventory. Wayfarer Bow uses shared equipment and Reed Arrows in `inventory.items`; release must atomically spend ammo plus readiness before projectile launch. Satchel Fieldcraft and the Forge share recipes, real timers, queue, durable collect/cancel rollback and diary; arrows do not count as crafted gear or bird equipment. New module URLs belong in all three worker lists and the guarded updater. The release owner promotes the global build/cache. See [WILDERNESS_ARCHERY_V395.md](WILDERNESS_ARCHERY_V395.md).

Landscape atlas v394b (`landscape-atlas-v394b-20260913`) puts first-person controls at the edges with a compact north-up minimap and on-demand current-realm atlas. Preserve native movement/look/cast pointer ownership, 44px controls, short-landscape safe areas, deliberate quest dismissal, and the single walking frame. The minimap reads existing terrain/footprints at <=4 Hz without another WebGL context. The realm panel borrows/restores the actual Empire atlas DOM, controls and camera; its avatar marker never becomes GPS or quest authority. Restore the lease on close, failure, navigation and teardown. All synchronous local scripts in index.html must use their exact consuming URLs in all three worker lists and be required before activation; otherwise a fresh offline launch can miss bird catalogues and fail before init. Show Quests opens the existing board with plain loading/error/Retry states; preserve complete OSM evidence, provider fallback including HTTP 200 remarks, response-body deadlines, stale-location guards and existing rewards. Quest brief positioning must distinguish a side dock from a bottom dock. See [LANDSCAPE_ATLAS_V394.md](LANDSCAPE_ATLAS_V394.md).

Photo accuracy v393 is prepared, not published. It now uses the free local `photo_local.py` worker (BioCLIP 2 worldwide, Birder cross-check and a separate bird detector), with no Google calls or paid fallback. The client requires `photo-local-v393`, accepted/verified and score >=0.90; original-pixel, two-view and cross-model gates must remain fail-closed. Keep the worker isolated from BirdNET, bounded CPU/queue, pinned model checksums and transactional worker+adapter/sound HTTP proof. Retain the 24 MP framing, extended zoom and large honest progress/cancellation UI. See [LOCAL_PHOTO_V393.md](LOCAL_PHOTO_V393.md) for scope, exact rules, measured evidence and deployment. Earlier Gemini quota notes are superseded; no billing is needed for this implementation.

Photo recovery v392 (`photo-recovery-v392-20260911`) keeps preparation, analysis and outcome beside the camera photo, with an explicit same-crop retry and a 45-second deadline across location, upload and response-body waits. Preserve the v350 acceptance policy and existing discovery/save funnel; late or failed responses cannot award birds. Landscape crop stages remain square with Identify visible. Only index.html and the worker change at runtime; existing module pins remain intact. Regression coverage is in `tests/test_photo_client_gate_20260905.cjs`.

Continuous countryside v391 (`continuous-world-v391-20260910`) retains the actual village/town scene across outward and return travel, with original materials and fixed-density trees on real geographic metre/DEM terrain. Preserve player/camera/input identity, immutable chunks, full-horizon prefetch, explicit unknown ground, original paid rooms/harvest, and the separation from real GPS quests. Finished equipment/combat and the canonical Merlin owner fix join this coordinated release; no 3D Merlin assets are included. All changed loader, worker and updater pins must agree. See [WORLD_CONTINUITY_V391.md](WORLD_CONTINUITY_V391.md).

First-person exploration combat (integrated in `continuous-world-v391-20260910`): the user's route decision is resolved — zombies appear only in confirmed wilderness between villages/towns; settlements, homes, interiors and unknown coverage remain nonhostile. Reuse the compact HUD and shared `inventory.equipment['@player']` ownership ledger, actual bird weapon/scroll definitions, Forge stats and atomic potion/refill/save transactions. `wilderness_combat_core.js` shares battle damage/RNG/heal/rider rules; the runtime borrows the existing renderer/frame. Preserve `safeAt` versus confirmed `refugeAt` (loading must never heal), bounded actors/projectiles/receipts, cancelled touch/focus behavior, and exclusion of @player from bird quests. v391 retained-scene compatibility reads canonical settlement records through the character adapter, with optional owner-provided world safety overrides. See [FIRST_PERSON_COMBAT_V1.md](FIRST_PERSON_COMBAT_V1.md) and [FIRST_PERSON_EQUIPMENT_V1.md](FIRST_PERSON_EQUIPMENT_V1.md). The original combat-location question is answered; do not ask it again. Publication belongs to the existing release owner.

Merlin 2D v390 (`merlin-flight-v390-20260910`) integrates the reviewed 164-pose sprite work from c14383c. Renderer, metadata and v2 pixels must ship together; preserve the clip rectangles/talon anchors, heading during pickup recovery, one-loop lifecycle and existing Play/Rest economy. Both script pins, the explicit atlas URL, all three worker lists and the updater agree; unchanged CSS stays on v1. No 3D model is included. Integration details: [MERLIN_RELEASE_V390.md](MERLIN_RELEASE_V390.md).

Market tabs v389 (`market-tabs-v389-20260910`) separates Buy (materials and larder supplies) from Sell (all owned Stores goods, including every collected Forge craft). Preserve the existing rarity prices, Trader discount, building gate, equipped-gear ownership and strict quantity guards. Inventory, purse and trade/quest progress commit atomically through durable save with in-place rollback and deferred notices. Tab state is UI-only; preserve focus, scroll and original bags. Released through PR 322, merge e713eb87a852253504e8beef92744a7514b2e6de, with public runtime and actual worker verified. Details: [MARKET_TABS_V389.md](MARKET_TABS_V389.md).

Landscape v388 (`landscape-v388-20260910`) is released through PR 321, merge 6af560cf5926ab9255ac04e4f48b84993a5d11de. At landscape widths of at least 640px, the normal game dock becomes a right-hand rail. Village/town overviews use the left half of the viewport below the shared header, with independently scrolling management on the right. The adapter moves existing stage nodes and restores their portrait positions; preserve DOM/listener/renderer identity, focus, full-screen walking, projected house-screen composition and existing saves. Its public runtime and actual worker were verified. Details: [LANDSCAPE_V388.md](LANDSCAPE_V388.md).

The released v387 Birdex projection now groups both catalogue and saved rows by the catalogue's complete scientific identity, with the existing name resolver as fallback. Discovered counts use the same key. Never deduplicate the owned flock or rewrite/delete saved discovery aliases to fix duplicate cards. Raw scientific taxa preserve distinctions such as Redpoll/Arctic Redpoll and subspecies; do not replace this with accepted-species taxonomy lumping. See [BIRDEX_IDENTITY_V387.md](BIRDEX_IDENTITY_V387.md).

Home and countryside v387 (`home-countryside-v387-20260910`) separates geographic first-person travel from the real quest map, adds passive avatar location, village-edge travel and immediate flight, and supplies six ElevenLabs ground/wood/stone footsteps through existing audio controls. Home uses charcoal/gold, a compact Discover container with unchanged buttons, and villages directly beneath. Coin shortages offer three explicit navigation choices in a dismissible native modal, without dispatching or spending. Preserve real metres/DEM, GPS and save authority, bounded scenery, full-quantity purchase guards, and the matching three-list/module/updater pins. Details and verification: [HOME_COUNTRYSIDE_V387.md](HOME_COUNTRYSIDE_V387.md). Released in approved merge c99a19ccc8af456bba0262b19b01eabb71e7e5a7; further changes require their own release authorization.

Connected world v386 (`connected-world-v386-20260910`) joins the actual house and saved homestead to continuous geographic walking and bird-style flight. Home v3 preserves rooms, possessions and receipts, anchors once to an accepted real fix (or explicit map choice), and relocates atomically with revision guards. Geographic avatar motion uses real metres and canonical village/town coordinates but never supplies GPS fixes or physical-quest rewards. Keep decoded terrain coverage explicit, source/model/cache bounds, native clocks, forced navigation/save replacement disposal, desk DOM identity and exact loader/three-list/updater pins. Details and recovery: [CONNECTED_WORLD_V386.md](CONNECTED_WORLD_V386.md), [PLAYER_HOME_V386.md](PLAYER_HOME_V386.md).

Homestead v385 (`homestead-v385-20260910`) doubles the personal clearing's ground, walking and decorating areas, adds 32 distinct decoration designs and three paid, walkable rooms, and migrates home saves to v2 without losing existing possessions. Preserve the same live command-desk DOM, clear passages, per-space/total placement caps, atomic timber/build/find transactions and finite UTC tree receipts. Village and Town Walk share native contextual controls, actual player supplies, 12 new three-stage field stories and generated opaque cutout objects; existing fifty requests, thirty scrolls and claim identities stay intact. Actual tree geometry falls only after durable payment, leaves collision-free stumps and regrows from saved availability; never replace an indoor collision world during an outdoor update. Town walking uses its heart ward's saved identity and exactly one borrowed render per frame. Changed/new modules, discovery CSS and atlas must agree across loaders, all three worker lists and the updater. Feature, verification and recovery contracts: [HOMESTEAD_V385.md](HOMESTEAD_V385.md), [VILLAGE_FIELDWORK_V385.md](VILLAGE_FIELDWORK_V385.md).

Enchanted study v384 (`enchanted-study-v384-20260910`) gives Home a new magical medieval study painting, actual player/walk/care/reward/village information and all unlocked game routes. The house monitor projects the same `#app` DOM through a depth-correct transparent aperture; Stand and Sit move the real camera. Preserve node/listener/scroll identity, inert walking projection, explicit scanner gestures, gate rechecks, pause/resize/reduced-motion/context-loss restoration and existing save/economy authority. Changed home/scan modules plus the new image belong in all three worker lists and the guarded updater; unchanged home core retains its v380 pin. Details: [ENCHANTED_STUDY_V384.md](ENCHANTED_STUDY_V384.md).

Side chest repair v383 (`side-chest-v383-20260910`) normalizes generated and saved side-chest bundles with the existing quest-core normalizer before strict validation. Legacy two-unit rabbit/mouse and pigeon/vole food stays intact; deterministic additions satisfy the existing food minimum. Repairs persist only with the atomic reward/receipt transaction, so failed saves restore the original chest and retries/reloads cannot duplicate rewards. IDs, coordinates, fresh-GPS/185 m gates, detour switching and pocket completion remain unchanged. Only the app shell/cache release advances; all external module URL pins remain at their existing releases. See [SIDE_CHEST_V383.md](SIDE_CHEST_V383.md).

Pocket quests and saved detours v382 give walking/side quests persisted hidden-time bonus XP, useful materials and Pocket Pathfinder through durable completion receipts. Detours require >=500 m from the original route for 40 minutes, confirmed by a new fresh accurate fix; saved elapsed time survives pocket/reload gaps, while an observed accurate return inside 500 m resets it. Resume Original works anywhere immediately and saves every unclaimed detour find; only explicit completion pays rewards. Preserve in-place quest container transfers, transactional pocket ownership, async completion identity guards, all v381 collection/45 m chest/route rules, and command-centre startup. Ordinary/starter/woodland and side discoveries commit rewards before success effects. Both changed cores use `pocket-detours-v382-20260910` in index/all three worker lists; unchanged v381 modules keep their pins. Details: [POCKET_DETOURS_V382.md](POCKET_DETOURS_V382.md), [POCKET_QUESTS_V382.md](POCKET_QUESTS_V382.md), [DETOURS_V382.md](DETOURS_V382.md).

Map trails v381 models the building story stops and wandering tavern at their actual checkpoint/tavern coordinates, and draws two dotted quest edges six metres either side of the unchanged route. All map item claims share the yellow circle’s 185 m boundary and fresh/accurate GPS guard; distant side-quest items cannot be banked on exit. Walking chests use exactly 45 m and can be collected before missed flags, while other waymarkers and finish remain ordered. Preserve durable reward receipts/rollback, saved quests, terrain/culling/disposal and all worker/updater pins. Details: [MAP_TRAILS_V381.md](MAP_TRAILS_V381.md).

Personal home v379 (v380 narrow-phone correction) introduces Merlin’s woodland clearing, first-person house and aspect-matched medieval command desk. The existing Scan interface is the computer; returning launches open it while saved quests remain intact. Keep home progression/decoration/discoveries separate from settlement economy, commit timber and home changes atomically, preserve once-only gifts, clear desk/door paths, explicit sensor gestures and disposal/generation guards. Four new home assets belong in every worker list and updater. Controls, lifecycle, tests and recovery: [PLAYER_HOME_V379.md](PLAYER_HOME_V379.md).

Scan home v378 promotes both real scanner buttons above a painted home panel with gated care/reward shortcuts and actual village happiness. Preserve explicit media gestures, continuous-listener generation guards, camera crop/upload, honest economy-derived counts, existing route gates and active-walk startup. Home shares the badge heartbeat; no new timer or reward/save authority. Register all four home assets in every worker list and the guarded updater. Contracts and evidence: [SCAN_HOME_V378.md](SCAN_HOME_V378.md).
...
Living map v373 stabilizes the original forest density and verified elevations through zoom/quality changes, adds grass and native water detail, and places fictional enterable wayside stops on eligible mapped walking nodes. Preserve arrival freshness/accuracy/distance gates, delayed-load cancellation, independent furnished-room renderer disposal, exact camera return and transactional visits without settlement/economy grants. All five new modules/styles and changed query pins belong in every worker list and the updater. Behavior, bounds and evidence: [LIVING_MAP_V373.md](LIVING_MAP_V373.md).

Woodland gathering v372 recovers missed initial map-style installation on idle and adds timber supplies inside verified woodland polygons. Keep hole validation, fixed supply IDs independent of tree detail/route clearings, stale-worker guards and existing GPS/range/durable-claim authority. Narrow maps start at 360 trees/DPR 1.25 with at most four tree draws; moving-frame adaptation preserves the honest terrain fallback. `village_harvest.js` and `village_harvest_core.js` let walkers gather three timber or two stone from actual scenery after three strikes, once per object per UTC day. Preserve collision, transactional rollback, indoor/modal input isolation and chip disposal on exit. Register both harvest modules and matching geographic/walking pins in all worker lists and the updater. Controls, performance limits and evidence: [WOODLAND_HARVEST_V372.md](WOODLAND_HARVEST_V372.md).
...
Walkable interiors v368 uses `building_rooms_core.js`, `building_rooms_scene.js` and `building_rooms.js`: twenty stable cabin layouts plus thirteen service/civic rooms, attached to the existing building/home identities. Physical village doors and village/town building cards enter real geometry on the borrowed walking renderer. Keep the exact outdoor return, direct-card return, furniture collision, indoor discovery/input isolation, exposure restoration and complete mesh disposal. Track/cancel town RAF and pause timers as well as village timers. Pub counter actions must pass their actual ward seed to the existing shop, preserving discovery/building and Magpie Market gates. Visits never create population or production; v370 explicit finite finds are the only new item/reward transaction, and existing needs persistence remains authoritative. Register all three modules in all worker lists and the legacy updater. Tests and player/maintainer details: WALKABLE_INTERIORS_V368.md.

Village discoveries v367 adds 50 original villager requests, 30 randomized fictional lore entries, and 3–5 seeded catalogue loot pickups per walked village. `village_discovery_content/core.js` assigns a saved shuffled quest deck: no repeat among the first 50 distinct villages, even when quests are declined or unfinished. Actual ledger-backed Peeps can offer/finish the village request; a physical request post preserves playability in empty villages without inventing population. Ordered blue task markers, gold loot/request markers and pale lore scrolls live only during the borrowed walking session. Flood-fill placement uses the real collision world and spawn; no pickups across disconnected walls/water. Journal retains found lore and quest progress. The index adapter saves claim flags and actual inventory rewards together, rolling back on storage failure; repeated finish/loot actions cannot pay twice. Rewards use modest existing catalogue gear/materials and do not bypass equipment or trade gates. Register all three discovery modules in every worker shell and the legacy updater. Preserve touch journal scrolling, modal movement reset, Back/Escape closing the journal before walking, and disposal on every exit. Tests and review evidence: VILLAGE_DISCOVERIES_V367.md.

Geographic terrain v366 uses Mapterhorn Terrarium WebPs, 512 px, source maxzoom13 and elevation exaggeration1; this zoom cap is a rendering budget rather than a provider limitation. Preserve original map/route coordinates and existing GPS, checkpoint, distance, save and reward authority. Forest placement stays bounded, deterministic and confined to verified woodland polygons with holes respected, route corridor clearance and worker generation/disposal guards. Keep geographic_marker_layer.js loaded before geographic marker creation: public batched projection preserves DOM handlers and true coordinates while avoiding GPU depth readback. Preserve the honest terrainReduced mode with real-DEM hillshade, pitched woodland, 2D→3D retry and flat/offline fallback. Maintain provider attribution and the same-origin terrain credits; register all seven dependencies in the three worker lists and updater. Physical-phone/outdoor claims require their own evidence. Full design and source licence details: GEOGRAPHIC_MAP.md.

Illustrated world v366 (combined release integration, 2026-09-07): Birds begins with Companions/Birdex and compact rarity/family filters; the currency/resource strip and floating coin badge are hidden throughout Birds, Bird information and companion equipment, while remaining available in other appropriate views. Card fronts show actual art, name/bond, level, fullness/health and care; the owned reverse now flips with mouse/keyboard and keeps the inactive face inert. Nested Feed, activity, facts and equipment retain separate actions. Activity uses the existing effective assignment resolver; idle is labelled Free at the Academy, and the compact icon beneath the name/hearts navigates without reopening another modal. Imported source accounts in data/bird-education-enrichment-v366.json override legacy notes before the primary-guide dataset; retain exact revisions, contributor/licence links and taxonomy notes, and never mark these imported accounts verifiedPrimary. Source paragraphs must not pass through the old advice/game-template keyword filters. Primary natural-history enrichment is in data/bird-facts-v366.json: prefer canonical/scientific identity, including explicit education-only Peregrine/European Robin aliases; omit generic legacy placeholders, retain links to every source and keep game statistics inside a separate details section. Bird information has a sticky close control, Escape/Tab handling and focus restore; browser Back also closes the Feed sheet. Generated folio and settlement atlas provenance: assets/illustrated-world-v366/README.md. The atlas changes only existing marker presentation, never coordinates, tier grouping, ownership or difficulty. fitEmpireAtlasHeight measures the real dock and observes layout boundaries, not scroll/pan frames. The global-money observer ignores clipped canvas descendants and its own badge, avoiding forced layout on every map-marker transform while preserving immediate overlay/header checks. All consuming files, facts, art and village walking modules must remain in all three worker lists and the updater. Combined behavior/evidence: BROWSER_V366.md; geographic 3D is integrated in the same release.

Field map v365 (`field-map-v365-20260907`) gives the real map distinct woodland/grassland, cream paths, teal water and legible ink labels. `field_map_ui` changes only provider layer paint/visibility and map-control icons; it never rewrites coordinates, sources, route shapes, camera or saves. Keep its CSS/JS in all three worker shell lists and the updater. Stores navigation now checks the existing inventory gate centrally in `switchScreen`, including HUD shortcuts; established equipment/quest/level12 eligibility stays unchanged. HUD balances remain visible with truthful locked labels. Evidence: BROWSER_V365.md.

Walkable village (2026-09-07, integrated in v366): `village_walk.js/core/scene/css` adds a lazy first-person visit using the exact existing ledger-backed scene, a borrowed renderer/canvas, model/terrain collision and independent touch/keyboard input. `VILLAGE_WALK_INTEGRATION.patch` is the narrow index adapter, tested in the isolated feature checkout; release owner applies it and registers all four files in all three SW shell lists plus the updater. Preserve its first-priority back/navigation exit, animation-only frame flag, tracked RAF/timer cancellation and render/resize guards. Temporary batches restore originals and dispose on exit; no new context or save schema. Do not mistake desktop phone emulation for physical-phone performance. Contract, behavior, tests and measured limits: `VILLAGE_WALK_20260907.md`.

Tutorial contrast v364 (`tutorial-contrast-v364-20260907`) fixes the shared Normal-theme tutorial hint/title/body colors left dark when the bubble became woodland. Tap Merlin, Tap Feed and adjacent lesson prompts use bright cream/gold on explicit dark surfaces; Comic keeps ink on paper. Only scoped inline CSS and the release/cache stamp change: step IDs, actions, saves, tutorial resume and all module URL pins stay intact. Focused contrast and saved-step checks are documented in BROWSER_V364.md.

Companion life v363 (`companion-life-v363-20260907`) combines first-only discovery rewards, existing CHA/Goldcrest migration, continuous card swipes, four special portrait animations and Merlin flight/Play atop Appearance/walking quests. All runtime modules and eight new WebP assets install atomically for offline use; lazy decoding remains bounded during play. Existing module URL pins stay unchanged while the build/cache advances. Preserve Personality in the active carousel panel, the special portrait swipe exception and delegated tap cancellation. Combined validation and saved-client upgrade evidence: BROWSER_V363.md.

Merlin flight follow-up (`merlin-flight-v1-20260907`, awaiting coordinating release): successful Play starts a bounded 25-second interactive flight using 64 generated raster frames. An explicit play sky accepts pebble taps while native game controls keep their own input; Care and Finish remain available. Taps, pickup and carry never consume inventory, save or grant care rewards. Existing Play cost/cooldown and the saved ten-second Rest transaction remain in `merlin_companion_core.js`, unchanged. Rest, expedition absence, navigation, backgrounding, resize and motion-preference changes cancel the visual session. Keep the hidden perched host absolutely positioned on its branch so a manually finished flight lands at the correct place. The 256px atlas cells share body pivot [128,160] and per-frame talon attachments; use the actual atlas in motion-continuity tests. Provenance and motion references: `assets/merlin-flight/README.md`. Node/Python regression and disposable real-browser evidence: `tests/test_merlin_flight_20260907.*`, `tests/run_merlin_flight_v1.cjs`, `BROWSER_MERLIN_FLIGHT.md`. The four consuming dependencies are in all three SW shell lists and the legacy updater. Build/cache version promotion remains with the release owner; no deployment was performed by the animation task.

Appearance v362 (`appearance-v362-20260907`) integrates woodland v360 and walking quests v361. Settings → Appearance offers Normal (default woodland) and the preserved v359 Comic Book style. Only `gameState.settings.appearance` changes; `appearance_core.js` applies before first paint and after load, and uses the existing durable save with rollback on storage failure. Comic root variables are scoped to the selected theme; both styles and all quest surfaces work together. Keep unchanged v360/v361 module pins distinct from the current build pin. Both themes, appearance core/UI and print art must be registered in all three service-worker lists and the updater. Changing a theme while a quest map brief is open remeasures and refits the same selected route without changing its geometry or progress. Theme behavior/save protection and release completeness are covered by appearance tests. Browser/update evidence is documented in BROWSER_V362.md.

Personality card correction (2026-09-07, pending release integration): full equipment cards now show **Personality (CHA)**, with the same `cha` value on companion/Birdex reverse cards, field guide and sound discovery backs. No second personality stat is introduced: CHA remains the existing diplomacy and civic-role input. Goldcrest's `BURBZ_CHARM` baseline is 9/10 (90 at level 1), a requested game-design choice about its vocal leadership personality, not a scientific rating. `migrateBirdPersonality` runs on local and cloud flock loads, raises only Goldcrest to its level/training-aware minimum, preserves higher trained values, fills missing CHA and does not touch XP, injuries, names, equipment, bond, size or any other stat. UI reads the same saved value the roles use; no blanket biology-version bump. Regression: `node tests/test_bird_personality_20260907.cjs` (also pytest).

Discovery rewards (2026-09-07, pending release integration): accepted scans now grant discovery XP/coins/toast/encounter banner only for the first persisted species. `rememberDiscoveredBird` claims the Birdex record synchronously before reward callbacks; legacy aliases, catalogue IDs, scientific names and companion-only saves count as known. Repeat sound/photo detections no longer grant player or companion XP; sightings, best confidence, saved photo metadata and explicit quest/recruitment actions remain. The sound shelf keeps one tile per species with a 420ms repeat fade (disabled for reduced motion), no queued notice. Existing alias indices and full `burbz_state`/cloud shape stay intact. Seven runtime regression groups: `node tests/test_discovery_once_20260907.cjs` (also runs in pytest). Release owner must append their build/cache stamp; no standalone deploy from this branch. This uses the existing whole-save persistence: two independent devices with stale divergent saves are not a server-atomic discovery ledger. No XP is removed or replayed for old discoveries.

Special card sprites (unpublished, 2026-09-07): unlocked Rook Witch, Peregrine Falcon, Brandon Lee (Carrion Crow), and Steven (Herring Gull) portraits play one short generated eight-frame animation per tap. `special_bird_sprites.js/css` owns bounded lazy playback, alpha sheets and fixed scene plates live in `assets/special-birds/`, and original pictures remain the fallback. Default-only special character naming preserves player custom names and peregrine identity/moves. Steven is limited to Herring Gull/European Herring Gull/Larus argentatus. The full companion portrait now selects canonical Birdex art for these four only. Preserve the `.special-bird-play` exception in swipe-start control exclusion, the ancestor swipe click guard, eager image/fallback attrs and inactive panel inertness. Modules are in all three SW shell lists; seven media files are in the live updater and all three offline shell lists. Release/build stamp belongs to the consolidating owner. Validation: `tests/test_special_bird_sprites_20260907.cjs` and isolated real-game `tests/run_special_bird_sprites_20260907.cjs`; artwork provenance/cadence in `assets/special-birds/README.md`. No XP, care, item, save or Merlin-flight mechanics change.

Continuous companion swipes (2026-09-07, release pending): `#birdEquipTrack` keeps the current full card plus two inert neighboring panels. Only the shared track translates; no card opacity fade or timed HTML swap. The incoming panel and decoded portrait become `#birdEquipBody` at commit. `renderBirdEquip(target, previewBird)` renders neighbors without opening their pickers; always target the non-inert center for card actions/portrait effects. `birdEquipSwipeReset` cancels gestures and animations on render, close and resize. Regrabbing a settling swipe freezes its actual visual position; cancelled/vertical/multitouch gestures do not commit. Existing flock wrapping is preserved. One 8ms tick uses the existing vibration preference, with unsupported API fallback; reduced motion skips settling animation. `birdEquipArtReady` hides the reserved portrait fallback only after decoding. Inline pager/image handlers must remain exported from the game's IIFE. Reproducible touch regression + 390px evidence: `tests/run_continuous_card_swipe_20260907.cjs`, `tests/serve_card_swipe_fixture_20260907.py`, `BROWSER_CARD_SWIPE_20260907.md`. Release/cache pin remains owned by the consolidating release.

Walking quests v361: `walking_route_core.js` derives complete loops and useful out-and-back walks from eligible tagged OSM ways with shared node topology; no tile feature inference, proximity joins or router self-certification for new offers. Preserve every route vertex and route evidence through preview/start/reload; v1 routes must never enter legacy doubling, trimming, rechart or automatic return upgrades. Complete distance includes return; trailhead proximity is straight-line and excludes travel to start. Unknown/conditional/private access, barriers, missing geometry and bounded-data failures produce honest alternatives. New checkpoints require accurate GPS and <=45m arrival, with an explicit out-and-back turnaround; legacy saves retain their route, progress and reward rules, but uncertified routes cannot award progress. `walking_encounter_core.js` adds fictional Alderwing discoveries on existing ordered checkpoints, after durable chest claims. Narrative choices award no currency, persist once and enter completed journals; existing Twenty Roads campaign and rewards stay intact. Generated alpha-preserving WebPs and prompt provenance live in `assets/walking-quests/`; quest presentation is scoped in `walking_quest_ui.js/.css`. Map inspection activates the real offer overlay, fits the complete route above the measured brief, and keeps a flat camera without passive GPS recentering; delayed map load rechecks the current selection before fitting. Closing inspection restores the normal player camera. Network, encounter, UI and integration behavior are covered by the new 20260907 tests. Do not interpret simulated GPS/QA screenshots as outdoor validation.

Woodland finish v360: replaces the rejected v359 bright CSS opt-in with `woodland_ui.css` and `body.woodland-ui`. Restores the preceding black/walnut, parchment text and brass/gold palette; adds bevelled panels, restrained highlights, clearer reading text and active/focus states. v358 architecture/flock geometry and v357 mobile depth precision remain unchanged. v359 fonts, battlefield art and short-screen battle layout are retained; bright paper texture is no longer requested. No gameplay handlers or saves change. Historical comic stylesheet remains inactive for rollback.

Comic UI v359: `comic_ui.css` is a CSS-only presentation layer over the existing game, enabled by `body.comic-ui`. Bright ivory/yellow/teal panels, thick ink edges and hard offset shadows cover the HUD, dock, collections, quests, Academy room controls, settlements, shops, Forge, dialogs and battle. Bird artwork, canvases, 3D renderers, progression and combat code are unchanged. Do not apply global image filters or place the print texture over a world canvas. The new generated print/battlefield and local fonts are documented in `assets/comic-ui/README.md` and registered in all three service-worker shell lists and the legacy updater. Keep scoped overrides for older screen-ID selectors and inline rarity text; pale text on paper is a regression. Preserve hidden/locked controls and the original room/quest/battle handlers. The arena keeps its bounded scrolling layout, with a two-column squad arrangement on short landscape screens and original bird portraits.

Crafted Academy v358 (`crafted-academy-v358-20260907`) rebuilds all 13 Academy room models: chamfered timber/plaster bays, real joinery, continuous curved roofs with individual tiles, striped fabric awnings, a woven nursery and a fluted Observatory with an external telescope balcony. Static architecture still bakes into one vertex-coloured shell per room; windows and animated props remain separate. Geometry is the artwork here: no new raster downloads or gameplay/save changes. `userData.architecture` records model version/signature/part counts for diagnostics. Keep helper geometry finite, outward-facing and room-sized; fractional roof powers must clamp the eave parameter. Existing anchors, owned-room filtering, touch routing, adaptive resolution and v357 highp depth sampling remain unchanged.

Settlement birds now hinge at their shoulders. The old wing boxes rotated about their centres at z=±0.16, visibly leaving the body on every stroke. Tapered wing geometry extends from shoulder-space at z=±0.04; both village and town continue using the same factory/animation. Keep three meshes per bird and the bounded shared flock resources. `tests/test_flock_shoulders_v358.cjs` fails on v357, checks full-wave attachment and forward-facing flight, and `tests/test_academy_geometry_v358.cjs` checks every model. Both run in pytest through `test_academy_and_flock_20260907.py`. Browser before/after close-ups and actual room taps: `tests/run_academy_models_v358.cjs`; integration and evidence notes: `BROWSER_V358.md`.

Ink precision v357 (`ink-precision-v357-20260907`) fixes horizontal screen-wide stripes on mobile GPUs: the compositor must declare `uniform highp sampler2D depth` explicitly. High-precision floats do not upgrade a sampler's independent default lowp precision. Quantized depth was being detected as false contours. The compositor also requests highp arithmetic; devices without fragment highp retain cel shading without the depth pass. Preserve this precision contract and the actual-GL regression in `tests/run_ink_precision_v357.cjs` (simulated mobile sampler precision, seven DPRs, flat-plane rejection and real-box outline retention). Academy code/URL and gameplay remain unchanged.
...
Previous build: `concise-onboarding-v353-20260906`. The opening is four short steps and all 18 tutorial chapters use concise action-first copy (35 steps total). Required play/rest lessons are removed; care mechanics and rewards are unchanged. Stable lesson IDs and a legacy positional mapping resume existing v7 saves at the next surviving lesson without replaying food or resetting completed chapters. Every bottom-dock destination remains visible in a fixed position; locked icons are outlines with accessible unlock hints. Capture-phase click guarding preserves existing unlock rules for routes and quick actions. Badge labels refresh correctly after unlock and locked destinations have no actionable badges. Published through PR #284, main `bfa2c76aff838d8ecf2c359400e6a4f15b3af7a8`.

Published release: 2026-09-06 (`woodland-ui-polish-v352-20260906`), PR #283, merged main `ee5046e0f1a4686292a4922bd1424703cf80e3c0`. Normal production sync completed and all managed-file hashes were verified. CSS-only polish added readable desktop widths, consistent gold panel edges, clearer statistics/quest text and narrow-phone action wrapping. Gameplay and save data were unchanged.

Previous release: 2026-09-05 (`painted-forge-anvil-v351-20260905`) — Yaan requested a better Forge screen with a generated anvil. The Craft tab now centres a 512px transparent painted iron/oak anvil matching equipment artwork; its original alpha is preserved. The hearth level and capability remain visible; native “Improve your forge” disclosure holds the existing price and upgrade action. Empty anvils are counted in one compact row; active jobs keep their existing progress/collect/cancel controls. Recipe stats/costs are more readable, action targets are at least 44px, narrow phones move Forge buttons below recipe text, and Materials becomes one column when needed. Craft stays the default; explicit Equip links stay explicit. No recipe, timer, refund, rarity, tempering, equipment, currency or saved-state logic changed. No settlement texture pack was added. Asset provenance/prompt is in `assets/forge/README.md`; it is included in the service worker and legacy updater. Existing v350 core URL pins remain unchanged. Browser evidence is documented in `BROWSER_V351.md`.

Previous release: 2026-09-05 (`little-folk-residents-v350-20260905`) — **Peeps are little humanoid people.** Yaan explicitly corrected v348: birds are Alderwing’s intelligent dominant species; humanoid village/town folk have simple birdlike minds and need the player’s care and guidance. **Peeps** is their provisional name. Use a warm, gently funny tone. The original simple hooded silhouette returns, with face, hands and walking legs, stable clothing/skin variation by saved identity and a shared animated humanoid rig. Actual birds, zombie-bird enemies and animals remain their species. This is a canon invariant, not a request to turn bird companions into people.

The v2 resident ledger corrects species to `kind:humanoid`, retaining IDs, names, home/job assignments and the next-ID counter. `peep_needs_core.js` gives the bounded visible census persistent local thirst, energy and enjoyment. Thirst sends a Peep along real reachable routes to a built staffed well; water helps only after arrival. Tired Peeps return to their own home and sleep. Missing entertainment visibly lowers their mood; missing or blocked destinations never grant imaginary relief. Needs and positions survive save/reload; changing village/town coordinate frames remaps positions without resetting needs. Long offline gaps add no new penalty. This simulation makes no NPC API calls and does not alter resource production, taxes or existing economy clocks. Do not apply ambient visitor animation over census residents. The v348 building renderer is preserved.

Tapping or greeting a humanoid Peep plays an original ElevenLabs nonsense murmur from a sixteen-clip bank. It uses the existing sound manager, modest volume, no immediate repeat, one active voice across both scenes and no queued tap storm; sound mute stops active speech. Resident selection/info remains independent of playback. The voice files and changed audio/resident modules are required by the updated service-worker shell. No account credentials or third-party calls ship to players. The separate unfinished v349 progressive-navigation draft is excluded; its worktree must be preserved.

Photo identification now fails closed through the managed `photo_id.py` adapter: a clear live bird, useful visible features, a sufficiently large subject, confidence at least 0.90 and an unambiguous species are required. Ambiguous, tiny, blurred and nonbird inputs return **Bird not found** with a closer/clearer tip. The client requires the v350 acceptance marker before any discovery or reward path. There is no weak fallback guess. These are conservative heuristics, not a claim of calibrated recognition accuracy. Clear Robin/Great Tit fixtures and four negative fixtures have a repeatable real HTTP release proof; the user's original distant photo was not supplied. Audio recognition remains separate.

Attack spells open a 44px thumb slider near the acting bird, with a curved guide from the actual bird artwork, target/splash markers and explicit Cast/Cancel. Selection, dragging and cancellation spend nothing. The projectile arrives before engine resolution; casting locks duplicate inputs. Frost is direct, Ember splashes adjacent occupied slots at 35%, Tempest at 55%; defeated slots never bridge a gap. The engine applies each target's defence and barrier to its own scaled hit. Costs, cooldown and turn are spent once. All damaged birds receive one brief hit pulse, or a static highlight with reduced motion. Removed targets cancel freely; screen navigation clears a pending guide and waits during flight. Existing physical attacks and all-target signatures retain their behavior.

All five quest categories use original transparent painted icons in `assets/quest-categories/`, matching the equipment art. Provenance is recorded beside the assets. New/changed runtime modules, voice clips and icons are pinned in the service-worker shell. `scripts/install-photo-id.sh` promotes and restarts the photo adapter before the frontend, verifies real HTTP outcomes, and restores the prior adapter on failure. Never replace runtime verification with a source-copy check. Browser evidence is documented in `BROWSER_V350.md`.

Photo readiness correction: the backend has no `/api/health` route. The first v350 attempt safely restored the old adapter before publishing because that invented probe returned 404. Readiness now POSTs `captureSource=camera` without a file to the actual `/api/identify/image` route, requiring its specific HTTP 400 JSON response before the six real-image tests. A response from an unrelated/missing route is never accepted as readiness.

Both settlements use `settlement_lighting.js`: a warmer key and cool fill, cached stationary shadow maps, up to three instanced contact/doorway-light draws, and spatial batches of stationary tree parts. Animated birds, animals, Peeps and building selection remain separate. This is conventional raster lighting, not ray tracing. Moving Peeps get contact shadows rather than stale projected shadows. The quality governor targets a 16.7ms budget, reduces resolution on sustained misses (including very slow frames), recovers only after three good stretches, and leaves reduced-motion frame caps intact. Tests on Windows Chrome with Intel Iris Xe/D3D11 at 390×844 CSS pixels and DPR 2 averaged 13.05ms village, 13.49ms town and 15.51ms for a 48-building town; p95 was 23.3/27.0/28.4ms. These are desktop phone-emulation observations, not a physical-phone or universal 60fps guarantee. Software rendering was much slower.
...
`settlement_life_core.js` owns the saved resident census and visual routines. Its workforce calculation preserves the existing food-first allocation. Each population member keeps an id/name/kind (v350 corrects the old species label), a capacity-backed home and an assigned job; removed buildings/capacity changes reconcile only displaced assignments. The renderer shows a bounded sample across wards, with actual doorway routes, work, leisure, home time and sleep. Routes compile once per scene and use rotated footprints, so bounding boxes do not falsely seal a neighbour's street. These routines never award resources or change economic/offline clocks. The dog faces its direction of travel and uses forward/back two-joint legs with planted stance and a sniffing pause; reduced motion is respected.
...
Previous release: 2026-09-03 (`trading-manager-gates-v346-20260903`) — **trade begins at the Magpie Market, and village management must be earned.** The Magpie Market is now an early level-4 Academy goal immediately after the Quest Roost; paid item buying and selling — including trail-tavern rounds and village supply carts — are hard-locked until it is built, with every locked commerce surface pointing back to the recommended Market build. Any construction shortfall now opens a matching bird-and-duration SEND sheet directly: coins, timber, the new Stone Run, or the exact specialist-material errand; unaffordable Hall and whole-settlement upgrade buttons therefore stay tappable. The Academy gains the level-6 Project Manager's Office, but its build card stays locked until one village contains every village building type; only then can it be built, and only a built office permits a Project Manager appointment (the Town Lord Mayor remains separate). Existing roles never auto-grant the office, so no save can bypass the completed-village milestone. Contracts: `tests/test_trading_manager_gates_20260903.py` plus the updated Market, quest, Academy, store and role suites; the three Academy render cores and service-worker cache use the same release pin.

Previous release: 2026-09-03 (`visible-build-shortfall-v345-20260903`) — **the build button inside a liberated village works when the purse is short.** The button was registering correctly, but the coins/timber quest prompt opened at z-index 640 beneath the new building interior at z-index 1200, then `buildingInteriorBuild` immediately repainted the room over it. The player heard the tap and saw nothing. The handler now detects the open resource prompt, closes the room and leaves the prompt visible; affordable builds still start and redraw the room normally. Contract: `tests/test_visible_build_shortfall_20260903.py` executes both branches and pins the live PWA build/cache marker.

Previous release: 2026-09-02 (`generated-building-interiors-v344-20260902`) — **every building now opens onto a real painted place.** The fifteen governor buildings and the unbuilt plot each have an original 1448×1086 woodland-fantasy painting under `assets/building-interiors-manga/`, generated for Burbz from the game's own visual language and stored as same-origin WebP files. `building_interior_core.js` is no longer an SVG illustrator: it is a small deterministic art/state adapter that selects the correct painting and keeps level, crew, stores and construction truth in the DOM. The room art has a restrained slow camera breath, material-specific glints and construction dust; `prefers-reduced-motion` turns all movement off. All sixteen files are precached as best-effort generated art, the core moved in every service-worker list, and the head build/cache lineage moved together. Contracts: `tests/test_step_inside_buildings_20260901.py` now proves the complete unique 16-image roster, full-size valid WebP files, honest state attributes, no SVG fallback and complete release plumbing.
...
9. **Hide coin HUDs in tutorials and Birds.** Hide both displays while the
   shared tutorial overlay is active, including replay and free-action steps,
   and throughout Birds, Bird information and companion equipment. Restore
   them in other appropriate views without modifying the purse.
