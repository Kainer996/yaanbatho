...
| `empire_grid_core.js` | The Empire screen's colour ladder (`empire-grid-v320-20260825`). Pure: given one holding's plain facts it names the single thing that holding wants most, as an id + tone + icon + line of words. Reads no game globals, so the whole ladder is unit-tested in bare Node. |
...
- **2026-08-25 — the box of boxes (Claude).** Release
  `empire-grid-v320-20260825`, from Yaan's screenshot of the Empire screen.
