Bird patch map v463 (`bird-patch-map-v463-20260924`) draws the home patch of each bird the player hears or photographs on the real map, pulses it while the bird sings, and teaches that a bird met again in its patch is the same bird. Meetings stay on the phone. Keep the exact v463 pins in index.html, all three worker lists and the updater. See [BIRD_PATCH_MAP_V463.md](BIRD_PATCH_MAP_V463.md).
