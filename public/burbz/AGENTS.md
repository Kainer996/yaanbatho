...
First-person equipment feature (awaiting coordinated integration): `first-person-kit-v1-20260910` compacts the quest tracker and adds icon controls plus the keeper loadout at `inventory.equipment['@player']`. Preserve the shared gear bag, atomic bird/player swaps and rollback, Market selling protection, and player-only exclusion from the bird-equipment quest. See [FIRST_PERSON_EQUIPMENT_V1.md](FIRST_PERSON_EQUIPMENT_V1.md) for tests, shared-file boundaries and release promotion. Combat actor/target routing remains an explicit unresolved decision; do not present the separate unregistered projectile groundwork as complete gameplay.
