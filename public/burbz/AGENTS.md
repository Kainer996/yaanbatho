...
Illustrated world v366 (combined release integration, 2026-09-07): Birds begins with Companions/Birdex and compact rarity/family filters; counters remain in their appropriate HUD/Stores views. Card fronts show actual art, name/bond, level, fullness/health and care; the owned reverse now flips with mouse/keyboard and keeps the inactive face inert. Nested Feed, activity, facts and equipment retain separate actions. Activity uses the existing effective assignment resolver; idle is labelled Free at the Academy, and the compact icon navigates without reopening another modal. Primary natural-history enrichment is in data/bird-facts-v366.json: prefer canonical/scientific identity, including explicit education-only Peregrine/European Robin aliases; omit generic legacy placeholders, retain links to every source and keep game statistics inside a separate details section. Bird information has a sticky close control, Escape/Tab handling and focus restore; browser Back also closes the Feed sheet. Generated folio and settlement atlas provenance: assets/illustrated-world-v366/README.md. The atlas changes only existing marker presentation, never coordinates, tier grouping, ownership or difficulty. fitEmpireAtlasHeight measures the real dock and observes layout boundaries, not scroll/pan frames. The global-money observer ignores clipped canvas descendants and its own badge, avoiding forced layout on every map-marker transform while preserving immediate overlay/header checks. All consuming files, facts, art and village walking modules must remain in all three worker lists and the updater. Combined behavior/evidence: BROWSER_V366.md; geographic 3D integration follows its separate handoff before release promotion.
