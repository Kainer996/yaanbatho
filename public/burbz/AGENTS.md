...
Photo accuracy v393 is prepared, not published. The cropper retains up to 24 MP until the player frames the bird, removes the arbitrary 8× cap (minimum 48 source pixels), and keeps the large photo, honest indeterminate progress, cancellation and persistent outcome together in both orientations. Recognition retains the v350 confidence/margin/pixel gates and requires two blind readings to agree on scientific identity; the client requires `photo-evidence-v393` plus `verified:true`. Both readings share 35 seconds; the client retains its 45-second deadline and save/discovery authority. Never turn a disagreement, quota failure or late response into a bird. The configured free provider has a confirmed 20-requests-per-day cap, exhausted during this investigation; full real-image verification and publication are pending. Do not bypass the guarded photo installer to publish. See [PHOTO_ACCURACY_V393.md](PHOTO_ACCURACY_V393.md).
