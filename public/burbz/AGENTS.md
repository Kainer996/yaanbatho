...
Current release: 2026-08-24 (`battle-pick-your-bird-v314-20260824`) — three asks from Yaan about battles. **(1) Send in any bird.** Turns are dealt by the Combat Readiness meter, so "use any bird at any time" needed a RULE, not a tap handler. The rule: **the turn is the resource and the player chooses who spends it.** New pure `substituteActor(battle, index)` SWAPS `cr` between the bird that came up and the bird sent in. The tempting alternative — reset the substitute's meter — is a hard exploit: the ready bird would still sit at 100, so the next tick hands the player another turn, for ever. Swapping conserves team readiness exactly, and a 4000-turn sim shows the greediest swapping strategy shifts the player's turn share by 1 turn in 4000 (rounding, not gain). The turn's cooldown tick travels with it: `tickTurnCooldowns` records which skills it decremented so `untickTurnCooldowns` can hand them back, which makes swapping to and fro idempotent — without that ledger, eleven swaps would bank eleven free cooldown turns. Player side only; the evil Burbz fly their own order. UI: every living non-acting bird in the flock wears a dashed border and a `SEND IN` flag, `battleTargetPick` routes player-side taps into the substitution, and the aim resets because the pending move belonged to the bird that stepped aside. **(2) The struck bird lights up and shakes.** New `.arena-unit.hit` runs `arenaShake` plus a new `arenaHitFlash` (brightness 1.9 + a 22px glow). **This uncovered a latent bug: the shake never actually played.** `playBattleEvents` set the class on the card and then called `renderArena()` in the same tick — and `renderArena` rewrites `host.innerHTML` for BOTH rows, so the element carrying the class was destroyed before a frame was painted. The event now records `struck` and the class is applied AFTER the re-render. Both sides come free because a damage event's `ev.side` is the side that TOOK the hit. Rule for next time: in this arena, anything set on a card by hand must be applied after `renderArena()`, never before. **(3) The aimed blow drawn on the target's own health bar.** `.au-hpbar.doomed` grows to 7px, gains a red rim and a glow; `.au-hp-doom` is an absolutely-positioned slice from `pct - doomPct` to `pct` with a 2px white `border-left` — the tick is the point of it, marking where the bar will END, so a 6% graze reads as clearly as a killing blow. Width floors at 5% so a graze is never a hairline; `.lethal` (when `preview.min >= f.hp`) brightens the whole thing and the damage chip appends `· KO`. Only the bird actually aimed at glows — an AoE move glows on all of them — while the existing per-rival damage NUMBERS still show on every rival for comparison. Reduced motion keeps the flash and the colour, drops the shake and the pulse. Verified in headless boots at 390px against a real tier-4 fight: substitution swaps the TURN flag, the aimed rival's bar shows `left:94% width:6%`, and the exchange lit `unit_opponent_0` plus all three `unit_player_*` cards. Contracts: `tests/test_battle_pick_your_bird_20260824.py` (14 tests). Both key invariants are mutation-tested: reset instead of swap trips two tests, lighting before the re-render trips one. Sharp edge hit again during the pin sweep: v313's suite had `OWN_RELEASE_PIN` on the head build, so the sweep dragged it forward — it names v313 now, and its "no core moved" test asserts against its OWN pin rather than `CURRENT_BUILD`. battle_core's pin also moved off v287, so the four suites that track it (conquest, night-hunter, turn-potions, and v287's own) follow it here.
