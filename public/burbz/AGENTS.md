Paced tutorial v467 (`paced-tutorial-v467-20260924`): after Alderwing the opening is five short lessons (Birdhouse, discover a bird, meet Merlin, the Kitchen, explore), 12 bubbles instead of 22. After each lesson Merlin rests for two minutes (`MERLIN_LESSON_REST_MS`, localStorage `burbzTutorialRest:*`): no chapter auto-starts and nothing hands off across screens. A lesson's own chapters still flow on (`MERLIN_LESSON_GROUPS`). The Home goal ends the rest and starts the next lesson at once. Meet Merlin is its own `companion` chapter; the Play step and `academy_tour` are gone. The play flight clears the care menu on takeoff, Care lands Merlin first, Play waits during a lesson, and no lesson starts mid-flight. See PACED_TUTORIAL_V467.md. Tests: `node tests/test_paced_tutorial_v467.cjs`, browser `node tests/run_paced_tutorial_v467.cjs`.
