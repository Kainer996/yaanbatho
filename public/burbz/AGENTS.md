...
Current review build: `woodland-ui-polish-v352-20260906`. CSS-only visual polish adds readable desktop widths for collection/ledger screens and Merlin story bubbles, consistent gold panel edges, clearer statistics and quest text, and narrow-phone quest action wrapping. Maps, scenes, artwork, game handlers and save data are unchanged. Existing current-release test pins advance with the build and service-worker cache. This build has not been deployed.
