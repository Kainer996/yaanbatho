...
Pocket quest rewards (pending v382 integration) use `quest_pocket_core.js` for persisted hidden-page intervals on both walking and side quests. Preserve no-save suspension/resume hooks, existing pocket totals, completion receipts, atomic XP/material/badge/history saves and concurrent finish guards. Hidden means screen-away/background, not verified physical pocket or background GPS. Profile shows Pocket Pathfinder; completion records keep the exact bonus breakdown. Release owner registers/pins the module with the coordinated detour/claim changes. Details: [POCKET_QUESTS_V382.md](POCKET_QUESTS_V382.md).
