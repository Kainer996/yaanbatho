...
Current release: 2026-09-03 (`visible-build-shortfall-v345-20260903`) — **the build button inside a liberated village works when the purse is short.** The button was registering correctly, but the coins/timber quest prompt opened at z-index 640 beneath the new building interior at z-index 1200, then `buildingInteriorBuild` immediately repainted the room over it. The player heard the tap and saw nothing. The handler now detects the open resource prompt, closes the room and leaves the prompt visible; affordable builds still start and redraw the room normally. Contract: `tests/test_visible_build_shortfall_20260903.py` executes both branches and pins the live PWA build/cache marker.
