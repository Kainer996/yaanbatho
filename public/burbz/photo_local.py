...
UNCERTAIN_MESSAGE = "Bird detected, but species not confirmed. Try another angle showing the bird’s head, wings and tail. A tighter close-up may hide useful details."
...
def clear_pixels(image, box):
    """Check bounded source pixels; enlargement cannot rescue a tiny/blurred bird."""
...
class Recognizer:
    def __init__(self, root):
        ...
        self.bundle_hash = hashlib.sha256((root/"manifest.json").read_bytes()).hexdigest()
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def reply(self, status, result):
            if self.path == "/identify":
                # Diagnose rejection/service failures without retaining any photo,
                # species, filename, coordinates, account or uploaded metadata.
                print(json.dumps({"event": "photo-result", "model": MODEL,
        ...
        def do_GET(self):
            self.reply(200 if self.path == "/health" else 404, {"ready": self.path == "/health", "policy": POLICY,
                       "model": MODEL, "sourceHash": SOURCE_HASH, "bundle": recognizer.manifest["id"],
                       "bundleHash": recognizer.bundle_hash})
