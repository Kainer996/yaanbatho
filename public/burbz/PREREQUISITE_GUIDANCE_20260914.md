...
The original recipe is retained for the current page session and across screen navigation. It is **not persisted across reload/app closure** in this bounded slice; reopening the same recipe recreates the plan entirely from current authoritative state. No new game-save schema, grants or reset hooks were introduced.
