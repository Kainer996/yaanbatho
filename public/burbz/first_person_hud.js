...
function attach(s){
 ...
 let last=-Infinity,focusBefore=null,packKey='',equipment=null;const title=character.querySelector('strong'),level=character.querySelector('small'),bearing=compass.querySelector('.fp-bearing'),quest=tracker.querySelector('strong'),progress=tracker.querySelector('.fp-progress'),bar=progress.firstElementChild;
 ...
 on(tracker.querySelector('button'),'click',()=>{const expanded=tracker.classList.toggle('fp-expanded');tracker.querySelector('button').setAttribute('aria-expanded',String(expanded));tracker.querySelector('button').setAttribute('aria-label',expanded?'Collapse current quest':'Expand current quest');});
