...
function attach(s){
 ...
 function expandQuest(open){tracker.classList.toggle('fp-expanded',open);const b=tracker.querySelector('button');b.setAttribute('aria-expanded',String(open));b.setAttribute('aria-label',open?'Collapse current quest':'Expand current quest');}
 function closePanel(){if(navigation?.close())return true;if(panel.hidden){if(tracker.classList.contains('fp-expanded')){expandQuest(false);tracker.querySelector('button').focus({preventScroll:true});return true;}return false;}panel.hidden=true;pack.setAttribute('aria-expanded','false');s.uiBusy=false;s.reset();last=-Infinity;update(performance.now()/1000);focusBefore?.focus({preventScroll:true});return true;}
 function showPack(){if(s.failed||s.uiBusy||typeof s.options.character!=='function')return;focusBefore=document.activeElement;s.reset();s.uiBusy=true;panel.hidden=false;pack.setAttribute('aria-expanded','true');paintPack();panel.scrollTop=0;last=-Infinity;update(performance.now()/1000);panel.querySelector('button').focus({preventScroll:true});}
 ...
 function update(time){if(time-last<.12)return;last=time;
  ...
  navigation?.update(time);for(const b of el.querySelectorAll('.vw-exit,.cw-wings')){b.title=b.textContent.trim();b.setAttribute('aria-label',b.title);}const c=stats();pack.hidden=typeof s.options.character!=='function';if(!s.room&&s.options.exitLabel)text(el.querySelector('.vw-exit'),s.options.exitLabel);text(title,c.name||'Alderwing keeper');text(level,Number.isFinite(c.level)?'Level '+Math.max(1,Math.floor(c.level))+' · '+(s.options.flight?'Academy explorer':'Wayfarer'):'Wayfarer');
  ...
  const journal=move('.vd-journal',tools);if(journal){journal.hidden=!!s.room;if(!journal.dataset.icon){journal.dataset.icon='journal';journal.innerHTML=root.BurbzPlayerEquipment.icon('journal');journal.setAttribute('aria-label','Field journal');journal.title='Field journal';}}const guide=move('.vd-guide',tracker);
  for(const selector of ['.vr-door','.vr-service','.il-use','.vd-interact','.vh-use','.af-land','.cw-visit','[data-walk-action]']){for(const node of el.querySelectorAll(selector)){if(node.parentNode!==tray)tray.append(node);node.classList.add('fp-action');if(s.root.classList.contains('vw-touch'))node.setAttribute('data-touch','true');}}
  ...
  tray.hidden=s.uiBusy||s.failed;el.classList.toggle('fp-nearby',[...tray.children].some(node=>!node.hidden));character.hidden=s.uiBusy;compass.hidden=s.uiBusy;tools.hidden=s.uiBusy;
  el.classList.toggle('fp-busy',!!s.uiBusy);if(!panel.hidden)paintPack();
 }
 const full=el.querySelector('.vw-fullscreen');if(full)full.innerHTML=root.BurbzPlayerEquipment.icon('fullscreen');
 return{update,closePanel,key(code){if(navigation?.key(code))return true;if(code==='KeyB'){if(!closePanel())showPack();return true;}return false;},diagnostics(){return{character:stats(),actions:[...tray.children].filter(b=>!b.hidden).map(b=>({label:b.textContent,disabled:b.disabled})),trackerHidden:tracker.hidden,packOpen:!panel.hidden,navigation:navigation?.diagnostics()};},dispose(){navigation?.dispose();equipment?.dispose();for(const node of [tools,tracker,tray,character,compass,panel])node.remove();el.classList.remove('fp-ready');}};
}
