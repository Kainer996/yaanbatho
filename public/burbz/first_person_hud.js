...
function attach(s){
 ...
 function update(time){if(time-last<.12)return;last=time;
  ...
  for(const selector of ['.vr-door','.vr-service','.il-use','.vd-interact','.vh-use','.af-land','.cw-visit','[data-walk-action]']){const node=move(selector,tray);if(node){node.classList.add('fp-action');if(s.root.classList.contains('vw-touch'))node.setAttribute('data-touch','true');}}
