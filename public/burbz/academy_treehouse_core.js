(function(root, factory) {
  ...
  const QUEST_TEMPLATES = {
    ...
    // --- Food gathering: these quests stock the Academy pantry (their item
    // drops are food parcels that unpack straight into pantry shelves) ---
    berry_harvest: { id:'berry_harvest', label:'Berry Harvest', minutes:45, icon:'🫐', minLevel:1, starter:true, coins:[3,10], branches:[0,1], xp:18, items:['berry_bundle','fruit_basket','berry_bundle'], beats:['flits out to the hawthorn hedges','strips the brightest rowan sprays','checks a windfall apple for sweetness','glides home with a full berry haul'] },
    bug_hunt: { id:'bug_hunt', label:'Bug Hunt', minutes:30, icon:'🐛', minLevel:1, starter:true, coins:[2,7], branches:[0,1], xp:15, items:['bug_box','worm_tin','bug_box'], beats:['sweeps low over the meadow grass','snaps up beetles and caterpillars','turns a mossy log for fat grubs','carries the wriggling catch home'] },
    pond_dip: { id:'pond_dip', label:'Pond Dipping', minutes:60, icon:'🎣', minLevel:2, coins:[5,13], branches:[0,1], xp:24, items:['pond_haul','pond_haul','bug_box'], beats:['skims the reedbeds at first light','stalks the shallows like a heron','strikes — a silver minnow flashes','flies back trailing pondweed treasure'] },
    hunt_prey: { id:'hunt_prey', label:'Hunting Flight', minutes:90, icon:'🦅', minLevel:3, coins:[8,18], branches:[0,1], xp:40, items:['prey_cache','carrion_find','prey_cache'], beats:['rides a thermal over the far fields','hangs perfectly still above the stubble','folds and drops in a hunting stoop','returns with prey for the pantry cold-store'] }
  };
