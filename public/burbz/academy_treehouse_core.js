(function(root, factory) {
  ...
  const TREEHOUSE_ROOMS = [
    ...
    { id:'hospital', label:'Bird Hospital', icon:'🏥', cost:150, unlockLevel:3, floor:3, branch:'left', x:26, y:53, role:'healing', effect:'Future: recovery care and status cures.' },
    { id:'crowbar', label:'The Crowbar', icon:'🍻', cost:175, unlockLevel:4, floor:4, branch:'right', x:72, y:42, role:'social', effect:'Social buffs, rumours, visitor birds and morale.' },
    ...
    { id:'workshop', label:'Nest Workshop', icon:'🛠️', cost:220, unlockLevel:5, floor:5, branch:'left', x:25, y:31, role:'craft', effect:'Craft perches, gear, habitats and upgrades.' },
    { id:'market', label:'Recruitment Roost', icon:'🪶', cost:250, unlockLevel:6, floor:5, branch:'right', x:73, y:29, role:'recruit', effect:'Recruit discovered Birdex species for coins when you are ready.' },
    { id:'nursery', label:'Hatchery Nursery', icon:'🥚', cost:300, unlockLevel:7, floor:6, branch:'left', x:36, y:18, role:'bond', effect:'Baby-bird care and bonding progression.' },
    { id:'observatory', label:'Moon Observatory', icon:'🔭', cost:350, unlockLevel:8, floor:6, branch:'right', x:64, y:14, role:'forecast', effect:'Forecast spawns, migrations and night encounters.' },
  ...
  const QUEST_TEMPLATES = {
    // Coin ranges are tuned so longer expeditions pay better per hour of
    // away-time, while the short starter errands stay small pocket money.
    find_seed: { id:'find_seed', label:'Find Seed', minutes:3, icon:'🌾', minLevel:1, starter:true, coins:[2,6], xp:6, items:['seed_satchel'], beats:['hops straight from the garden path','checks the soft grass for fallen seed','tucks a tiny seed satchel under one wing','returns ready for another quick errand'] },
    find_coins: { id:'find_coins', label:'Find Coins', minutes:4, icon:'🪙', minLevel:1, starter:true, coins:[5,12], xp:8, items:['shiny_pebble'], beats:['flutters toward a sunny lane','spots a glint beside an old root','trades a bright pebble for pocket coins','returns jingling with tiny treasure'] },
    scavenge: { id:'scavenge', label:'Scavenge', minutes:5, icon:'🧺', minLevel:1, starter:true, coins:[4,10], xp:10, items:['soft_moss','berry_bundle'], beats:['sets off on a quick cosy scavenge','checks mossy twigs and berry leaves','bundles useful bits for the Academy','returns with simple supplies'] },
    short_forage: { id:'short_forage', label:'Hedgerow Forage', minutes:60, icon:'🌿', minLevel:1, coins:[20,40], xp:30, items:['berry_bundle','shiny_pebble','soft_moss'], beats:['sets off beneath the safe branches','finds a singing hedgerow path','checks a glittering hollow','returns with a beakful of useful things'] },
    supply_run: { id:'supply_run', label:'Pantry Supply Run', minutes:120, icon:'🥣', minLevel:2, coins:[45,85], xp:60, items:['seed_satchel','worm_tin','berry_bundle'], beats:['sets off with a tiny satchel','trades gossip at a squirrel market','spots a cache of field snacks','returns to the tree with supplies'] },
    moon_scout: { id:'moon_scout', label:'Moonlit Scout', minutes:180, icon:'🌙', minLevel:4, coins:[80,150], xp:100, items:['moon_feather','old_map','shiny_pebble'], clueId:'moon_branch_map', beats:['sets off as the leaves glow silver','follows Merlin runes through the canopy','marks a safe route past the shadow woods','returns with a new clue for Bird Burbs'] }
  };

  const TRAINING_TEMPLATES = {
    wing_sprints: { id:'wing_sprints', label:'Wing Training', minutes:120, icon:'🪽', stat:'spd', statLabel:'SPD', xp:40, bonus:1, hunger:10, happiness:-2, copy:'A focused two-hour mobile session: quick to understand, long enough to check back later.', beats:['warms up on the lantern perch','runs short wing-burst laps','practises soft landings','finishes with brighter reactions'] },
    perch_strength: { id:'perch_strength', label:'Perch Strength', minutes:180, icon:'🪵', stat:'atk', statLabel:'ATK', xp:65, bonus:1, hunger:14, happiness:-3, copy:'A three-hour mid-length drill, good for an afternoon check-in and clear stat growth.', beats:['tests balance on the training rail','pulls weighted seed pouches','works through safe target strikes','rests after a stronger session'] },
    focus_roost: { id:'focus_roost', label:'Focus Roost', minutes:300, icon:'🎯', stat:'int', statLabel:'INT', xp:110, bonus:2, hunger:18, happiness:-4, copy:'A five-hour deeper training block for idle-game style progress while you are away.', beats:['studies target calls on the notice board','tracks moving feather markers','solves a tiny route puzzle','returns sharper and calmer'] }
  };
  ...
  function createBirdExpedition(bird, templateId='short_forage', nowMs=Date.now()) {
    ...
    // Strong birds haul a little extra, but never more than half the quest's
    // top payout — an uncapped stat bonus let maxed birds farm hundreds of
    // coins an hour off 3-minute errands.
    const statScore = (bird.power || 80) + (bird.int || 40) + (bird.spd || 40) + (bird.stamina || 40);
    const powerBonus = Math.min(Math.ceil(template.coins[1] * 0.5), Math.floor(statScore / 600));
