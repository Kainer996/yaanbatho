...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.BurbzFirstPersonSpellCore=api;})(globalThis,function(){
 ...
 function create({targets=()=>[],obstacle=()=>null,impact=()=>{},valid=()=>true}={}){
  ...
  function launch(pose,skill,token){if(projectiles.length>=MAX_PROJECTILES||!skill||![pose.x,pose.y,pose.z,pose.yaw,pose.pitch].every(Number.isFinite))return null;const p={id:++serial,position:{x:pose.x,y:pose.y,z:pose.z},direction:direction(pose),skill:structuredClone(skill),token,age:0,fall:0};projectiles.push(p);return p;}
  function step(dt){dt=clamp(Number(dt)||0,0,.1);for(let i=projectiles.length-1;i>=0;i--){const p=projectiles[i];if(!valid(p.token)){projectiles.splice(i,1);continue;}const speed=p.skill.projectileSpeed||SPEED,radius=p.skill.projectileRadius||RADIUS,gravity=p.skill.gravity||0,a=p.position,b={x:a.x+p.direction.x*speed*dt,y:a.y+(p.direction.y*speed-p.fall)*dt-gravity*dt*dt/2,z:a.z+p.direction.z*speed*dt};p.fall+=gravity*dt;let hit=obstacle(a,b,radius),target=null;hit=Number.isFinite(hit)&&hit>=0&&hit<=1?hit:null;
