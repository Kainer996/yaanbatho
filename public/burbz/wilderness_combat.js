...
(function(root){'use strict';
 ...
 function attach(s){
  ...
  const controls=root.BurbzFirstPersonCastControls.attach({host,look:s.look||host.querySelector('.vw-look'),signal:events.signal,pose,blocked:uiBlocked,begin:beginCast,cast:releaseCast,cancel:cancelCast,castAimScale:()=>core.charging()===FIREBALL?.5:1,aim:(dx,dy,k)=>{const p=player(),pitch=p.pitch-(root.BurbzLookSettings?.vertical(dy)??dy)*k;p.yaw-=dx*k;p.pitch=root.BurbzVillageWalk?.clampPitch?root.BurbzVillageWalk.clampPitch(p,pitch):Math.max(-1.1,Math.min(1.1,pitch));s.dirty=true;}});
  const climb=host.querySelector('.cw-wings')&&host.querySelector('button[aria-label="Flap"]');climb?.classList.add('wc-climb');
  ...
  function dispose(){if(disposed)return;disposed=true;reset();root.BurbzCalmAudio?.setMood('calm');fireFX.dispose();controls.dispose();core.dispose();outposts?.dispose();adapter.outposts?.dispose();boundary?.dispose();events.abort();group.removeFromParent();birdArt.dispose();for(const g of Object.values(geometry))g.dispose();for(const m of Object.values(materials))m.dispose();zombie.clear();shots.clear();bursts.clear();climb?.classList.remove('wc-climb');if(travel?.isConnected&&travelParent?.isConnected)travelParent.insertBefore(travel,travelNext?.parentNode===travelParent?travelNext:null);hud.remove();toast.remove();host.classList.remove('wc-ready','wc-paused','wc-noncombat','wc-hurt');}
