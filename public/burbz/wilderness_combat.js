...
(function(root){'use strict';
 ...
 function attach(s){
  ...
  function syncBirds(){const ids=new Set(core.actors.filter(a=>a.fighter.hp>0).map(a=>a.id));for(const [id,m]of zombie)if(!ids.has(id)){birdArt.remove(m);zombie.delete(id);}for(const a of core.actors){if(a.fighter.hp<=0)continue;let m=zombie.get(a.id);if(!m){m=birdArt.bird();group.add(m);zombie.set(a.id,m);}birdArt.update(m,a,player(),motion.matches);}}
  ...
  function update(dt){if(disposed)return;const started=performance.now(),burnElapsed=lastUpdate===null?0:(started-lastUpdate)/1000;lastUpdate=started;elapsed+=Math.min(.05,dt);boundary?.refresh(player(),elapsed);if(paused()||!core.charging())controls.cancelCast();controls.update(dt);core.step(dt,pose(),{paused:!!paused(),elapsed:burnElapsed});if(!core.charging())controls.cancelCast();syncBirds();syncFX(core.engine.projectiles,shots,false);syncFX(core.engine.impacts,bursts,true);
