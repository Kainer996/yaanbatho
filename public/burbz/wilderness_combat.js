...
(function(root){'use strict';
 ...
 function attach(s){
  ...
  const controls=root.BurbzFirstPersonCastControls.attach({host,look:s.look||host.querySelector('.vw-look'),signal:events.signal,pose,blocked:uiBlocked,begin:beginCast,cast:releaseCast,cancel:cancelCast,castAimScale:()=>core.charging()===FIREBALL?.5:1,aim:(dx,dy,k)=>{const p=player();p.yaw-=dx*k;p.pitch=Math.max(-1.1,Math.min(1.1,p.pitch-(root.BurbzLookSettings?.vertical(dy)??dy)*k));s.dirty=true;}});
