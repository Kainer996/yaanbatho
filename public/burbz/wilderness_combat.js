...
(function(root){'use strict';
 ...
 function attach(s){
  ...
  function updateHUD(){const d=core.inspect(),k=adapter.kit().loadout,L=root.BurbzLootCore;const foe=d.actors.filter(a=>a.fighter.hp>0).sort((a,b)=>Math.hypot(a.position.x-player().x,a.position.z-player().z)-Math.hypot(b.position.x-player().x,b.position.z-player().z))[0];hud.querySelector('.wc-zone').textContent=d.safe?'Safe territory':d.hero.hp<=0?'Wounded · return to safety':foe?foe.fighter.name+(foe.phase==='windup'?(foe.aerial?' · dive incoming':' · peck incoming'):''):'Wilderness';hud.querySelector('.wc-health').textContent=Math.ceil(d.hero.hp)+' / '+d.hero.maxHp+' HP'+(d.hero.barrier?' +'+d.hero.barrier+' shield':'');hud.querySelector('.wc-meter i').style.width=d.state.cr+'%';weapon.setAttribute('aria-pressed',String(d.mode==='weapon'));spell.setAttribute('aria-pressed',String(d.mode==='spell'));weapon.disabled=!k.weapon;spell.disabled=!k.spell;potion.disabled=!core.canPotion();potion.title=(L.gearById(k.potion)?.label||'Equip a potion')+' (P)';potion.setAttribute('aria-label',potion.title);if(L.gearById(k.potion)?.icon)potion.textContent=L.gearById(k.potion).icon;else potion.innerHTML='<svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="M9 3h6M10 3v7l-5 8a2 2 0 0 0 2 3h10a2 2 0 0 0 2-3l-5-8V3M8 15h8"/></svg>';const ammo=d.skill?.ammo;const cd=d.mode==='spell'?d.state.cooldowns[k.spell]||0:0;hud.querySelector('.wc-attack-name').textContent=(d.skill?.label||'Equip in Satchel')+(ammo?' · '+(adapter.kit().ammo?.[ammo]||0)+' arrows':'')+(cd?' · '+Math.max(1,Math.ceil((cd*100-d.state.beat)/Math.max(1,root.BurbzBattleCore.effStat(d.hero,'spd'))))+'s':!d.safe?' · hold / release':'');host.querySelector('.fp-cast-stick').setAttribute('aria-label',(d.skill?.label||'Attack')+'. Hold to aim; release to attack. Q or right mouse.');hud.hidden=!!uiBlocked()||!!s.room||!!s.options?.room||!!s.flight;hud.classList.toggle('wc-flying',player()?.mode==='fly');host.classList.toggle('wc-paused',!!uiBlocked());host.classList.toggle('wc-noncombat',!!paused()&&!uiBlocked());}
