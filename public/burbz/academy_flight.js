...
function attach(s,input,keys){
 ...
 const controls=document.createElement('div');controls.className='af-controls';controls.innerHTML='<div class="af-right"><input type="range" min="-1" max="1" step=".05" value="0" aria-label="Flap or dive" orient="vertical"><span aria-hidden="true">Wings</span></div><button type="button" class="af-land" hidden></button><div class="af-status" role="status"></div><span class="af-beak" aria-hidden="true"></span>';
 ...
 // Interruptions release the controls; the bird keeps its momentum.
 function reset(){slider.value='0';motionState={yaw:s.player?.yaw};motion={bob:0,pitch:0,roll:0,mode:'cruising'};}
 ...
 function update(dt){if(s.room){controls.hidden=true;return;}controls.hidden=false;
  ...
  const nearest=core.nearest(s.player,pads,100);status.textContent=s.player.landed?'Perched · enter through the door':(s.player.wing?.stall>.5?'Stalling · flap or dive · ':'')+Math.round(s.player.y)+' m · '+(nearest?nearest.label+' '+Math.round(Math.hypot(s.player.x-nearest.x,s.player.y-nearest.y,s.player.z-nearest.z))+' m':'Academy canopy');
 }
