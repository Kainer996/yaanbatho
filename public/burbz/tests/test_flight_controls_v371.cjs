...
// Controls drive real flight (v480): only Flap beats the wings, backward
// airbrakes, the sideways push banks into a turn, the view steers the path,
// and the camera follows the wings. The body never strafes or reverses.
...
const ahead=advance(fresh(),{lift:1},240);assert(ahead.z<15-10,'flapping flies ahead');assert(Math.abs(ahead.x)<1e-9);assert(ahead.y>41,'flapping climbs');assert(ahead.wing.airspeed>C.settings().stall,'flapping keeps flying speed');
const stick=advance(fresh(),{forward:1},240);assert(!stick.wing.beating&&stick.wing.mode==='gliding','the stick alone never beats the wings');assert(stick.y<40,'still wings glide down');
...
for(const side of [1,-1]){const p=advance(fresh(),{side});assert(p.yaw*side<-1,'the sideways push turns the bird');assert(p.x*side>1,'the path curves into the turn');}
...
console.log('PASS Flap-only wingbeats, glide on the stick, airbrake, banked turns, view-steered dive and climb, wingbeats, still glide, left/right banking, settle, reduced motion and body isolation');
