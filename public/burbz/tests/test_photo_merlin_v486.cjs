'use strict';
// Photo Merlin v486: photos are identified the Merlin way. The page sends the
...
const BUILD='photo-merlin-v486-20260925';

test('v486 ships together: build marker, cache, worker lists and updater',()=>{
...
test('one contract across page, queue, adapter, worker, installer and proof',()=>{
 ...
 for(const [name,text] of [['page',html],['queue',queue],['adapter',adapter],['worker',worker],['installer',installer],['proof',proof]])
  assert(text.includes('photo-gemini-v486'),name);
 for(const [name,text] of [['page',html],['queue',queue],['adapter',adapter],['proof',proof]])assert(text.includes('merlin-v486'),name);
