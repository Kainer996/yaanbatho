'use strict';
const assert=require('node:assert/strict'),fs=require('node:fs'),vm=require('node:vm'),L=require('../loot_crafting_core.js'),C=require('../player_equipment_core.js'),O=require('../enemy_outposts_core.js');
...
// Equipment transactions use the real outpost ledger; only its unused network clock is fixed here.
function fixture(){const c={BurbzEnemyOutpostsCore:O,enemyOutpostAdapter:()=>({now:()=>0}),structuredClone,JSON,Math,Number,Object,Array,String,console,MERLIN_GUIDE_SIZE:{},generateBirdStats:()=>({maxHp:80}),ensureBirdAcademy:bird=>bird,gameState:fresh(),window:{BurbzPlayerEquipmentCore:C},lootCore:()=>L,queueCloudSave(){},queueActionBadgeUpdate(){},showToast(){},SFX:{questComplete(){},tap(){}},fail:false,saved:null,effects:0};c.localStorage={getItem:()=>c.saved===null?null:JSON.stringify(c.saved),setItem:(key,value)=>{if(c.fail)throw Error('Quota');c.saved=JSON.parse(value);}};c.updateQuestProgress=(type,n,effects)=>{c.gameState.quests.gear.progress+=n;effects.push(()=>c.effects++);};vm.createContext(c);vm.runInContext(guideSource+'\n'+['merlinBattleBird','getBattleFlock','snapshotGameState','restoreStateTree','restoreGameStateSnapshot','durableSaveState','saveState','commitEquipmentChange','playerEquipmentAdapter','enemyOutpostState','walkingCombatAdapter','equipItemOnBird','unequipItemFromBird','storesSellBag','storesSellRarity','storesSellQuote'].map(fn).join('\n'),c);c.merlinId=vm.runInContext('MERLIN_GUIDE.id',c);return c;}
let n=0;const check=(name,f)=>{f();n++;console.log('PASS',name);};
check('Every one of the current Forge definitions equips and unequips without changing total ownership',()=>{const c=fixture();for(const item of Object.values(L.GEAR)){c.gameState.inventory.gear[item.id]=1;assert(c.commitEquipmentChange(C.PLAYER,item.slot,item.id).ok);assert.equal(c.gameState.inventory.gear[item.id],0);assert.equal(c.saved.inventory.equipment[C.PLAYER][item.slot],item.id);assert(c.commitEquipmentChange(C.PLAYER,item.slot,null).ok);assert.equal(c.saved.inventory.gear[item.id],1);}assert.equal(c.gameState.quests.gear.progress,0);});
...
check('The canonical Forge roster lets Merlin equip all current items without adding him to the saved flock',()=>{
