...
test('v492 ships together: build marker, cache, loader order, three worker lists and the updater',()=>{
 ...
 const LATER=['quest-lines-v493-20260926','photo-merlin-v494-20260926','village-apart-v495-20260926'];assert([BUILD,...LATER].some(b=>html.includes("const BURBZ_BUILD = '"+b+"';")));assert([BUILD,...LATER].some(b=>sw.match(/const BURBZ_CACHE = '([^']+)'/)[1].endsWith('-'+b)));assert(sw.match(/const BURBZ_CACHE = '([^']+)'/)[1].includes('-'+BUILD));
