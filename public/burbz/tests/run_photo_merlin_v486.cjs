'use strict';
// Photo Merlin v486 in the real page: a camera photo carries the rough place,
...
const out=process.env.EVIDENCE_DIR||'/tmp/burbz-photo-merlin-v486';fs.mkdirSync(out,{recursive:true});
...
const raven={found:false,accepted:false,verified:false,retryable:false,policy:'photo-gemini-v486',model:'gemini-vision',modelName:'gemini-3.8-flash',
...
(async()=>{try{
 ...
 assert.equal(request.contract,'merlin-v486');assert.equal(request.lat,'53.87');assert.equal(request.lon,'-2.39');
