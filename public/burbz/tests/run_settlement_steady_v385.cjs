/* Bounded paired steady-look diagnostic after cold-sample stalls. Original village/town render-loop comparison. Run sequentially, never beside another browser.
...
const childProcess=require('node:child_process'),originalSpawn=childProcess.spawn;childProcess.spawn=function(command,args,options){const child=originalSpawn.apply(this,arguments);if(String(command).includes('chrome')){child.once('spawn',()=>{report.browserProcessEvents||=[];report.browserProcessEvents.push({event:'spawn',pid:child.pid,command,at:new Date().toISOString()});save();});child.stderr?.on('data',data=>fs.appendFileSync(path.join(out,'chromium-stderr.log'),data));child.once('exit',(code,signal)=>{report.browserProcessEvents||=[];report.browserProcessEvents.push({event:'exit',pid:child.pid,code,signal,at:new Date().toISOString(),expected:!!report.closing});save();});}return child;};
...
const out=process.env.EVIDENCE_DIR||path.join(evidence,'steady-look-'+(baseline?'v384':'v385'));
...
async function setup(appearance){
 ...
 page=await context.newPage();page.on('crash',()=>{report.browserProcessEvents||=[];report.browserProcessEvents.push({event:'page-crash',at:new Date().toISOString()});save();});page.on('pageerror',e=>report.errors.push(e.message));page.on('console',m=>{if(/Shader Error|GL_INVALID|INVALID_OPERATION|INVALID_VALUE|WebGL context lost/i.test(m.text()))report.glErrors.push(m.text());});
 await context.addInitScript(iso=>{window.steadyNativeClocks={performanceNow:performance.now,requestAnimationFrame,setTimeout,setInterval};const NativeDate=Date,fixed=new NativeDate(iso).getTime();function FixedDate(...args){if(new.target)return Reflect.construct(NativeDate,args.length?args:[fixed],new.target);return new NativeDate(fixed).toString();}Object.setPrototypeOf(FixedDate,NativeDate);FixedDate.prototype=NativeDate.prototype;FixedDate.now=()=>fixed;globalThis.Date=FixedDate;},fixedDate);await page.goto(url,{waitUntil:'domcontentloaded'});
...
async function sample(kind,appearance,walking,motion){
 ...
 // Throughput is compared only against the paired native-clock report, never the historical fake-clock baseline.
...
async function diagnose(canonical,appearance,kind,isWalking){
 const motion=isWalking?'look':'idle',label=appearance+'-'+kind+'-'+(isWalking?'walking':'overview')+'-'+motion;
 const clocks=await page.evaluate(()=>({unchanged:Object.entries(steadyNativeClocks).every(([k,v])=>v===(k==='performanceNow'?performance.now:window[k])),fixedDate:Date.now(),rafSource:requestAnimationFrame.toString(),performanceSource:performance.now.toString(),timeOrigin:performance.timeOrigin,now:performance.now()}));assert(clocks.unchanged);assert(clocks.rafSource.includes('[native code]'));assert(clocks.performanceSource.includes('[native code]'));assert.equal(clocks.fixedDate,new Date(fixedDate).getTime());
 const pose=isWalking?(canonical.samples.find(s=>s.label===appearance+'-'+kind+'-walking-idle')?.start.player||await page.evaluate(()=>__burbzVillageWalkDebug.state().player)):null;
 if(pose)assert(await page.evaluate(p=>__burbzVillageWalkDebug.place(p),pose));
 const cdp=await context.newCDPSession(page);await cdp.send('Performance.enable');const metrics=await cdp.send('Performance.getMetrics');await cdp.send('Profiler.enable');await cdp.send('Profiler.setSamplingInterval',{interval:1000});await cdp.send('Profiler.start');
 const start=await page.evaluate(()=>{window.steadyTasks=[];window.steadyObserver=new PerformanceObserver(list=>steadyTasks.push(...list.getEntries().map(e=>({at:e.startTime,duration:e.duration}))));steadyObserver.observe({type:'longtask'});return{at:performance.now()};});
 let warmed={at:start.at,revolution:null,tasks:[]};
 if(isWalking){await page.keyboard.down('ArrowRight');try{await page.waitForFunction(yaw=>Math.abs(__burbzVillageWalkDebug.state().player.yaw-yaw)>=Math.PI*2,pose.yaw,{polling:100,timeout:45000});}finally{await page.keyboard.up('ArrowRight');}warmed=await page.evaluate(yaw=>({at:performance.now(),revolution:Math.abs(__burbzVillageWalkDebug.state().player.yaw-yaw),tasks:steadyTasks.slice()}),pose.yaw);assert(warmed.revolution>=Math.PI*2);assert(await page.evaluate(p=>__burbzVillageWalkDebug.place(p),pose));const frames=await page.evaluate(()=>__burbzVillageWalkDebug.state().frames);await page.waitForFunction(n=>__burbzVillageWalkDebug.state().frames>n+3,frames);assert.deepEqual((await snapshot(kind,true)).player,pose,'Restores the exact initial player pose');}
 const measuredStart=await page.evaluate(()=>performance.now());await sample(kind,appearance,isWalking,motion);const measuredEnd=await page.evaluate(()=>performance.now());await page.waitForTimeout(150);
 const tasks=await page.evaluate(()=>{steadyObserver.disconnect();return steadyTasks;});const {profile}=await cdp.send('Profiler.stop');await cdp.send('Profiler.disable');await cdp.detach();const profileFile=label+'.cpuprofile';fs.writeFileSync(path.join(out,profileFile),JSON.stringify(profile));
 const row={label,appearance,kind,isWalking,pose,clocks,cpuProfileClock:{startTimeMicroseconds:profile.startTime,endTimeMicroseconds:profile.endTime,performanceMetrics:metrics.metrics.filter(m=>['Timestamp','NavigationStart'].includes(m.name))},warmup:{start:start.at,end:warmed.at,revolution:warmed.revolution,longTasks:warmed.tasks},measurement:{start:measuredStart,end:measuredEnd,longTasks:tasks.filter(t=>t.at+t.duration>=measuredStart&&t.at<=measuredEnd)},allLongTasks:tasks,cpuProfile:profileFile,cpuProfileSha256:hash(JSON.stringify(profile))};report.diagnostics.push(row);save();console.log('NATIVE CLOCK',label,'full revolution',row.warmup.revolution,'measurement long tasks',JSON.stringify(row.measurement.longTasks));
}
(async()=>{
 const canonical=JSON.parse(fs.readFileSync(baselineFile,'utf8'));assert(canonical.complete);
 report.protocol.steadyDiagnostic='Native performance.now, requestAnimationFrame and all timers are unchanged; only civil Date is fixed. Each appearance measures one full-revolution-warmed village look and one town overview; v385 adds one full-revolution-warmed town look. Ten total samples across both versions. CPU profiles and real longtasks include delayed tail entries.';report.protocol.views=['village-walking','town-overview',...(baseline?[]:['town-walking'])];report.protocol.motions='One native look sample after a full camera revolution per walking view, one idle town overview sample, for each appearance.';report.fpsSuitableForThroughput=true;report.clockControlledPredecessor='Original settlement-performance reports used Playwright fake RAF/performance and cannot support throughput claims. Those raw reports and the first clock-controlled warmup profiles remain retained.';report.diagnostics=[];
 await new Promise((resolve,reject)=>{server.once('error',reject);server.listen(port,'127.0.0.1',resolve);});browser=await chromium.launch({headless:true,executablePath:'/root/.cache/ms-playwright/chromium-1228/chrome-linux64/chrome',args:['--no-sandbox','--use-angle=swiftshader']});browser.on('disconnected',()=>{report.browserProcessEvents||=[];report.browserProcessEvents.push({event:'disconnected',at:new Date().toISOString(),expected:!!report.closing});save();});
 for(const appearance of ['normal','comic']){await setup(appearance);await overview('village');await walking('village',appearance);await diagnose(canonical,appearance,'village',true);await touch('.vw-exit');await overview('town');await diagnose(canonical,appearance,'town',false);if(!baseline){await walking('town',appearance);await diagnose(canonical,appearance,'town',true);await touch('.vw-exit');}await context.close();context=null;page=null;}
 for(const [file,digest]of Object.entries(report.hashes)){assert.equal(report.servedVersions[file].length,1);assert.equal(hash(fs.readFileSync(path.join(root,file))),digest);}assert.deepEqual(report.errors,[]);assert.deepEqual(report.glErrors,[]);assert.equal(report.samples.length,baseline?4:6);report.complete=true;report.checks.push('Native-clock paired samples, exact equal saved fixtures, full-revolution walking warmups and CPU profiles complete.');console.log('PASS native-clock diagnostic:',baseline?'v384':'v385');
})().catch(async error=>{report.failure=error.stack;console.error(error);if(page)await page.screenshot({path:path.join(out,'failure.png')}).catch(()=>{});process.exitCode=1;}).finally(async()=>{report.closing=true;save();if(browser)await browser.close();server.close();});
