...
test('all ten existing cinematic steps have finished local artwork and meaningful alt text',()=>{const intro=c.steps.filter(s=>s.cinematic);assert.equal(intro.length,10);const files=new Set();for(const step of intro){const art=scenes.sceneFor(step.id);assert.ok(art.description.length>30);assert.ok(fs.statSync(path.join(root,art.image)).size>100000);files.add(art.image);}assert.equal(files.size,5);assert.equal(scenes.sceneFor('alderwing-arrival-v395'),null);});
