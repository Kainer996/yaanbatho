...
def test_client_verifies_catalogue_membership_itself():
    ...
    assert "const inGameCatalogue = bird =>" in HTML
    ...
    assert "eligibleCandidates.filter(inGameCatalogue)" in HTML


def test_catalog_match_decision_uses_client_catalogue_not_only_backend_flag():
    # A stale backend `catalogMatched:false` must not block a bird the client
    # can play. Exact sound evidence and the client catalogue are authoritative.
    start = HTML.index("function handleBirdCandidates(")
    end = HTML.index("\nfunction showCatalogUnmatchedRecognition(", start)
    assert "opts.catalogMatched" not in HTML[start:end]
    assert "confirmedSpeciesKeys" in HTML[start:end]
...
def test_unmatched_notification_still_reachable_for_genuinely_uncatalogued_birds():
    ...
    assert "showCatalogUnmatchedRecognition(top, opts.source)" in HTML
