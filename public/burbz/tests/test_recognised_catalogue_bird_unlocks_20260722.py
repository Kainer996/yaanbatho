...
def test_client_verifies_catalogue_membership_itself():
    # The helper that checks the client catalogue must exist and cover the
    # common name, canonical species name, and scientific name forms.
    assert "function speciesInClientCatalogue(bird)" in HTML
...
def test_catalog_match_decision_uses_client_catalogue_not_only_backend_flag():
    ...
    # client can play.
    assert (
        "speciesInClientCatalogue(top) || opts.catalogMatched !== false" in HTML
    )
