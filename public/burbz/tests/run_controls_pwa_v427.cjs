...
(async()=>{try{
 ...
 page=await context.newPage();page.setDefaultTimeout(90000);page.on('pageerror',e=>report.errors.push(e.message));
 ...
 const cache=await page.waitForFunction(async build=>{const keys=await caches.keys(),key=keys.find(k=>k.endsWith(build)),reg=await navigator.serviceWorker.getRegistration();return navigator.serviceWorker.controller&&reg?.active?.state==='activated'&&key?key:false;},build,{timeout:240000});
 const cacheName=await cache.jsonValue();report.cache=cacheName;
 ...
 await page.locator('#tutorialNavigationPause').click();await page.locator('[data-game-route][data-screen="map"]:visible').first().click();await page.locator('#mapQuestShowBtn').click();await page.locator('#walkQuestSheet.open.is-walk-picker').waitFor();
 ...
 assert.match(await page.locator('#walkQuestSheetStatus').innerText(),/offline|location/i);assert.equal(await page.locator('[data-quest-offer]').count(),0);
