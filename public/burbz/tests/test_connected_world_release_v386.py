...
def test_connected_world_installation_and_live_copy_are_atomic():
    ...
    for name in ['BURBZ_ASSETS', 'BURBZ_CORE', 'BURBZ_INSTALL_REQUIRED']:
        ...
        for module in modules:
            assert entries.count(f'./{module}?v={CURRENT_BUILD if module == 'village_walk.js' else CONTINUOUS_PIN if module == 'geographic_world.js' else BUILD if module in CHANGED else WORLD_PIN}') == 1, (name, module)
    for module in modules:
        assert (ROOT / module).is_file()
        assert f'"{module}"' in updater, module
        if module != 'geographic_cache.js':
            assert f'{module}?v={CURRENT_BUILD if module == 'village_walk.js' else CONTINUOUS_PIN if module == 'geographic_world.js' else BUILD if module in CHANGED else WORLD_PIN}' in html, module
