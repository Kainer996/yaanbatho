...
async function invoke(result,ok=true){
  const awards=[],bats=[],toasts=[],button={addEventListener(){},classList:{contains:()=>false},scrollIntoView(){}};const context={$:()=>button,AbortController,setTimeout,clearTimeout,FormData:class{append(){}},getCurrentPositionForPhotoId:async()=>null,
...
function photoHarness(fetcher, position=async()=>null) {
  const elements=Object.fromEntries(['captureBtn','photoIdStatus','photoIdMessage','photoIdRetry','birdCropOverlay'].map(id=>[id,{hidden:true,classList:{contains:()=>false},scrollIntoView(){},addEventListener(type,fn){this[type]=fn;}}]));
  const awards=[],timers=new Map();let serial=0;
  const ctx={$:id=>elements[id],AbortController,FormData:class{append(){}},getCurrentPositionForPhotoId:position,
...
test('failure stays visible after the toast, and retry uses the same photo through the discovery gate',async()=>{
  ...
  const h=photoHarness(async()=>({ok:++calls>1,json:async()=>calls===1?{found:false,message:'Photo identification is unavailable right now.'}:{found:true,accepted:true,policy:'photo-evidence-v350',species:'European Robin',scientificName:'Erithacus rubecula',confidence:.98}}));
...
for(const stage of ['location','upload','body'])test(`${stage} stalls time out and late results cannot award discoveries`,async()=>{
  ...
  release({ok:true,json:async()=>({found:true,accepted:true,policy:'photo-evidence-v350',species:'European Robin',confidence:.98})});
  for(let i=0;i<8;i++)await Promise.resolve();
