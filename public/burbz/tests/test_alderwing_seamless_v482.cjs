...
test('v482 ships together: build marker, cache, three worker lists, loader and consumers',()=>{
 ...
 const LATER=['academy-day-night-v483-20260925','desk-screen-v484-20260925','smooth-sky-plain-plot-v485-20260925'],shipped=[BUILD,...LATER],cache=sw.match(/const BURBZ_CACHE = '([^']+)'/)[1];
 assert(shipped.some(b=>html.includes("const BURBZ_BUILD = 'smooth-sky-plain-plot-v485-20260925';")));assert(cache.includes('-'+BUILD+'-')||cache.endsWith('-'+BUILD));assert(shipped.some(b=>cache.endsWith('-'+b)));
