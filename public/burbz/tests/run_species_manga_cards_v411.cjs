...
(async()=>{try{
 ...
 if(pwa){
  ...
  const build=await run('BURBZ_BUILD');
  const expected=urls.map(u=>sha(fs.readFileSync(path.join(root,u.replace(/^\/burbz\//,'')))));
  let installed;
  // Explicitly observe the actual current worker cache; old caches and an
  // unresolved async wait predicate cannot stand in for a completed install.
  for(let attempt=0;attempt<120;attempt++){
   installed=await page.evaluate(async({urls,build})=>{
    const keys=await caches.keys(),key=keys.find(k=>k.endsWith('-'+build));
    const worker=!!navigator.serviceWorker.controller;
    if(!key)return{keys,worker};
    const cache=await caches.open(key),hashes=[];
    for(const url of urls){const r=await cache.match(url);if(!r){hashes.push(null);continue;}const digest=await crypto.subtle.digest('SHA-256',await r.arrayBuffer());hashes.push(Array.from(new Uint8Array(digest),b=>b.toString(16).padStart(2,'0')).join(''));}
    return{keys,key,worker,hashes};
   },{urls,build});
   if(installed.worker&&JSON.stringify(installed.hashes)===JSON.stringify(expected))break;
   await new Promise(r=>setTimeout(r,1000));
  }
  assert(installed.worker&&installed.key,'Worker did not take control: '+JSON.stringify(installed));assert.deepEqual(installed.hashes,expected);report.offlineCache=installed;
  await run(`closeBirdInfo();closeBirdEquip();if(merlinTutActive)endMerlinTutorial(false);currentBurbzMode='companions';switchScreen('birdex');saveState();`);
  const saved=await run('gameState.flock.map(b=>b.id)');await context.setOffline(true);await page.reload({waitUntil:'domcontentloaded'});
  // The public worker caches uninstrumented HTML. Complete offline proof through
  // actual visible controls and persisted save, without requiring a closure hook.
  await page.locator('.nav-item[data-screen="birdex"]').click();
  await page.locator('#burbzModeTabs [data-mode="companions"]').click();
  assert.deepEqual(await page.evaluate(()=>JSON.parse(localStorage.getItem('burbz_state')).flock.map(b=>b.id)),saved);
