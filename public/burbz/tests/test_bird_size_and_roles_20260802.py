...
ROLE_CORE_PIN = "settlement-tiers-v203-20260803"
RELEASE_PIN = "uk-bird-alias-completion-v216-20260803"
...
def test_role_holders_are_reserved_from_quests_training_and_battles():
    ...
    assert "const flock = getBattleFlock().filter(b => !birdAssignedPost(b.id) && !sleepReadinessForBird(b, Date.now()).sleeping);" in html
