...
(async()=>{try{
 ...
 await page.locator('#settingsBtn').click();assert.equal(await page.locator('#toggleInvertLook').getAttribute('aria-checked'),'true');await page.screenshot({path:path.join(out,'offline-invert-look.png')});await page.locator('#settingsCloseBtn').click();await page.locator('#tutorialNavigationPause').click();await page.locator('[data-game-route][data-screen="map"]:visible').first().click();await page.waitForFunction(()=>document.querySelector('#mapQuestShowBtn')?.dataset.wired==='1',null,{timeout:90000});await page.locator('#mapQuestShowBtn').click();await page.locator('#walkQuestSheet.open.is-walk-picker').waitFor();
