...
const root=path.resolve(__dirname,'..'),out=process.env.EVIDENCE_DIR||'/tmp/burbz-tutorial-pwa-v429',build='hands-on-world-tutorial-v429-20260920';
const files=['alderwing_intro.js','first_person_hud.js','geographic_world.js','player_home.js','village_walk.js','wilderness_combat.js'];
...
(async()=>{try{
 ...
 pass('Actual public worker installed the tested document and all 6 changed modules at exact pinned URLs');
 await page.locator('#settingsBtn').click();await page.locator('#toggleInvertLook').click();assert.equal(await page.locator('#toggleInvertLook').getAttribute('aria-checked'),'true');await page.locator('#settingsCloseBtn').click();
 ...
 const after=await page.evaluate(()=>JSON.parse(localStorage.getItem('burbz_state')));assert.equal(after.settings.invertVerticalLook,true);for(const key of ['playerHome','inventory','player'])assert.deepEqual(after[key],saved[key]);
 await page.locator('#settingsBtn').click();assert.equal(await page.locator('#toggleInvertLook').getAttribute('aria-checked'),'true');await page.screenshot({path:path.join(out,'offline-invert-look.png')});await page.locator('#settingsCloseBtn').click();await page.locator('#merlinTutorialReadingPause:visible,#tutorialNavigationPause:visible').first().click();await page.locator('[data-game-route][data-screen="map"]:visible').first().click();await page.waitForFunction(()=>document.querySelector('#mapQuestShowBtn')?.dataset.wired==='1',null,{timeout:90000});await page.locator('#mapQuestShowBtn').click();await page.locator('#walkQuestSheet.open.is-walk-picker').waitFor();
