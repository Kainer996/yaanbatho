...
test('changed module pins and footstep assets agree across all three worker lists and updater',()=>{
 ...
 for(const list of ['BURBZ_ASSETS','BURBZ_CORE','BURBZ_INSTALL_REQUIRED']){const entries=[...sw.match(new RegExp('const '+list+' = \\[([\\s\\S]*?)\\];'))[1].matchAll(/^\s*['"](\.\/[^'"]+)['"]/gm)].map(m=>m[1]);for(const name of [...modules.map(f=>f+'?v='+(['first_person_hud.js','first_person_hud.css','geographic_world.js','village_walk.js'].includes(f)?'first-person-kit-v1-20260910':rev)),...assets])assert.equal(entries.filter(e=>e==='./'+name).length,1,list+': '+name);}
