...
def test_sound_scan_area_leads_with_the_button_then_shelf_then_data_note():
    ...
    button = html.index('id="scanBtn"')
    note = area.index('id="merlinDataNote"')
    # v378 promotes the existing button above the session tools on Home.
    assert button < html.index('id="soundScanArea"')
    assert stage < shelf < grid < note
...
def test_encounter_banner_only_pops_on_first_hearing_of_a_species():
    ...
    # v494: a photo the player confirmed says so instead of a model percentage.
