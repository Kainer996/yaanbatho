...
function attach(s,options={}){
 ...
 const geos=[],mats=[],textures=[],objects=[];let disposed=false,artReady=false;
 ...
 function update(t,force=false){if(disposed||!force&&t-last<.12)return;last=t;const r=get(),story=q(),activities=core.activities(r);nearest=null;let best=2.4;
  ...
  for(const o of [...objects,...residentObjects]){if(o.group&&!o.group.visible)continue;const d=Math.hypot(o.pos.x-player().x,o.pos.z-player().z);if(d<best&&reachable(o.pos)){best=d;nearest=o;}}
 ...
 update(0,true);
 return {update,closePanel,positions:()=>points,hud(){return hudState?{...hudState}:null;},key(code){if(code==='KeyE'){use();return true;}if(code==='KeyJ'){journal();return true;}return false;},diagnostics(){return{questId:get().questId,artReady,activities:core.activities(get()).map(a=>({id:a.story.id,title:a.story.title,step:a.step,completed:a.completed})),objects:objects.map(o=>({kind:o.kind,id:o.id,visible:o.group.visible,art:o.art,resolved:!!o.resolved,...o.pos,x:o.pos.x+frame.x,y:o.pos.y+(frame.y||0),z:o.pos.z+frame.z})),residents:residentObjects.map(o=>({name:o.name,...o.pos,x:o.pos.x+frame.x,y:o.pos.y+(frame.y||0),z:o.pos.z+frame.z})),nearest:nearest?{kind:nearest.kind,id:nearest.id}:null};},dispose(){closePanel();disposed=true;lifetime.abort();s.abort.signal.removeEventListener('abort',abortFromHost);journalButton.remove();guide.remove();interact.remove();group.removeFromParent();geos.forEach(g=>g.dispose());mats.forEach(m=>m.dispose());textures.forEach(t=>t.dispose());post.remove();panel.remove();}};
}
