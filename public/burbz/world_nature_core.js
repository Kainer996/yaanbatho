...
// Unmapped land: where the map records woods nearby, woods are mapped here too,
// so unmapped ground is open country. Where nothing is recorded, a natural
// mosaic of wood, meadow and scrub keeps Alderwing's authored woodland.
function unmapped(n,openBias){
 const wood=mix(.58,.14,openBias);
 if(n.large<wood)return'woodland';
 if(n.large<wood+mix(.12,.18,openBias)&&n.patch>.55)return'scrub';
 return'meadow';
}
...
function sample({altitude=0,slope=0,cover:mapped=null,wet=0,lat=50,seasons=season(),openBias=0,x=0,z=0,field=0}={}){
 const n={patch:noise(x,z,48,409),large:noise(x,z,190,977),grain:hash(Math.floor(x/5),Math.floor(z/5),853),fine:noise(x,z,9,131),bloom:noise(x,z,23,557)};
 ...
 const autumn=seasons.autumn,winter=seasons.winter,spring=seasons.spring;
 ...
 switch(kind){
  case'wood':case'woodland':{
   ground=lerp3(C.woodFloor,C.litter,autumn*.6+winter*.3);trees=kind==='wood'?.82:.72;shrubs=.18;flora=.35;
  ...
  case'meadow':case'park':case'sport':case'urban':{
   ...
   trees=mowed?.07:.06+(1-alpine)*.05;shrubs=mowed?.03:.07;flora=mowed?.2:.95;
  ...
  case'heath':{
   ...
   ground=lerp3(lerp3(moor,heather,patch*.75),bracken,clamp(slope*1.6-.2,0,.55)*(1-patch));trees=.035;shrubs=.12;flora=.9;
  ...
  case'scrub':{ground=lerp3(C.scrub,grass,.3);trees=.12;shrubs=.5;flora=.7;species.hawthorn=.5;species.birch=.3;species.oak=.2;plants.gorse=.3;plants.bush=.35;plants.grass=.3;plants.fern=.15;plants.flowers=.08;break;}
  case'farmland':case'orchard':{
   ...
   ground=kind==='orchard'?C.orchard:lerp3(pick,grass,winter*.5);trees=kind==='orchard'?.3:.012;shrubs=.01;flora=kind==='orchard'?.4:.15;
  ...
  case'wetland':{ground=lerp3(C.wetland,C.peat,n.patch*.6);trees=.05;shrubs=.05;flora=.8;species.birch=.7;species.dead=.2;species.hawthorn=.1;plants.reeds=.5;plants.grass=.3;plants.flowers=.12;break;}
