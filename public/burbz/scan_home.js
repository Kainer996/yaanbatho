...
(function(root){'use strict';const C=root.BurbzScanHomeCore;let options,targets=new Map(),boundSection;const signatures=new Map();let layoutModel,layoutFrame=0,layoutObserver,layoutKey="";
 ...
 function render(snapshot){if(!options?.visible())return;const m=C.derive(snapshot),focused=document.activeElement?.dataset?.homeAction;targets=new Map();
  ...
  update('desk-stores-list',m.stores,()=>m.stores.slice(0,2).map(s=>`<button type="button" class="desk-gear" data-home-action="store-${escape(s.id)}" aria-label="${escape(s.name)}, ${s.count} in Stores">${picture(s.art,s.icon)}<span>${escape(s.name)}</span><b>×${s.count}</b></button>`).join('')||empty(m.gates.inventory?'No weapons or armour in Stores.':'Stores unlock as you progress.'));
  update('desk-kitchen-list',m.kitchen,()=>m.kitchen.slice(0,2).map(b=>{const full=100-b.hunger;return row('feed-'+b.id,b.name,(b.away?'Away · ':b.label+' · ')+Math.round(full)+'% full',picture(b.art),bar(full,'Fullness',b.level),b.away);}).join('')||empty(m.gates.kitchen?'Your birds are well fed.':'Kitchen unlocks as you progress.'));
  update('desk-training-list',m.training,()=>m.training.slice(0,2).map(s=>row('train-'+s.id,s.name,s.ready?'Finished · collect reward':s.detail+' · '+Math.max(1,Math.ceil(s.remaining/60000))+'m',null,bar(s.ready?100:s.progress,'Training progress'))).join('')||empty(m.gates.training?'No active drills. Choose a bird to train.':'Training unlocks as you progress.'));
  update('desk-hospital-list',m.hospital,()=>m.hospital.slice(0,2).map(b=>row('patient-'+b.id,b.name,Math.round(b.hp)+' / '+Math.round(b.maxHp)+' HP · '+(b.admitted?'Recovering':'Needs care'),picture(b.art),bar(b.hp/b.maxHp*100,'Health'))).join('')||empty(m.gates.hospital?'No injured birds.':'Hospital unlocks as you progress.'));
 ...
 function fitLayout(){
  ...
  const key=[width,main.clientHeight,columns,...ids,featured].join('|');if(key===layoutKey)return;layoutKey=key;
  ...
  for(const id of ids){const el=panelElement(id),h=el.clientHeight;el.classList.toggle('home-panel-detail',h>=188&&el.clientWidth>=180);}
 ...
 let noticesFocus;
 function openNotices(button){const el=document.getElementById('homeBuildingNotices');noticesFocus=button;el.classList.add('show');el.querySelector('button').focus();}
 function closeNotices(restore=true){const el=document.getElementById('homeBuildingNotices');if(!el?.classList.contains('show'))return;el.classList.remove('show');if(restore)noticesFocus?.focus({preventScroll:true});}
