...
(function(root){'use strict';const C=root.BurbzScanHomeCore;let options,targets=new Map(),boundSection;const signatures=new Map();
 ...
 function render(snapshot){if(!options?.visible())return;const model=C.derive(snapshot),focused=document.activeElement?.dataset?.homeAction;targets=new Map();
  ...
  model.builds.forEach(b=>targets.set(b.id,b.target));
  update('scanHomeBuildings',model.builds,()=>model.builds.map(b=>`<button type="button" class="home-building" data-home-action="${escape(b.id)}"><span class="home-building-icon" aria-hidden="true">${escape(b.icon||'🏠')}</span><span><strong>${escape(b.name)}</strong><small>${escape(b.place)} · ${escape(b.detail)}</small></span><span aria-hidden="true">›</span></button>`).join(''));
  const buildingSection=document.getElementById('scanHomeBuildingSection');if(buildingSection)buildingSection.hidden=!model.builds.length;
