...
(function(root){'use strict';const C=root.BurbzScanHomeCore;let options,targets=new Map(),boundSection;const signatures=new Map();let layoutModel,layoutFrame=0,layoutObserver,layoutKey="";
 ...
 const EMPIRE_PAGE={villages:'villages',towns:'towns',regions:'realm'};
 const shortTime=ms=>{const m=Math.max(1,Math.ceil(ms/60000));return m<60?m+'m':Math.floor(m/60)+'h'+(m%60?' '+m%60+'m':'');};
 ...
 function render(snapshot){if(!options?.visible())return;const m=C.derive(snapshot),focused=document.activeElement?.dataset?.homeAction;targets=new Map();
  ...
  // Each column title opens its own Empire page. Regions live with the realm map.
  for(const column of m.empire)targets.set('column-'+column.id,{kind:'empire-page',page:EMPIRE_PAGE[column.id]});
  ...
  update('desk-building-list',m.empire,()=>m.empire.map(column=>`<section class="desk-empire-column" aria-labelledby="desk-empire-${column.id}"><h3 id="desk-empire-${column.id}"><button type="button" class="desk-empire-open" data-home-action="column-${column.id}" aria-label="Open your ${column.title}${column.rows.length?', '+column.rows.length:''}"><span>${column.title}</span>${column.rows.length?`<b>${number(column.rows.length)}</b>`:''}<i aria-hidden="true">›</i></button></h3><div class="desk-empire-holdings" tabindex="0" role="region" aria-label="${column.title}">${column.rows.map(h=>`<button type="button" class="desk-empire-holding is-${h.tone}" data-banner="${column.id==='villages'?'saltmere':''}" data-work="${h.waiting?'✓'+h.waiting:h.buildCount?'🔨'+h.buildCount:h.underway?'⌛'+h.underway:''}" data-home-action="holding-${column.id}-${escape(h.id)}" title="${escape([h.name,h.status,h.governor?.name,h.work,...h.buildNames].filter(Boolean).join(' · '))}" aria-label="${escape(h.name+', '+h.status+(h.governor?', '+h.role+': '+h.governor.name:'')+(h.work?', '+h.work:''))}"><span class="desk-empire-name">${picture(h.governor?.art,h.assigned?'🪶':'!')}<strong>${escape(h.name)}</strong></span><small>${escape(h.status)}</small>${h.work?`<small class="desk-empire-work">${h.buildCount?'🔨 ':h.waiting?'✓ ':''}${escape(h.work)}</small>`:''}</button>`).join('')||empty('None yet')}</div></section>`).join(''));
 ...
 function applyProgression(m,academy,academyOpen){
  const summaries={stores:m.forgeReady?m.forgeReady+' ready to collect':'Weapons, armour & spells',kitchen:m.kitchen.length?m.kitchen.length+' to feed':'All well fed',training:m.training.length?(m.training.some(s=>s.ready)?m.training.filter(s=>s.ready).length+' ready to claim':(m.training.length===1?'1 drill':m.training.length+' drills')+' · next in '+shortTime(Math.min(...m.training.map(s=>s.remaining)))):'No active drills',hospital:m.hospital.length?m.hospital.length+' need care':'All healthy',completed:m.completed.length+' buildings to check',building:m.completed.length?m.completed.length+' completed':m.villageDesk.length+' villages',academy:academyOpen?academy.built+' of '+academy.total+' built'+(academy.ready?' · '+academy.ready+' ready':''):'Not open yet'};
  ...
  // A box that wants the player now wears a warm gold edge.
  const attention={stores:m.forgeReady>0,kitchen:m.kitchen.some(b=>!b.away&&['hungry','urgent'].includes(b.level)),training:m.training.some(s=>s.ready),hospital:m.hospital.some(b=>!b.admitted)};
  for(const id of ['discover','today','stores','kitchen','training','hospital','academy','building']){
   ...
   if(id in attention)el.dataset.homeAttention=String(!!attention[id]);
 ...
 function fitLayout(){
  ...
  // The Empire box is its heading, the column titles and one whole tile, so
  // the first tile is never clipped at the bottom.
  const empirePanel=panelElement('building'),chrome=Math.ceil((empirePanel.querySelector('.desk-panel-heading')?.offsetHeight||44)+(empirePanel.querySelector('.desk-empire-column h3')?.offsetHeight||22)+11);
  // Short landscape shares the height between a whole Empire tile and the
  // room row; on the smallest phones the rooms keep just their titles.
  const landscapeRoom=Math.max(34,Math.min(52,Math.floor(main.clientHeight-fitPadding-(trackCount-1)*fitGap-scanHeight-goalHeight-chrome-44)));
  const otherMin=shortLandscape?landscapeRoom:dense?44:careColumns>1?88:54;
  const spare=Math.floor(main.clientHeight-scanHeight-goalHeight-careRows*otherMin-(trackCount-1)*fitGap-fitPadding-chrome);
  // Short landscape: wide tiles fill the row; elsewhere they stay square.
  const tile=shortLandscape?Math.max(44,Math.min(60,spare)):Math.max(44,Math.min(width<760?104:128,Math.floor((width-36)/3),spare));
  const tracks=[];let row=1;
  main.style.setProperty('--home-columns',columns);main.style.setProperty('--empire-tile-size',tile+'px');
  if(shortLandscape){
   // The Empire, scanner and quest strip run down the left three columns; the
   // Academy tree stands tall on the right; the four rooms share the last row.
   if(fullEmpire){panelElement('building').style.gridArea=`${row} / 1 / ${row+1} / 4`;tracks.push((tile+chrome)+'px');row++;}
   panelElement('discover').style.gridArea=`${row} / 1 / ${row+1} / 4`;tracks.push(scanHeight+'px');row++;
   panelElement('today').style.gridArea=`${row} / 1 / ${row+1} / 4`;tracks.push(goalHeight+'px');row++;
   panelElement('academy').style.gridArea=`1 / 4 / ${row} / 5`;
   others.forEach((id,i)=>{const col=1+i%columns,r=row+Math.floor(i/columns);panelElement(id).style.gridArea=`${r} / ${col} / ${r+1} / ${col+1}`;});
   for(const id of others)panelElement(id).classList.add('home-panel-stacked');
   tracks.push(`minmax(${otherMin}px,1fr)`);
  }else{
   if(fullEmpire){panelElement('building').style.gridArea=`${row} / 1 / ${row+1} / ${columns+1}`;tracks.push((tile+chrome)+'px');row++;}
  ...
  for(const id of ids){
   ...
   el.classList.toggle('home-panel-list',!!el.querySelector('.desk-panel-scroll button')&&h>=minimum&&el.clientWidth>=(id==='academy'?90:140));
  }
  ...
  const required=(fullEmpire?Math.max(44,shortLandscape?44:tile)+chrome:0)+scanHeight+goalHeight+careRows*otherMin+(rows-1)*gap+padding;
  boundSection.dataset.homeConstrained=String(main.clientHeight<required);
  main.dataset.minimumHeight=String(required);
 }
 // A tap anywhere in a box opens that box's screen, like its title does.
 // Rows and tiles inside keep their own actions. The notices sheet is not a box.
 function boxAction(el){
  if(!el?.closest||el.closest('#homeBuildingNotices,.scan-home-tools'))return null;
  const column=el.closest('.desk-empire-column');if(column)return column.querySelector('.desk-empire-open');
  return el.closest('.desk-panel[data-home-panel]')?.querySelector('.desk-panel-heading')||null;
 }
 function onClick(event){if(event.target.closest('[data-build-next]')){const i=buildOptions.findIndex(b=>b.id===buildSelection);buildSelection=buildOptions[(i+1)%buildOptions.length]?.id;renderBuildOption();return;}if(event.target.closest('[data-home-notices-close]')){closeNotices();return;}const button=event.target.closest('[data-home-action]')||boxAction(event.target);if(button&&boundSection.contains(button)){const target=targets.get(button.dataset.homeAction);if(target?.kind==='home-notices'){openNotices(button);return;}if(target){closeNotices(false);options.open(target);}}}
