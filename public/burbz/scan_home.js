...
(function(root){'use strict';const C=root.BurbzScanHomeCore;let options,targets=new Map(),boundSection;const signatures=new Map();
 ...
 function render(snapshot){if(!options?.visible())return;const m=C.derive(snapshot),focused=document.activeElement?.dataset?.homeAction;targets=new Map();
  ...
  update('desk-building-list',m.builds,()=>m.builds.map(b=>row(b.id,b.name,b.place+' · '+b.detail,picture(null,b.icon))).join('')||empty(m.gates.village?'No projects ready for your resources and crews.':'Liberate a village to start building.'));
  for(const [id,rows]of Object.entries({stores:m.stores,kitchen:m.kitchen,training:m.training,hospital:m.hospital,completed:m.completed,building:m.builds})){const label=document.getElementById('desk-'+id+'-count');if(label)label.textContent=rows.length?number(rows.length):'';}
