...
(function(root){'use strict';const C=root.BurbzScanHomeCore;let options,targets=new Map(),boundSection;const signatures=new Map();
 ...
 function render(snapshot){if(!options?.visible())return;const m=C.derive(snapshot),focused=document.activeElement?.dataset?.homeAction;targets=new Map();
  ...
  const brief=m.actions.filter(a=>['next-quest','quests','forge'].includes(a.id));brief.forEach(a=>targets.set(a.id,a.target));if(m.walk){targets.set('walk',m.walk.target);brief.push({id:'walk',title:m.walk.name,detail:'Resume your walk',icon:'map'});}
  update('scanHomeActions',brief,()=>brief.map(a=>`<button type="button" class="home-action" data-home-action="${escape(a.id)}">${icon(a.icon)}<span><strong>${escape(a.title)}</strong><small>${escape(a.detail)}</small></span><span aria-hidden="true">›</span></button>`).join('')||'<p class="desk-empty">Your next adventure starts with a bird.</p>');
  const empty=copy=>`<p class="desk-empty">${escape(copy)}</p>`;
  const slotNames={weapon:'Weapon',armour:'Armour',trinket:'Enhancement',spell:'Spell',potion:'Potion'};
  update('desk-equipment-list',m.equipment,()=>m.equipment.map(i=>`<button type="button" class="desk-kit-slot" data-home-action="equip-${i.slot}" title="${escape(slotNames[i.slot]+': '+(i.name||'Empty'))}" aria-label="${escape(slotNames[i.slot]+': '+(i.name||'Empty')+'. Change equipment')}">${picture(i.art,i.icon||'＋')}<small>${slotNames[i.slot]}</small></button>`).join(''));
  update('desk-stores-list',m.stores,()=>m.stores.map(s=>`<button type="button" class="desk-gear" data-home-action="store-${escape(s.id)}" aria-label="${escape(s.name)}, ${s.count} in Stores">${picture(s.art,s.icon)}<span>${escape(s.name)}</span><b>×${s.count}</b></button>`).join('')||empty(m.gates.inventory?'No weapons or armour in Stores.':'Stores unlock as you progress.'));
  update('desk-kitchen-list',m.kitchen,()=>m.kitchen.map(b=>{const full=100-b.hunger;return row('feed-'+b.id,b.name,(b.away?'Away · ':b.label+' · ')+Math.round(full)+'% full',picture(b.art),bar(full,'Fullness',b.level),b.away);}).join('')||empty(m.gates.kitchen?'Your birds are well fed.':'Kitchen unlocks as you progress.'));
  update('desk-training-list',m.training,()=>m.training.map(s=>row('train-'+s.id,s.name,s.ready?'Finished · collect reward':s.detail+' · '+Math.max(1,Math.ceil(s.remaining/60000))+'m',null,bar(s.ready?100:s.progress,'Training progress'))).join('')||empty(m.gates.training?'No active drills. Choose a bird to train.':'Training unlocks as you progress.'));
  update('desk-hospital-list',m.hospital,()=>m.hospital.map(b=>row('patient-'+b.id,b.name,Math.round(b.hp)+' / '+Math.round(b.maxHp)+' HP · '+(b.admitted?'Recovering':'Needs care'),picture(b.art),bar(b.hp/b.maxHp*100,'Health'))).join('')||empty(m.gates.hospital?'No injured birds.':'Hospital unlocks as you progress.'));
  ...
  update('desk-building-list',{villages:m.villageDesk,builds:m.builds},()=>m.villageDesk.map(v=>`<section class="desk-village"><button type="button" class="desk-village-head" data-home-action="village-${v.seed}">${icon('village')}<span><strong>${escape(v.name)}</strong><small class="desk-village-badge ${v.waiting||v.finished?'ready':''}">${escape(villageBadge(v))}</small></span></button>${v.waiting?`<p class="desk-village-note">${escape(v.pendingNames.join(', '))} · Visit to open</p>`:v.underway?`<p class="desk-village-note">${escape(v.underwayNames.join(', '))}</p>`:''}${v.builds.map(b=>row(b.id,b.name,b.detail,picture(null,b.icon))).join('')}</section>`).join('')||empty('Liberate a village to start building.'));
  ...
  if(focused&&document.activeElement?.dataset?.homeAction!==focused)Array.from(boundSection.querySelectorAll('[data-home-action]')).find(el=>el.dataset.homeAction===focused)?.focus({preventScroll:true});return m;
 }
