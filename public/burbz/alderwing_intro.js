...
(function(root){'use strict';
 ...
 let api,view=null,box=null,resizeTimer=0,practice=null,pending=null,saveFailed=false,tilting=false;
 ...
 function clear(){box?.remove();box=null;view=null;practice=null;pending=null;saveFailed=false;tilting=false;clearTimeout(resizeTimer);}
 ...
 function advance(){if(!visible())return;if(pending){if(pending.from===phase()&&pending.host===view.host&&pending.kind===view.kind)commit(pending.to);return;}const p=phase();if(matchingView()&&next[p]&&ready()&&(p!=='landscape'||oriented(p)))commit(next[p]);}
 function render(){
  ...
  // Portrait is the last lesson. It asks for a tilt but never locks anyone in.
  const button=box.querySelector('button');button.hidden=!pending&&(recovery||!next[p]);tilting=rotate&&!oriented(p);button.disabled=!pending&&((p==='landscape'&&tilting)||!ready());button.textContent=pending?'Retry save':p==='portrait'?'Continue tutorial':'Next';
  box.querySelector('small').textContent=saveFailed?'Progress could not be saved. Please try again.':recovery?'Your lesson is saved; no practice has been skipped.':practiceSteps.includes(p)?progress():p==='door'?'Walk to the door and choose Step outside.':p==='return'?'Use the shelter door.':p==='chair'?'Walk to the chair and choose Sit at the command desk.':tilting?(p==='portrait'?'Turn your phone upright, or tap Continue tutorial.':'Rotate your phone when you’re ready.'):rotate?(phone()&&!fold?'Already in '+p+'.':'Continue when you’re ready.'):'';
 }
 ...
 root.addEventListener('resize',()=>{practice?.inputs.clear();clearTimeout(resizeTimer);resizeTimer=setTimeout(()=>{if(!active()||!view)return;const p=phase(),wasWaiting=tilting;render();if(wasWaiting&&visible()&&matchingView()&&!pending&&['landscape','portrait'].includes(p)&&oriented(p))commit(next[p]);},160);});
