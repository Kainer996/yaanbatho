...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.BurbzPeepNeeds=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function sanitize(raw){
    ...
    return {...pos,thirst:clamp(raw.thirst,0,100),energy:clamp(raw.energy,0,100),fun:clamp(raw.fun,0,100),hunger:clamp(raw.hunger,0,100),
      at:clamp(raw.at,0,9e15),yaw:clamp(raw.yaw,-Math.PI*2,Math.PI*2),action:['home','work','green','well','fun','food','sleep','outside','travel','blocked'].includes(raw.action)?raw.action:'home',
  ...
  function initial(person,env,now){
    ...
    return {...start.point,thirst:20+seed%40,energy:65+seed%30,fun:45+seed%40,hunger:15+seed%35,at:now,yaw:0,action,target:action,targetKey:start.key,sceneKey:env.sceneKey||'',elapsed:seed%18,distance:0,cycle:seed%3,route:[]};
  }
  function schedule(person,hour,env){
    let h=Number(hour)||0,hsh=0;for(const c of String(person.id))hsh=(hsh*31+c.charCodeAt(0))>>>0;
    const shift=(hsh%3)*.25;h=(h-shift+24)%24;
    const want=h<6||h>=21?'home':h<8?'food':h<12?'work':h<13?'food':h<17?'work':h<19?'green':'fun';
    return env.destinations[want]?want:want==='home'?'outside':'green';
  }
  function advance(person,env,now,hour){
    ...
    const eating=m.action==='food'&&atTarget('food');
    m.hunger=clamp((Number(m.hunger)||0)+dt*(eating?-9:.12),0,100);
    ...
    const urgent=(night||m.energy<=25)?(dest.home?'home':'outside'):m.thirst>=65&&dest.well?'well':m.hunger>=65&&dest.food?'food':m.fun<=35&&dest.fun?'fun':null;
    const sameNeed=(urgent==='home'&&m.action==='sleep')||urgent===m.action||(m.action==='travel'&&urgent===m.target);
    const done=m.action==='home'&&m.elapsed>=18||m.action==='work'&&(m.elapsed>=60||schedule(person,hour,env)!=='work')||m.action==='green'&&m.elapsed>=22||m.action==='food'&&(m.hunger<=10||!eating)||m.action==='well'&&(m.thirst<=10||!drinking)||m.action==='fun'&&(m.fun>=90||!playing)||m.action==='sleep'&&m.energy>=90&&!night||m.action==='outside'&&m.energy>=55&&!night||m.action==='blocked'&&m.elapsed>=15;
    ...
    const busy=['well','fun','food','sleep','outside'].includes(m.action)&&!done;
    if(!arrived&&((urgent&&!sameNeed&&!busy&&m.action!=='blocked')||done)){
      let goal=urgent;
      if(!goal){m.cycle=(m.cycle+1)%3;goal=schedule(person,hour,env);}
      if(goal==='outside'){m.action='outside';m.elapsed=0;m.route=[];}
      else{
        const target=dest[goal],route=target&&env.routeFrom(m,target);
        if(goal===m.action&&atTarget(goal)){m.elapsed=0;}
        else if(route&&route.length){m.route=route;m.distance=0;m.elapsed=0;m.target=goal;m.targetKey=target.key;m.action='travel';}
    ...
    const labels={home:'At home',work:'Working',food:'Having a meal',green:'Mooching on the green',well:'Drinking at the well',fun:'Enjoying a little entertainment',sleep:'Sleeping at home',outside:'Resting outside · needs a home',blocked:'Waiting for a clear path'};
    const trips={home:night||m.energy<=30?'Heading home to sleep':'Heading home',work:'Walking to work',food:'Going for a meal',well:'Going to the well',fun:'Looking for entertainment',green:'Mooching to the green'};
    ...
    return {x:m.x,z:m.z,yaw:m.yaw,activity:m.action==='travel'?trips[m.target]:labels[m.action],moving:m.action==='travel',inside:!!atTarget(m.action==='sleep'?'home':m.action)&&['home','sleep','work','food','fun'].includes(m.action),room:dest[m.action==='sleep'?'home':m.action]?.target||null,stride:now/1000*6,mood,need,thirst:Math.round(m.thirst),energy:Math.round(m.energy),fun:Math.round(m.fun)};
  }
  return {sanitize,initial,advance,schedule};
});
