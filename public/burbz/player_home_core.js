...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.BurbzPlayerHomeCore=api;})(globalThis,function(){'use strict';
 ...
 function initial(veteran=false){return{version:3,outlook:true,anchor:null,rooms:{},trees:{},intro:veteran?'done':'welcome',arrival:veteran?'done':'home',tier:0,owned:{bench:1,flowers:1,rug:1},placed:[],finds:[],nextId:1};}
 ...
 if(inside){const main=p.area==='room',halfX=main?4.2:3.7,halfZ=main?4.6:3.7;if(Math.abs(p.x)+b.w/2>halfX||Math.abs(p.z)+b.d/2>halfZ)return{ok:false,error:'Keep furniture inside the walls.'};const fixed=main?(s.tier?FIXED:FIXED.slice(0,1)):extraFixed,path=main?{x:0,z:1.5,w:1.5,d:6.5}:{x:0,z:.6,w:1.5,d:5.6};if(!item.flat&&(fixed.some(f=>overlap(b,f))||overlap(b,path)||main&&Object.keys(ROOMS).some(id=>s.rooms[id]&&overlap(b,roomPath(id)))))return{ok:false,error:'Leave a clear path to the desk, room doors and reading tables.'};}
 else if(Math.hypot(p.x,p.z)+Math.hypot(b.w,b.d)/2>YARD.decorate||overlap(b,{x:0,z:0,w:5.5,d:4.5})||overlap(b,{x:0,z:5,w:2,d:6})||FINDS.filter(f=>f.area==='yard').some(f=>overlap(b,{...f,w:1.3,d:1.3}))||visibleTrees(s).some(t=>overlap(b,{...t,w:t.r*2+.5,d:t.r*2+.5})))return{ok:false,error:'Keep the house, path, tree roots and little discoveries clear.'};
 ...
 function world(home,area,now=Date.now(),options={}){const s=normalize(home),inside=indoor(area),main=area==='room',fixed=inside?(main?(s.tier?FIXED:FIXED.slice(0,1)):extraFixed):[{x:0,z:0,w:5.5,d:4.5}];return{day:dayKey(now),radius:inside?7:options.connected?Infinity:15*YARD_SCALE,segments:[],height:()=>0,surface:(x,z)=>inside?'wood':Math.abs(x)<1&&z>=2&&z<=YARD.walk-.3?'stone':'ground',spawn:()=>inside?{x:0,y:0,z:main?3.8:2.8,yaw:0,pitch:-.02}:{x:0,y:0,z:4,yaw:Math.PI,pitch:0},allowed(x,z){if(!Number.isFinite(x+z)||inside&&!roomOpen(s,area))return false;if(inside?(Math.abs(x)>(main?4.2:3.7)||Math.abs(z)>(main?4.6:3.7)):!options.connected&&Math.hypot(x,z)>YARD.walk)return false;const person={x,z,w:.54,d:.54};return !fixed.some(f=>overlap(person,f,0))&&!s.placed.some(p=>p.area===area&&!ITEMS[p.item].flat&&overlap(person,bounds(p),0))&&(inside||!s.outlook||(((x+10)/6)**2+((z-24)/4)**2>1&&!overlap(person,{x:7,z:18,w:4.8,d:3.2},0)&&!overlap(person,{x:9,z:20,w:2.6,d:1.8},0)))&&(inside||!visibleTrees(s).some(t=>treeState(s,t.id,now)<3&&Math.hypot(x-t.x,z-t.z)<t.r+.27));}};}
 return{ITEMS,TIERS,ROOMS,FINDS,TREES,visibleTrees,FIXED,YARD,YARD_SCALE,MAX_PER_SPACE,MAX_PLACED,MAX_MAP_LAT,indoor,roomOpen,dayKey,treeState,initial,normalize,normalizeAnchor,available,validPlacement,propose,safePosition,world};
});
