...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.BurbzPlayerHomeCore=api;})(globalThis,function(){'use strict';
 ...
 s.farm=normalizeFarm(v.farm,s);return s;}
 ...
 const b={...p,w:2,d:2};if(overlap(b,houseFootprint(home))||overlap(b,{x:0,z:5,w:2,d:6})||FINDS.filter(f=>f.area==='yard').some(f=>overlap(b,{...f,w:1.3,d:1.3}))||visibleTrees(home).some(t=>overlap(b,{...t,w:t.r*2+.5,d:t.r*2+.5}))||home.outlook&&(overlap(b,{x:7,z:18,w:4.8,d:3.2})||overlap(b,{x:9,z:20,w:2.6,d:1.8})))return{ok:false,error:'Leave the house, paths, trees and discoveries clear.'};
