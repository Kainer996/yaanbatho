...
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;root.BurbzPlayerHomeCore=api;})(globalThis,function(){'use strict';
 ...
 function initial(veteran=false){return{version:3,outlook:true,anchor:null,rooms:{},trees:{},intro:veteran?'done':'welcome',arrival:veteran?'done':'home',tier:0,owned:{bench:1,flowers:1,rug:1},placed:[],finds:[],nextId:1,academyTree:null,farm:{version:1,plots:[],seeds:{},nextId:1,harvests:0}};}
 ...
 function normalizeFarm(raw,home){const farm={version:1,plots:[],seeds:{},nextId:Math.max(1,count(raw?.nextId)),harvests:count(raw?.harvests)};for(const id of Object.keys(CROPS))farm.seeds[id]=Math.min(999,count(raw?.seeds?.[id]));
