...
`+T.ShaderChunk.lights_fragment_begin.split('vDirectionalShadowCoord[ i ] ) : 1.0;').join('vDirectionalShadowCoord[ i ] )*(1.0-cwShadowFade)+cwShadowFade : 1.0;'));};material.customProgramCacheKey=function(){return key.call(this)+':continuous-horizon-v474';};material.needsUpdate=true;}
 ...
 // They bend onto the distant land at the edge of the shown square just as
 // the ground does, so a lake runs on into its distant water with no step.
 function waterSurface(options){const m=groundMaterial.clone();styleFog(m);styleGround(m);root.BurbzShoreWater.style(m,waterTime,options);return m;}
 // One water clock for every water surface, near and far.
 const waterTime=root.BurbzShoreWater.sky.time;
 const waterMaterial=waterSurface(),seaMaterial=waterSurface({sea:1});
 ...
 // Distant water is the same deep water as the near lakes, so they meet unseen.
 const farWater=[.018,.05,.07];
 ...
 const streamMaterial=root.BurbzWorldWater.streamMaterial(T,{time:waterTime,style:styleFog}),fallMaterial=root.BurbzWorldWater.fallMaterial(T,{time:waterTime,style:styleFog}),spray=root.BurbzWorldWater.createSpray(T,scene,{time:waterTime});
 // Near running water, flecks of foam and leaves ride the current and the
 // water is heard: a brook babbles, a river rushes, a fall roars.
 const drift=root.BurbzWorldWater.createDrift(T,scene,{style:styleFog}),waterSound=root.BurbzWorldWater.createSound({allowed:()=>!s.room&&root.BurbzCalmAudio?.birds?.()===true});let nearWater=null,heardWater=null,lastWater=-Infinity,lastWaterFrame=null;
 ...
 function readFeatures(){if(closed||!loaded)return;const changedWater=[];const sources=[...new Set((map.getStyle()?.layers||[]).filter(l=>['landcover','landuse','water','waterway','transportation'].includes(l['source-layer'])).map(l=>l.source))];
  ...
  waterways=root.BurbzWorldWaterCore.corridors(streamLines().filter(r=>!r.corridor).map(r=>({kind:r.waterway.kind,intermittent:r.waterway.intermittent,line:r.line})),160,s.player).filter(c=>!corridors.some(a=>a.kind==='river'&&k.corridorContains(a,c.x,c.z,4)));
 ...
 const streamCache=new Map(),authoredLines=new Map();
 function streamLines(){const rows=[];for(const e of features.values())if(e.kind==='stream'&&!e.waterway?.tunnel)for(const [i,line] of e.lines.entries())if(line.length>1&&line.every(Boolean))rows.push({id:e.id+'#'+i,waterway:e.waterway,line});
  for(const r of riverCorridors()){const line=authoredLine(r);if(line)rows.push({id:r.id,waterway:{kind:'river',intermittent:false},line,corridor:r.c});}
  return rows;}
 // A settlement's own river, and each neighbour's, runs on as a stream line
 // through the continuous world, so it flows, falls and sounds like a real
 // one. Neighbours' flat river planes give way to it.
 function riverCorridors(){const rows=corridors.flatMap((c,i)=>c.kind==='river'?[{id:'authored:host#'+i,c}]:[]);
  for(const p of places.values())for(const [i,c] of (p.content.surface?.corridors||[]).entries())if(c.kind==='river'){if(c.object)c.object.visible=false;rows.push({id:'authored:'+p.record.id+'#'+i,c:{...c,x:c.x+p.x,z:c.z+p.z}});}
  return rows;}
 function authoredLine({id,c}){if(authoredLines.has(id))return authoredLines.get(id);if(datum===null)return null;
  const a={x:c.x-c.ux*c.end,z:c.z-c.uz*c.end},b={x:c.x+c.ux*c.end,z:c.z+c.uz*c.end},ha=joined(a.x,a.z),hb=joined(b.x,b.z),n=Math.max(2,Math.ceil(c.end/4)),line=[];
  if(!Number.isFinite(ha)||!Number.isFinite(hb))return null;for(let j=0;j<=n;j++)line.push({x:a.x+(b.x-a.x)*j/n,z:a.z+(b.z-a.z)*j/n});if(hb>ha)line.reverse();authoredLines.set(id,line);return line;}
 // The nearest mapped centre-line pieces to a chunk, for its river water.
 function flowSegments(cell){const cx=cell.x+k.CHUNK/2,cz=cell.z+k.CHUNK/2,rows=[];
  for(const r of streamLines()){const kind=r.waterway?.kind;if(!['river','canal','stream'].includes(kind))continue;
   for(let i=1;i<r.line.length;i++){const a=r.line[i-1],b=r.line[i];if(!a||!b)continue;const d=Math.sqrt(root.BurbzVillageWalkCore.distance2(cx,cz,a,b));if(d<120)rows.push({ax:a.x,az:a.z,bx:b.x,bz:b.z,d,speed:kind==='canal'?.25:kind==='stream'?1:.8});}}
  return rows.sort((p,q)=>p.d-q.d).slice(0,root.BurbzShoreWater.FLOWS);}
 // The nearest river mapped as an area: its bank, or the viewer's own spot
 // when on it. Its centre line alone would say the water is further away.
 function riverAreaNear(x,z,reach){let best=null;for(const e of features.values()){if(e.kind!=='water'||!e.rings||!(e.river||/\briver\b/.test(e.waterClass||'')))continue;const b=e.bounds;if(x<b.x0-reach||x>b.x1+reach||z<b.z0-reach||z>b.z1+reach)continue;
   const on=inside(e,x,z);for(const ring of e.rings)for(let i=1;i<ring.length;i++){const a=ring[i-1],c=ring[i],ex=c.x-a.x,ez=c.z-a.z,t=Math.max(0,Math.min(1,((x-a.x)*ex+(z-a.z)*ez)/(ex*ex+ez*ez||1))),px=a.x+ex*t,pz=a.z+ez*t,d=on?0:Math.hypot(x-px,z-pz);if(d<reach&&(!best||d<best.d))best={d,x:on?x:px,z:on?z:pz};}}
  return best;}
 // Shaped streams within reach of a point, for the sound and the flecks.
 function nearbyStreams(x,z,reach){const rows=[];for(const r of streamLines()){const p=streamCache.get(r.id),b=p?.bounds;if(b&&b.x1>=x-reach&&b.x0<=x+reach&&b.z1>=z-reach&&b.z0<=z+reach)rows.push(p);}return rows;}
 function preparedStreams(cell){const rows=[],pad=12;for(const r of streamLines()){let p=streamCache.get(r.id);if(!p){if(datum===null)continue;const c=r.corridor;p=root.BurbzWorldWaterCore.prepare(r.line,r.waterway.kind,r.waterway.intermittent,(x,z)=>joined(x,z),c?q=>k.corridorWidth(c,(q.x-c.x)*c.ux+(q.z-c.z)*c.uz)*.95:null);if(c)p.width=c.width*.95;streamCache.set(r.id,p);}
 ...
 function makeChunk(cell){const t0=performance.now(),mark={};const data=k.groundMesh(cell,joined,actualRadius);if(!data)return false;
  ...
  mark.mesh=performance.now();const flowing=root.BurbzWorldWaterCore.build(cell,k.CHUNK,preparedStreams(cell),sample,{wet:(x,z)=>lakeIn(ctx,x,z)});mark.streams=performance.now();
  ...
  // A river mapped as an area flows the way its mapped centre lines run.
  const flows=mask?.flowing?flowSegments(cell):null,flowSpeed=flows?.length?Math.max(...flows.map(f=>f.speed)):.8;
  if(mask?.fullWater==='river'&&flows?.length){material=waterSurface({flows,flowSpeed});shoreMaterials.push(material);}
  else if(mask?.fullWater)material=mask.fullWater==='sea'?seaMaterial:waterMaterial;
  else if(mask){shoreTexture=new T.CanvasTexture(mask.canvas);shoreTexture.flipY=false;shoreTexture.generateMipmaps=false;shoreTexture.minFilter=shoreTexture.magFilter=T.LinearFilter;material=groundMaterial.clone();styleFog(material);styleGround(material);root.BurbzShoreWater.style(material,waterTime,{...mask,texture:shoreTexture,flows,flowSpeed});shoreMaterials.push(material);}
  ...
  const stream=root.BurbzWorldWater.streamMesh(T,flowing,streamMaterial),fall=root.BurbzWorldWater.fallMesh(T,flowing.curtain,fallMaterial);if(stream)group.add(stream);if(fall)group.add(fall);chunk.spray={mist:flowing.mist,splash:flowing.splash};chunk.streamTriangles=flowing.indices.length/3;
  for(const corridor of corridors)if(corridor.kind!=='river')for(const bank of corridor.bankMaterial?[false,true]:[false]){const data=k.ribbonMesh(cell,corridor,(x,z)=>k.meshHeight(cell,chunk.data,x,z),bank);if(!data?.indices.length)continue;const geo=new T.BufferGeometry();geo.setAttribute('position',new T.Float32BufferAttribute(data.positions,3));geo.setAttribute('uv',new T.Float32BufferAttribute(data.uv,2));geo.setIndex(data.indices);geo.computeVertexNormals();const ribbon=new T.Mesh(geo,bank?corridor.bankMaterial:corridor.material);ribbon.receiveShadow=true;group.add(ribbon);}
 ...
 function update(time){homeUI.update();craftRuntime?.update(time);campRuntime?.update(time);syncControls();const homePlace=places.get('home');build.hidden=closed||!!s.room||s.uiBusy||s.player.mode!=='walk'||!opts.openBuild||!homePlace||opts.getHome?.()?.tier<1||Math.hypot(s.player.x-homePlace.x,s.player.z-homePlace.z)>root.BurbzPlayerHomeCore.YARD.walk;if(s.room)waterSound?.update(null,.2);if(closed||s.room)return;skyDriver?.update(time,weatherAt(time));const reducedMotion=matchMedia('(prefers-reduced-motion: reduce)').matches;waterTime.value=reducedMotion?0:time;shadows();natureEye.value.set(s.player.x,s.player.y+1.38,s.player.z);groundClock.value=time;registerNature(time);spray.update(s.source.renderer,scene.fog);
  // Running water near the viewer: flecks drift on it and it is heard.
  if(time-lastWater>.2){lastWater=time;nearWater=root.BurbzWorldWaterCore.nearest(nearbyStreams(s.player.x,s.player.z,90),s.player.x,s.player.z,90);
   // A broad river area is heard from its own bank as a wide river.
   const area=riverAreaNear(s.player.x,s.player.z,60),heard=area&&(!nearWater.water||area.d<nearWater.water.d)?{...nearWater,water:{d:area.d,x:area.x,z:area.z,y:world.height(area.x,area.z),width:14,kind:'river',steep:.15,speed:.8}}:nearWater;
   heardWater=heard;waterSound?.update(root.BurbzWorldWaterCore.listen(heard,{x:s.player.x,y:s.player.y+1.38,z:s.player.z,yaw:s.player.yaw}),.2);}
  drift.update(reducedMotion||lastWaterFrame===null?0:time-lastWaterFrame,nearWater,s.player,(x,z)=>world.height(x,z));lastWaterFrame=time;
  // Water mirrors the real sky: the sun or moon, and the colour overhead.
  const waterSky=root.BurbzShoreWater.sky;waterSky.rain.value=Math.max(0,Math.min(1,weatherNow?.rain||0));if(skyDriver&&waterSky.sun.value){waterSky.sun.value.copy(skyDriver.direction());const top=skyDriver.group?.children[0]?.material?.uniforms?.top?.value;if(top)waterSky.zenith.value.copy(top);}for(const object of sky){const p=initialSky.get(object);object.position.set(p.x+s.player.x,p.y+s.player.y,p.z+s.player.z);}
  ...
  for(const p of places.values()){
    ...
    if(near){if(group.parent!==scene){scene.add(group);group.updateMatrixWorld(true);}group.visible=true;p.content.update?.(time,{listener:{x:s.player.x-p.x,y:s.player.y+1.38-p.base,z:s.player.z-p.z,yaw:s.player.yaw},sound:!!root.BurbzCalmAudio?.birds?.(),reduced:reducedMotion});p.cull?.update(time,s.player,DETAIL);}
