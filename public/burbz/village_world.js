...
async function attach(s,opts){
 ...
 const metrics={created:0,retired:0,maxChunks:0,streamMs:[],transitions:0,loadingFrames:0,loadingMs:0,movingFrames:0,movingMs:0,distance:0};let zone='settlement',campRuntime=null,lastExplored=null;
 ...
 function allowed3(x,y,z,requireCoverage=true){if(campRuntime?.blocked(x,y,z)||s.combat?.blocked?.(x,y,z))return false;const h=height(x,z);if(h===null||(requireCoverage&&!visibleCoverage(x,z)))return false;for(const b of solidBoxes)if(x>b.min.x-.28&&x<b.max.x+.28&&z>b.min.z-.28&&z<b.max.z+.28&&y<b.max.y+.3&&y>b.min.y-.3)return false;
