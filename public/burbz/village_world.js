...
async function attach(s,opts){
 ...
 function syncControls(){const craft=craftRuntime?.sync();if(craft){wing.textContent=craft.label;wing.setAttribute('aria-disabled',String(!craft.enabled));}wing.hidden=!!s.room||s.uiBusy;rise.hidden=descend.hidden=wing.hidden||s.player.mode!=='fly';if(s.room)visit.hidden=true;}
