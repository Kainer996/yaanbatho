...
async function attach(s,opts){
 ...
 function refreshPlaces(){if(!opts.records||datum===null)return;const at=C().unproject(origin,{...s.player});if(!at||placeCentre&&C().distance(at,placeCentre)<32)return;placeCentre=at;
  ...
  const wanted=records.map(record=>({record,p:C().project(origin,record)})).filter(r=>r.p).map(r=>({...r,d:Math.hypot(r.p.x-s.player.x,r.p.z-s.player.z)})).filter(r=>r.d<750).sort((a,b)=>a.d-b.d).slice(0,6),ids=new Set(wanted.map(r=>r.record.id));
  ...
  for(const row of wanted){if(places.size+preparing.size>=12||places.has(row.record.id)||preparing.has(row.record.id)||row.record.kind!=='home'&&Math.hypot(row.p.x,row.p.z)<actualRadius+6)continue;const value=raw(row.p.x,row.p.z);if(value===null){placeCentre=null;continue;}
   const pendingPlace={...row.p,record:row.record,radius:row.record.kind==='home'?32:row.record.tier==='village'?100:250,abort:new AbortController()};preparing.set(row.record.id,pendingPlace);
   ...
   placeChain=placeChain.catch(()=>{}).then(async()=>{
    ...
    const content=row.record.kind==='home'?opts.createYard?.(T,home):await opts.createSettlement?.(T,row.record,{signal:pendingPlace.abort.signal});
 ...
 function selectDiscoveries(){if(s.room||s.uiBusy)return;const place=Math.hypot(s.player.x,s.player.z)<actualRadius?originDiscovery:inPlace(s.player.x,s.player.z);if(!place||activeDiscovery===place.record.id)return;const d=place.content.discovery;if(!d?.api){s.discoveries?.dispose();s.discoveries={update(){},dispose(){},hud(){return null;},closePanel(){return false;}};activeDiscovery=place.record.id;s.root.querySelector('.vw-title strong').textContent=place.record.name;return;}s.discoveries?.dispose();s.discoveries=root.BurbzVillageDiscoveries.attach(s,{...d,scene:place.content.group,movers:place.content.movers,frame:{x:place.x,y:place.base,z:place.z},preserveHeading:true});activeDiscovery=place.record.id;s.root.querySelector('.vw-title strong').textContent=place.record.name||s.options.name;}
 ...
 function update(time){homeUI.update();syncControls();if(closed||s.room)return;skyDriver?.update(time);shadows();for(const object of sky){const p=initialSky.get(object);object.position.set(p.x+s.player.x,p.y+s.player.y,p.z+s.player.z);}
