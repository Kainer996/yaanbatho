...
function attach(s,{adapter,safeAt,ground,allowed,clear,walkClear,notice}){
 ...
 function encounter(pose){const candidates=[...visible.values()].filter(v=>data().camps[v.id]&&!data().camps[v.id].liberated&&Math.hypot(v.point.x-pose.x,v.point.z-pose.z)<75).sort((a,b)=>Math.hypot(a.point.x-pose.x,a.point.z-pose.z)-Math.hypot(b.point.x-pose.x,b.point.z-pose.z));const v=candidates[0];if(!v)return null;const c=data().camps[v.id];return{campId:c.id,guards:v.layout?c.hp.map((hp,i)=>({id:c.id+':guard:'+i,campId:c.id,member:i,enemyKind:c.enemyKinds?.[i]||'chickenz',hp,maxHp:O.HP,position:{...v.layout.positions[i]}})).filter(g=>g.hp>0):[]};}
