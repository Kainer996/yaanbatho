...
function attach(s,{adapter,safeAt,ground,notice}){
 ...
 const probes=[];let lastReason=null;
 ...
 function build(c,p){const g=new T.Group();g.name=c.id;g.position.set(p.x,ground(p.x,p.z),p.z);const covers=[];
 ...
 function suitable(c,p){lastReason=null;if(!p||safeAt(p.x,p.z)!==false)return null;if(api.dark(c)!==true){lastReason='already-revealed';return false;}const h=ground(p.x,p.z);if(!Number.isFinite(h))return false;
  const checks=[[0,0],[-6,-6],[-6,6],[6,-6],[6,6],[-3,-5],[0,-5],[3,-5]];for(const [x,z]of [[-3,-2],[0,3],[3,-2],...props.map(b=>[b.x,b.z])])for(const [dx,dz]of [[0,0],[-.7,-.7],[-.7,.7],[.7,-.7],[.7,.7]])checks.push([x+dx,z+dz]);
  for(const [x,z]of checks)if(safeAt(p.x+x,p.z+z)!==false||!Number.isFinite(ground(p.x+x,p.z+z)))return null;
  for(const [x,z]of checks){const y=ground(p.x+x,p.z+z);if(Math.abs(y-h)>1.3||!s.world.allowed(p.x+x,p.z+z)){lastReason={x,z,slope:Math.abs(y-h),allowed:s.world.allowed(p.x+x,p.z+z)};return false;}}return true;}
 function* scan(){const nav=navigation();if(!nav||s.room||disposed)return;scanCount++;
  for(const v of visible.values()){const p=G.project(nav.origin,v.record);if(Math.hypot(p.x-s.player.x,p.z-s.player.z)>210){remove(v);continue;}v.point=p;v.group.position.x=p.x;v.group.position.z=p.z;const c=data().camps[v.id]||v.record;v.flag.material=c.liberated?materials.friendly:materials.hostile;}
  const saved=data().camps,candidates=O.nearby(nav.pose);for(let c of candidates){if(visible.has(c.id)||visible.size>=O.MAX_VISIBLE||rejected.has(c.id))continue;let p=G.project(nav.origin,saved[c.id]||c);if(Math.hypot(p.x-s.player.x,p.z-s.player.z)>150)continue;
   // Unknown coverage never becomes a rejected or hostile location; retry after loading.
   if(safeAt(p.x,p.z)!==false||!Number.isFinite(ground(p.x,p.z)))continue;
   let fit=!!saved[c.id];if(!fit){for(const option of O.sites(c)){const q=G.project(nav.origin,option);fit=suitable(option,q);yield;if(fit===null)break;if(fit){c=option;p=q;break;}}}if(fit===null)continue;if(fit===false){probes.push({id:c.id,reason:lastReason});if(probes.length>12)probes.shift();rejected.add(c.id);if(rejected.size>128)rejected.delete(rejected.values().next().value);continue;}
   if(fit)build(saved[c.id]||c,p);
  }
 ...
 function encounter(pose){const candidates=[...visible.values()].filter(v=>data().camps[v.id]&&!data().camps[v.id].liberated&&Math.hypot(v.point.x-pose.x,v.point.z-pose.z)<75).sort((a,b)=>Math.hypot(a.point.x-pose.x,a.point.z-pose.z)-Math.hypot(b.point.x-pose.x,b.point.z-pose.z));const v=candidates[0];if(!v)return null;const c=data().camps[v.id];return{campId:c.id,guards:c.hp.map((hp,i)=>({id:c.id+':guard:'+i,campId:c.id,member:i,hp,maxHp:O.HP,position:{x:v.point.x+(i-1)*3,z:v.point.z+(i===1?3:-2)}})).filter(g=>g.hp>0)};}
 ...
 return{update,encounter,blocked,mapPoints,dispose,isFriendly:(x,z)=>[...visible.values()].some(v=>data().camps[v.id]?.liberated&&Math.hypot(x-v.point.x,z-v.point.z)<18),diagnostics:()=>({visible:[...visible.values()].map(v=>({id:v.id,point:v.point,uuid:v.group.uuid})),saved:Object.keys(data().camps).length,probes:probes.slice(),scanCount,maxScanMs})};
}
