...
A bird visibly winds up for 0.55 s, lunges for 0.22 s, then recovers for 0.35 s. Damage uses the existing physical Peck resolver, armour and barriers; readiness remains SPD/second with 100 units per beat. The hit rechecks distance, line of sight to the player’s torso and wilderness eligibility, so the player can step away during the wind-up. A low canopy around the eye-height camera cannot make the player immune to a body hit; solid walls still block it. Safe settlements, homes, rooms, flight, hidden windows and unknown terrain retain their existing protection and cancellation rules. No loot, coins or GPS/quest rewards are added.
...
All changed dependency URLs use `wilderness-birds-v395-20260913` in index and all three service-worker lists, and the new visual module is in the guarded updater. The release owner promotes the global build/cache together with their world changes. Current first-person HUD/map modules keep their v394 pins.
