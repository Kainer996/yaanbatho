...
function attach(s){
 ...
 function refreshDoors(){if(s.options.room||s.flight)return;const version=s.continuity?.placeVersion||0;if(version===doorVersion)return;doorVersion=version;doors.length=0;unresolved.length=0;s.source.scene.updateMatrixWorld(true);for(const building of s.continuity?.buildings?.()||s.source.buildings||[]){if(building.userData.construction)continue;building.traverse(model=>{const d=model.userData;if(!d.architecture||!d.door||!d.footprint)return;const target={buildingId:building.userData.buildingId,seed:building.userData.wardSeed??s.options.seed,homeId:d.homeId||''};const info=valid(target);if(!info)return;const scale=model.getWorldScale(new T.Vector3()).z||1;const p=model.localToWorld(new T.Vector3(d.door.x,0,Math.max(d.door.z,d.footprint.maxZ)+.9/scale));const entry=model.localToWorld(new T.Vector3(d.door.x,0,d.door.z));const direction=p.clone().sub(entry).normalize();
