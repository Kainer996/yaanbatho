...
(function(root){'use strict';function attach(s){
 ...
 function select(){if(s.room||s.player.mode==='fly'||s.uiBusy||s.failed||document.hidden)return null;let best=null,distance=Infinity;for(const n of nodes){const d=(n.x-s.player.x)**2+(n.z-s.player.z)**2;if(d<distance&&d<10&&core.inReach(s.player,n,s.world)){distance=d;best=n;}}return best;}
 ...
 function rebuildCollision(){if(s.room){pendingCollision=true;return;}pendingCollision=false;const rebuilt=root.BurbzVillageWalkScene.create(T,s.source.scene,s.source.buildings,s.source.movers,s.source.scene.userData.walkTerrain);s.world=s.continuity?.replaceBase(rebuilt)||rebuilt;if(!s.world.allowed(s.player.x,s.player.z)){
