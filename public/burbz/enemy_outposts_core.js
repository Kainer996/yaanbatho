...
const kind=id=>['chickenz','ravenz','peregrinez','eaglez','hawkez','owlez'].includes(id)?id:'chickenz';
...
function normalize(raw){const s=empty();s.clock=Math.max(0,Number(raw?.clock)||0);for(const [id,c]of Object.entries(raw?.camps||{}).slice(0,MAX_SAVED)){const m=/^outpost-v1:(-?\d+):(\d+)$/.exec(id);if(!m||!c)continue;const p=candidate(+m[1],+m[2],c.site);if(p.id!==id||!G.validCoordinate(p))continue;const hp=Array.from({length:DEFENDERS},(_,i)=>Math.max(0,Math.min(HP,finite(c.hp?.[i])?c.hp[i]:HP))),liberated=hp.every(n=>n===0)&&c.liberated===true;s.camps[id]={...p,hp,enemyKinds:Array.from({length:DEFENDERS},(_,i)=>kind(c.enemyKinds?.[i])),liberated,liberatedAt:liberated?Math.max(0,Number(c.liberatedAt)||0):0,credit:liberated?Math.max(0,Math.min(CAP,Number(c.credit)||0)):0,at:Math.max(0,Number(c.at)||s.clock)};}return s;}
function discover(s,p,now,enemyKinds=[]){if(s.camps[p.id])return false;if(Object.keys(s.camps).length>=MAX_SAVED)return false;const m=/^outpost-v1:(-?\d+):(\d+)$/.exec(p.id);if(!m)return false;const exact=candidate(+m[1],+m[2],p.site);if(G.distance(exact,p)>.1)return false;s.camps[p.id]={...exact,hp:Array(DEFENDERS).fill(HP),enemyKinds:Array.from({length:DEFENDERS},(_,i)=>kind(enemyKinds[i])),liberated:false,liberatedAt:0,credit:0,at:Math.max(s.clock,now||0)};return true;}
